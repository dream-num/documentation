...
## 验证重启后数据保留 [#verify-persistence]

先停止 Quick Start，再从仓库根目录运行 `pnpm example:database-adapter`。打开 `http://127.0.0.1:3010/?unit=persistent-sheet&type=2`，修改单元格并在另一个浏览器确认。停止服务，再执行同一命令并打开同一 URL：修改应仍然存在。数据库位于 `examples/database-adapter/.data/collaboration.sqlite`，重启时不要删除。服务先读取已有 Unit，仅在 `UNIT_NOT_FOUND` 时调用 `createUnitFromData()`，不会每次启动覆盖数据。对照 [server/main.ts](https://github.com/dream-num/univer-collaboration-examples/blob/main/examples/database-adapter/server/main.ts) 迁入应用。
...
- `(unitID, sid, reqId)` 提交幂等记录[^submission-idempotency]；
- Unit 软删除、恢复与永久删除状态。
...
| Revision CAS[^revision-cas]    | 只有 head revision 等于预期值时才能确认下一次提交        |
...
[^revision-cas]: *CAS（比较并交换）* 仅在当前值仍等于预期值时更新数据。这里必须原子地检查当前修订号并写入下一状态，避免两个并发提交同时占用同一修订号。

[^submission-idempotency]: 幂等性允许重复尝试同一次提交，而不会把编辑应用两次。重试时要保留原提交标识；即使内容相同，使用新标识也会被视为新的提交。
