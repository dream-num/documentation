...
If you have a client license, set `UNIVER_LICENSE` in your shell **before building**. The example’s `vite.config.ts` embeds it into the browser bundle. Setting it only when starting an already-built server will not update the browser. See [client license configuration](/guides/license).
