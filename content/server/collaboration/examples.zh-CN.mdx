...
| [Database Adapter](#database-adapter) | 数据库适配器 | `pnpm example:database-adapter` |
...
| [Multi-node](#multi-node)             | 按文档路由的多节点部署 | `pnpm example:multi-node` |
...
展示内置 SQLite Adapter 的用法，以及自定义 Database Adapter 的实现。
...
提交。页面可在 **Alice · Editor** 与只读 **Bob · Viewer** 两个教学账号之间切换。
...
通过多节点部署，将不同文档的协同负载分散到多个服务节点，减轻单节点压力，提升整体并发处理能力。

```bash
pnpm example:multi-node
```

打开 `http://127.0.0.1:3010/`，点击 **New Sheet** 创建文件。将完整 URL 复制到另一个标签页，
验证同一文档的协同编辑；再创建几份文件，观察它们在不同节点上的分配情况。

- [示例说明](https://github.com/dream-num/univer-collaboration-examples/blob/main/examples/multi-node/README.zh-CN.md)
