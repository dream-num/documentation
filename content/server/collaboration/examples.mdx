...
| [Database Adapter](#database-adapter) | Database adapters | `pnpm example:database-adapter` |
...
| [Multi-node](#multi-node) | Document-based multi-node routing | `pnpm example:multi-node` |
...
Demonstrate how to use the built-in SQLite Adapter and implement a custom Database Adapter.
...
Distribute collaboration workloads for different documents across multiple server nodes to reduce the load on each node and increase overall concurrency.

```bash
pnpm example:multi-node
```

Open `http://127.0.0.1:3010/` and select **New Sheet** to create a file. Copy the full URL to another tab
to verify collaborative editing of the same document. Then create a few more files to observe how they
are distributed across nodes.

- [Example guide](https://github.com/dream-num/univer-collaboration-examples/blob/main/examples/multi-node/README.md)
