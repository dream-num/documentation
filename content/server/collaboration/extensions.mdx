...
## Connect the Web SDK history viewer [#web-history]

Mount the History Endpoint alongside the collaboration endpoints. The Web SDK `historyServerUrl` is the prefix for `/{unitId}/list`, `/{unitId}/creators`, and `/{unitId}/cs`; its default is `/universer-api/history`. If you expose a different prefix, configure the host History plugin and product History UI plugin with the same URL.

Restoring a version goes through the collaboration mutation path, not a standalone history REST restore endpoint. Keep collaboration loading, submission, and authorization configured. Read-only comparison of two materialized snapshots through `createUnitComparisonEngine()` is separate: it does not require a running history service and does not persist or restore versions.

## Live Share integration [#live-share]

Live Share synchronizes the presenter’s view with followers in the same Sheet. It reuses the collaboration Session and Unit room; it is distinct from Presence, which displays remote cursors without moving your view.

**Version boundary:** `@univerjs-pro/collaboration-endpoint@1.0.0-rc.0` handles cursor Presence for `INGEST` messages but does not handle `liveshare.*`. It has no ready-made Live Share service to register. Your backend must implement the protocol below, or use a version that explicitly provides it. Registering `UniverLiveSharePlugin` alone is insufficient.

Start with [browser integration](/server/collaboration/browser-integration) and [identity and authorization](/server/collaboration/identity-and-authorization). On the frontend, register `UniverLiveSharePlugin` after collaboration and license configuration, before loading the shared Sheet. See the [Web SDK guide](/guides/sheets/features/live-share).

| Client event | Backend responsibility |
| --- | --- |
| `liveshare.request_host` | Authorize presentation requests using the authenticated Session and joined Unit; choose a presenter and broadcast `liveshare.new_host`. |
| `liveshare.operation` | Accept operations only from the current presenter; attach the trusted presenter member ID, broadcast within the Unit, and cache the latest value for each operation ID. |
| `liveshare.fetch_operations` | Return the current presenter and cached operations to an authorized joining or reconnecting member. |
| `liveshare.terminate` | Validate that the sender may end the presentation, clear presentation state, and notify followers. |

Use the Session’s `memberID` to identify a connection, not a client-supplied user ID. Check document access and presentation permissions; validate operation IDs and payloads before forwarding them. Clear or transfer presenter state when the presenter leaves. View state is temporary and must not be stored as document changesets.

Verify with two independent browser sessions on the same Sheet: start presenting, switch worksheets, scroll and zoom, stop and resume following, reconnect a follower, then close the presenter. Also verify that an unauthorized member cannot become the presenter or inject operations. **Successful cell-edit synchronization does not prove Live Share works.**
