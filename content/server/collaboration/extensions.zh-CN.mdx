...
构造 History Service 时即订阅 Core Service 的 `unitCreated` 和 `changesetCommitted` Event 并更新索引。
`dispose()` 会解除这些订阅。默认策略按时间窗口和特殊 mutation 分段，而不是每条 changeset 都创建一个历史项。

History 提供 `getHistoryList`、`listHistoryCreators` 和 `getHistoryChangesets` 读取 Middleware。索引由
Service 在内部维护。`userProvider` 只补全姓名、头像等展示信息，不负责认证或授权。Event Listener 失败
不会回滚已经确认的 Core changeset，因此 History 始终应被理解为可重建的派生数据。
