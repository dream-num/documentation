...
## 连接 Web SDK 历史界面 [#web-history]

将 History Endpoint 与协同端点一起挂载。Web SDK 的 `historyServerUrl` 是 `/{unitId}/list`、`/{unitId}/creators` 和 `/{unitId}/cs` 的共同前缀，默认值为 `/universer-api/history`。如果应用使用其他前缀，请为宿主 History 插件和品类 History UI 插件配置相同的地址。

恢复版本通过协同 mutation 提交，不是调用独立的历史恢复 REST 接口。请保留协同加载、提交与授权配置。`createUnitComparisonEngine()` 对两份完整快照进行只读比较，不需要运行历史服务，也不负责保存或恢复版本。

## 演示跟随集成 [#live-share]

演示跟随将演示者的视图同步给同一 Sheet 中的跟随者，复用协同 Session 和 Unit 房间。它与只显示远端光标、不移动当前视图的 Presence 是两种不同能力。

**版本边界：**`@univerjs-pro/collaboration-endpoint@1.0.0-rc.0` 对 `INGEST` 消息只处理光标 Presence，尚未处理 `liveshare.*`，也没有可直接注册的 Live Share 服务。业务后端需实现下列协议，或使用明确提供该能力的版本。仅注册 `UniverLiveSharePlugin` 不足以启用演示跟随。

先完成[浏览器集成](/server/collaboration/browser-integration)和[身份与权限](/server/collaboration/identity-and-authorization)。前端在协同与许可证配置后、加载共享 Sheet 前注册 `UniverLiveSharePlugin`，具体用法见 [Web SDK 指南](/guides/sheets/features/live-share)。

| 客户端事件 | 后端职责 |
| --- | --- |
| `liveshare.request_host` | 使用已认证的 Session 和已加入的 Unit 校验演示权限，确定演示者后广播 `liveshare.new_host`。 |
| `liveshare.operation` | 只接受当前演示者的操作，补入可信的演示者 member ID，在 Unit 内广播，并按 operation ID 缓存最新值。 |
| `liveshare.fetch_operations` | 向有权限的新加入或重连成员返回当前演示者和已缓存的操作。 |
| `liveshare.terminate` | 校验发送者是否可以结束演示，清理演示状态并通知跟随者。 |

使用 Session 的 `memberID` 标识连接，不信任客户端传入的用户 ID。校验文档访问和演示权限，转发前验证 operation ID 与参数；演示者退出时清理或转移演示状态。视图状态是临时数据，不应作为文档 changeset 持久化。

使用两个独立浏览器会话打开同一 Sheet，依次验证：开始演示、切换工作表、滚动和缩放、停止与恢复跟随、跟随者重连、演示者关闭页面。同时验证无权限成员不能成为演示者或注入操作。**单元格修改能够同步，不代表演示跟随已经可用。**
