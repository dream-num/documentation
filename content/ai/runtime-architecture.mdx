...
When a CLI command runs, the expensive part is usually not parsing arguments. It is turning a Unit[^document-unit] into a live Univer
...
This chapter focuses on two Runtime[^live-runtime] capabilities:
...
| Worker Runtime | When the pool first encounters a key           | On TTL/LRU[^runtime-cache] eviction, invalidation, daemon shutdown, or Worker crash | One Worker, one backend handle, one loaded Unit, and collaboration state |
...
[^runtime-cache]: TTL evicts an idle cached Runtime after its configured lifetime; LRU evicts the least recently used entries when capacity is limited. These policies release runtime resources; they do not delete the persisted Unit.
