...
Worktree[^document-worktree] 为 Agent 提供隔离的编辑空间。Agent 可以在 draft 中多轮 pull、inspect、execute 和 commit，
...
[^document-worktree]: 这里的 Worktree 是文档内容的独立协同草稿，不是 Git worktree，也不是源码目录的检出副本。草稿修改需要经过审阅与合并流程才会进入主线。
