...
For each content command, the business CLI loads the target Unit[^document-unit] and synchronizes its latest content. From the agent's
...
Collaboration Runtime[^live-runtime] is the Unit execution environment and collaboration client for agents in Node.js. One Runtime
...
[^document-unit]: A Unit is one document instance, such as a workbook or a text document. A worksheet is part of a workbook, not a separate Unit.

[^live-runtime]: Here, Runtime means the live Univer instance and the resources used to operate on a loaded Unit, not the Node.js executable or the saved document snapshot.
