...
请让所有 `@univerjs/*` 和 `@univerjs-pro/*` 包保持**相同版本**。
...
<Steps>
  ...
  <Step>
    导入 PDF Facade[^facade-entry] 入口。这个副作用导入会向 `FUniver` 添加 PDF 方法和枚举：
...
当前 `@univerjs-pro/pdfs-ui` 没有导出独立的移动端 UI 插件。不要使用不存在的 `MobileUIPlugin` 导入或 `mobile: true` 选项。容器变窄不代表已完整支持触摸交互，需要在目标设备上验证现有 UI。
