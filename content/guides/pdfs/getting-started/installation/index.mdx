...
Univer PDFs is a Univer Office SDK product and does not currently provide a preset. Create PDF applications with plugin mode.
...
<Steps>
  ...
  <Step>
    Import the PDF Facade[^facade-entry] entry. This side-effect import adds PDF methods and enums to `FUniver`:
...
The current `@univerjs-pro/pdfs-ui` package does not export a dedicated mobile UI plugin. Do not invent a `MobileUIPlugin` import or a `mobile: true` option. A narrow container is not evidence of complete touch support; evaluate the existing UI on the target devices.
