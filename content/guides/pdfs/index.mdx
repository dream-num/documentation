...
Univer PDFs is the PDF product in Univer Office SDK. It represents a PDF as a formal `UNIVER_PDF` unit, preserves the imported source document, and stores accepted edits in a separate durable edit layer. The browser UI renders both layers as one continuous document.
...
To get started, check out the [Quick Start](/guides/pdfs/getting-started/installation) guide.
...
For collaboration, shared history or comments, and file conversion, compose the required modules from [Univer Server SDK](/server) in your application backend. Your application provides authentication, permissions, storage, and deployment; browser-only editing does not require these services.
