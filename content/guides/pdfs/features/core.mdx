...
The side-effect import `@univerjs-pro/pdfs/facade` extends `FUniver` with the PDF API. Calling `createPdf()` without a source document creates a blank A4 PDF; passing a snapshot returned by `save()` restores that unit.
...
Use Facade methods or commands to update a running PDF. Do not mutate `snapshot.document` or `snapshot.editState` directly.
