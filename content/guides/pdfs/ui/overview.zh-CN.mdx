...
UI 会在内部调用 PDF 命令和服务，功能区中有按钮并不代表存在同名公共方法。支持的编程式编辑请查看 [PDF Facade 指南](/guides/pdfs/getting-started/facade)，格式转换请查看[导入导出](/guides/pdfs/features/import-export)，浏览器打印请查看[打印](/guides/pdfs/features/print)。

## 菜单顺序与网格布局 [#menu-configuration]

通过 [`order`、`gridLayout` 和 `updateMenuConfig()`](/guides/pdfs/features/core#menu-order-and-grid-layout) 配置页签、分组和菜单项的顺序、网格位置及运行时更新；指南也提供自定义菜单 Facade 配置的入口。
