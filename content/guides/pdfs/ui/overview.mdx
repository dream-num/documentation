...
`@univerjs-pro/pdfs-ui` mounts PDF units in the standard Univer workbench and is licensed through Univer Office SDK.
...
The UI invokes PDF commands and services internally. A ribbon button does not imply a matching public method. Use the [PDF Facade guide](/guides/pdfs/getting-started/facade) for supported programmatic editing, [Import and export](/guides/pdfs/features/import-export) for conversion, and [Print](/guides/pdfs/features/print) for browser printing.

## Menu order and grid layout [#menu-configuration]

Use [`order`, `gridLayout`, and `updateMenuConfig()`](/guides/pdfs/features/core#menu-order-and-grid-layout) to configure tab, group, and item order, grid placement, and runtime updates. The guide also links to the custom-menu Facade options.
