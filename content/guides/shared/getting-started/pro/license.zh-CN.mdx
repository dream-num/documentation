...
- 对于服务端部署，已完成 Docker 或 Kubernetes 环境的配置。如果你尚未完成配置，请参考 [Docker 部署文档](/zh-CN/guides/docs/getting-started/pro/deploy#deploy-to-docker-compose) 或 [Kubernetes 部署文档](/zh-CN/guides/docs/getting-started/pro/deploy#deploy-to-k8s)。
...
2. 下载许可证文件
![下载许可证](@/content/guides/shared/getting-started/pro/license/new-02-cn.png)
...
2. 点击“获取试用许可证”按钮
![获取许可证](@/content/guides/shared/getting-started/pro/license/new-01-cn.png)
...
<Tabs items={['docker compose', 'K8s']}>
  <Tab>
    ...
    2. 在 univer-server 目录下运行服务脚本 `bash run.sh` 重启 universer 服务。
  </Tab>
  <Tab>
    1. 执行以下命令
...
![前端验证许可证失败](@/content/guides/shared/getting-started/pro/license/new-06.png)

2. 在输入合法许可证的情况下，项目将正常运行，且不会受到限制。
![前端验证许可证成功](@/content/guides/shared/getting-started/pro/license/new-07.png)

### 验证 Univer Server 中的许可证

访问 `host:8000/universer-api/license/key` 查看许可证权益信息。 例如：若你的服务在本地运行请访问 `http://localhost:8000/universer-api/license/key `。
...
![后端验证许可证](@/content/guides/shared/getting-started/pro/license/new-05.png)
