...
<Callout>
  如需了解更多设计细节请阅读 [Univer 命令系统](/zh-CN/guides/recipes/architecture/univer#command-system) 。
</Callout>
...
- [扩展 Canvas](/zh-CN/guides/recipes/practices/custom-canvas)
- [自定义主题](/zh-CN/guides/recipes/practices/custom-theme)
...
- `locales` 下可以设置多种语言，命名规则参考 [LocaleType](https://reference.univer.ai/zh-CN/globals#localetype)。可以在 `functionList` 下添加多个公式的翻译。详细的字段说明请参考 [如何在 Formula Engine 中添加公式](/zh-CN/guides/recipes/practices/custom-formula#how-to-add-formulas-in-formula-engine) 的部分。

<Callout>
  如果想复用公式系统提供的算法，增强公式算法的能力，可以通过插件配置的方式注册自定义公式，详细教程请参考 [自定义公式](/zh-CN/guides/recipes/practices/custom-formula/)。
</Callout>
