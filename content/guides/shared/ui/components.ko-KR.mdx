...
<Callout type="warning" title="주의">
  1. 이 메서드를 사용하기 전에 Univer의 렌더링이 완료되었는지 확인하세요.
  2. 등록이 필요한 컴포넌트는 사용하기 전에 올바르게 등록되었는지 확인하세요.
  3. 메모리 누수를 방지하려면 `dispose()` 메서드로 추가한 컴포넌트를 정리하고 제거하세요.
</Callout>

### 사용자 지정 메뉴 항목 추가

상단 메뉴 모음(Ribbon)과 오른쪽 클릭 메뉴(Context Menu) 모두에 사용자 지정 컴포넌트를 넣을 수 있습니다. 메뉴 항목에 사용자 지정 컴포넌트를 추가하려면 사용자 지정 플러그인을 만들어야 합니다. 빠르게 시작할 수 있도록 [사용자 지정 메뉴 항목 추가 모범 사례](/guides/recipes/tutorials/custom-plugin)를 준비했습니다.

### 기본 제공 컴포넌트 교체

<Callout type="error" title="경고">
  기본 제공 컴포넌트를 교체하면 일부 기능이 제대로 작동하지 않을 수 있습니다. 컴포넌트를 교체하기 전에 소스 코드와 문서를 충분히 읽고 신중하게 진행하세요.
</Callout>
