...
Next, we need to pop up the file selection box in the click event, read the CSV file selected by the user.Because this code does not involve Univer, so it will not be elaborated in this article. You can go to [Case Introduction](/en-US/guides/recipes/practices/csv-import-plugin#case-introduction) check the method `waitUserSelectCSVFile` source code.

Let's talk about how to convert the CSV two-dimensional array into the data structure `ICellData` of Univer.

[`ICellData`](/en-US/guides/sheets/model/cell-data) is the cell data structure in Univer, which contains the value and style of the cell, where the value is stored in the `v` attribute, and the style is stored in the `s` attribute.
...
The complete code of the plugin can be found in [Case Introduction](/en-US/guides/recipes/practices/csv-import-plugin#case-introduction).
...
- [ICellData](/en-US/guides/sheets/model/cell-data)
