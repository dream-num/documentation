...
export class MyFeatureRecorderController {
  constructor(
    @Inject(ActionRecorderService) private readonly actionRecorderService: ActionRecorderService,
  ) {
...
<Callout type="warning">
  `registerRecordedCommand` accepts commands and operations, but not mutation commands. Mutations are lower-level data changes and should be produced by replaying the recorded command instead.
</Callout>
...
export class MyReplayController {
  constructor(
    @Inject(ActionReplayService) private readonly replayService: ActionReplayService,
  ) {}
