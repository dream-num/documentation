...
本章节介绍 Univer 公式引擎的整体架构，建议在阅读本章节之前，先阅读[整体架构](/guides/recipes/architecture/univer)。

公式引擎设计的主要目标如下：

1. 支持 Univer 的不同文档类型和相关功能接入公式
2. 提供快速的使用体验，支持和 [web worker](/guides/recipes/architecture/web-worker) 和服务端计算
...
export class Vlookup extends BaseFunction {
  override calculate(
    ...
    const searchArray = (tableArray as ArrayValueObject).slice(0, [0, 1])

    // 对 tableArray 进行切片操作，获取 colIndexNumValue 指定列的矩阵
    const resultArray = (tableArray as ArrayValueObject).slice(0, [colIndexNumValue - 1, colIndexNumValue])
...
用于函数和函数描述的注册，用户也可以通过函数服务 API 注册自定义函数。
