...
<Tabs items={['Univer Sheets', 'Univer Docs']}>
  <Tab>
    ...
    If you are using collaborative editing features, you can use the `univerAPI.importXLSXToUnitIdAsync` method to import the `.xlsx` file and obtain a `unitId`, then load the workbook using the `univerAPI.loadServerUnit` method.
    ...
    Or if you just want to import the `.xlsx` file as a snapshot (`IWorkbookData`) and render it in the current instance, you can use the `univerAPI.importXLSXToSnapshotAsync` method.
