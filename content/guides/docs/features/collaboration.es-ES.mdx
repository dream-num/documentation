...
<Callout>
  El ajuste preestablecido `UniverDocsCollaborationPreset` de `@univerjs/preset-docs-collaboration` depende de los
  ajustes preestablecidos `UniverDocsDrawingPreset` y `UniverDocsAdvancedPreset` en tiempo de ejecución. Primero instale
  `@univerjs/preset-docs-drawing` y `@univerjs/preset-docs-advanced`.
</Callout>
...
- Puedes importar un archivo `.docx` al servidor Univer usando el método `FUniver.importDocToUnitIdAsync` proporcionado por [Importar Complemento](/guides/docs/features/import-export).
