...
<MetaData
  ...
  meta={{
    ...
    plugins: [{
      ...
      locale: '@univerjs-pro/exchange-client/locales/ja-JP',
...
<Callout type="warning" title="注意事項">
  インポート/エクスポート機能は Univer サーバーのサポートが必要です。Univer サーバーを正しくインストールして設定していることを確認してください。詳細は [Pro へのアップグレード](/guides/pro) を参照してください。
</Callout>
...
<Callout title="なぜバックエンドでインポート/エクスポートを実装するのか？">
  フロントエンドのみの実装では性能や結果が企業要件を満たせません。そのため、バックエンドサービスでインポート/エクスポートを提供しています。オープンソースの DOCX 解析ライブラリで `IDocumentData` 形式のデータ構造に変換し、Facade API で Univer に取り込めます。
</Callout>
...
| --- | --- | --- | --- |
| `unitId` | 必要 | サーバーに永続化して共有する協調ドキュメント | `importDOCXToUnitIdAsync` / `exportDOCXByUnitIdAsync` |
| スナップショット | 不要 | ローカル/オフライン処理、フォーマット変換のみ | `importDOCXToSnapshotAsync` / `exportDOCXBySnapshotAsync` |
...
共同編集が有効な場合、各ドキュメントは一意の `unitId` を持ちます。`univerAPI.importDOCXToUnitIdAsync` に `file` を渡すと `unitId` が返され、その `unitId` でドキュメントにアクセスできます。`file` は `File` オブジェクトまたはリモート URL を指定できます。
...
`univerAPI.importDOCXToSnapshotAsync` を使用すると、`.docx` を `IDocumentData` 形式で取得できます。
...
共同編集が有効な場合、各ドキュメントには一意の `unitId` があります。`univerAPI.exportDOCXByUnitIdAsync` に `unitId` を渡すと `File` が返ります。
...
// @univerjs-pro/exchange-client の downloadFile で保存
...
`univerAPI.exportDOCXBySnapshotAsync` に `IDocumentData` を渡すと `File` が返ります。
