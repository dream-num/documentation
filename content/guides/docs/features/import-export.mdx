...
| ----------- | ---------------------- | -------------------------------------------------------- | ------------------------------------------------------- |
| By `unitId` | Yes                    | Collaborative documents that are persisted on the server | `importDocToUnitIdAsync` / `exportDocByUnitIdAsync`     |
| By snapshot | No                     | Local or offline workflows, or format conversion only    | `importDocToSnapshotAsync` / `exportDocBySnapshotAsync` |
...
<Callout>
  The `UniverDocsAdvancedPreset` preset from `@univerjs/preset-docs-advanced` depends on the `UniverDocsDrawingPreset`
  preset at runtime. Please install `@univerjs/preset-docs-drawing` first.
</Callout>
...
interface IUniverDocsAdvancedPresetConfig {
  ...
  // The commercial license for Univer SDK Pro
  license?: string
}
```
