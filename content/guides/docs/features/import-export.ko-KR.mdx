...
<Callout title="Univer SDK가 백엔드 서비스를 통해 가져오기 및 내보내기를 구현하는 이유는 무엇인가요?">
...
메뉴 모음의 가져오기 항목을 통해 파일을 성공적으로 가져오면 페이지 왼쪽 아래에 알림이 표시됩니다. 알림에는 가져오기 성공 정보와 가져온 문서로 이동하는 링크가 포함됩니다. 기본 가져오기 및 내보내기 동작이 요구 사항에 맞지 않는다면 [문서 가져오기 사용자 지정](/blog/custom-import)을 참고해 가져오기 동작을 사용자 지정하는 방법을 알아보세요.
