...
<Callout type="warning" title="주의">
  가져오기 및 내보내기 기능을 사용하려면 Univer 서버의 지원이 필요합니다. Univer 서버가 올바르게 설치되고 구성되었는지 확인하세요. 자세한 내용은 [Pro로 업그레이드하기](/guides/pro)를 참고하세요.
</Callout>
...
<Callout title="Univer가 백엔드 서비스를 통해 가져오기 및 내보내기를 구현하는 이유는 무엇인가요?">
  프런트엔드만으로 구현한 가져오기 및 내보내기는 성능과 결과 측면에서 기업의 요구 사항을 충족하기 어렵습니다. 따라서 가져오기 및 내보내기 기능을 구현하는 백엔드 서비스를 제공합니다. 오픈 소스 DOCX 파싱 라이브러리를 사용해 파일을 `IDocumentData` 인터페이스에 맞는 데이터 구조로 파싱한 다음, Facade API로 데이터를 Univer에 가져올 수도 있습니다.
</Callout>
...
| --- | --- | --- | --- |
| `unitId` 사용 | 예 | 서버에 저장되는 협업 문서 | `importDOCXToUnitIdAsync` / `exportDOCXByUnitIdAsync` |
| 스냅샷 사용 | 아니요 | 로컬 또는 오프라인 워크플로 또는 형식 변환만 필요한 경우 | `importDOCXToSnapshotAsync` / `exportDOCXBySnapshotAsync` |
...
<Callout>
  `@univerjs/preset-docs-advanced`의 `UniverDocsAdvancedPreset` 프리셋은 런타임에 `UniverDocsDrawingPreset` 프리셋을 필요로 합니다. 먼저 `@univerjs/preset-docs-drawing`을 설치하세요.
</Callout>
...
<Callout type="warning" title="주의">
  이 메서드를 사용하려면 협업 기능이 필요합니다. 서버 문서를 만들고 `unitId`를 반환합니다. 협업 기능을 사용하지 않는다면 스냅샷 가져오기를 사용하세요.
</Callout>

협업 편집을 사용하면 각 문서에 고유한 `unitId`가 부여됩니다. `file` 매개변수와 함께 `univerAPI.importDOCXToUnitIdAsync`를 사용하면 문서에 접근할 수 있는 `unitId`가 반환됩니다. `file` 매개변수에는 `File` 객체나 원격 파일 URL을 사용할 수 있습니다.
...
<Callout>
  스냅샷 가져오기는 협업 기능이 필요하지 않습니다. 협업 문서에는 `unitId` 가져오기를 사용하세요.
</Callout>

`univerAPI.importDOCXToSnapshotAsync`를 사용해 `.docx` 파일을 가져오면 `IDocumentData` 형식의 문서 데이터가 반환됩니다.
...
협업 편집을 사용하면 각 문서에 고유한 `unitId`가 부여됩니다. `unitId` 매개변수와 함께 `univerAPI.exportDOCXByUnitIdAsync`를 사용하면 `File` 객체가 반환됩니다.
...
<Callout>
  스냅샷 내보내기는 협업 기능이 필요하지 않으며 로컬 워크플로에 적합합니다.
</Callout>

`IDocumentData`와 함께 `univerAPI.exportDOCXBySnapshotAsync`를 사용하면 `File` 객체가 반환됩니다.
