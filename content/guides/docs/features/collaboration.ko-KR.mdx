...
<Callout type="warning" title="주의">
  공동 편집 기능을 사용하려면 Univer 서버의 지원이 필요합니다. Univer 서버를 올바르게 설치하고 구성했는지 확인하세요. 자세한 내용은 [Pro로 업그레이드하기](/guides/pro)를 참고하세요.
</Callout>
...
<Callout>
  `@univerjs/preset-docs-collaboration`의 `UniverDocsCollaborationPreset` 프리셋은 런타임에 `UniverDocsDrawingPreset` 및 `UniverDocsAdvancedPreset` 프리셋에 의존합니다. 먼저 `@univerjs/preset-docs-drawing`과 `@univerjs/preset-docs-advanced`를 설치하세요.
</Callout>
...
- [가져오기 플러그인](/guides/docs/features/import-export)에서 제공하는 `FUniver.importDOCXToUnitIdAsync` 메서드를 사용하여 `.docx` 파일을 Univer 서버로 가져올 수 있습니다.
