...
<Callout>
  Die Voreinstellung `UniverDocsCollaborationPreset` von `@univerjs/preset-docs-collaboration` hängt von den
  Voreinstellungen `UniverDocsDrawingPreset` und `UniverDocsAdvancedPreset` zur Laufzeit ab. Bitte installieren Sie
  zuerst `@univerjs/preset-docs-drawing` und `@univerjs/preset-docs-advanced`.
</Callout>
...
- Sie können eine `.docx`-Datei mit der von der bereitgestellten Methode `FUniver.importDocToUnitIdAsync` in den Univer-Server importieren [Import-Plugin](/guides/docs/features/import-export).
