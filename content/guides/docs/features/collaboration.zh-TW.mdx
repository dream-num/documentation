...
<Callout>
  @univerjs/preset-docs-collaboration 的 `UniverDocsCollaborationPreset` 預設在執行時依賴 `UniverDocsDrawingPreset` 和
  `UniverDocsAdvancedPreset` 預設，請先安裝 @univerjs/preset-docs-drawing 和 @univerjs/preset-docs-advanced。
</Callout>
...
- 透過[匯入外掛](/guides/docs/features/import-export)提供的 `FUniver.importDocToUnitIdAsync` 方法，將 `.docx` 檔案匯入到 Univer 服務。
