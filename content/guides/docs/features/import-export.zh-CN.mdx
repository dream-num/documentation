...
<Callout type="warning" title="注意事项">
  导入导出功能需要 Univer 服务端支持，请确保你已经正确安装并配置了 Univer 服务端。具体请参考：[升级到 Pro](zh-CN/guides/docs/getting-started/pro/introduction)
</Callout>
...
通过菜单栏的导入入口成功导入后，页面左下角会弹出提示框，显示导入成功的信息，并提供访问导入文档的链接。若默认的导入导出行为无法满足你的需求，请查看[《自定义导入文档》](/zh-CN/guides/recipes/practices/import-docs)以了解如何自定义导入行为。
...
如果你拥有 Univer 的商业许可证，请参考[在客户端使用许可证](/zh-CN/guides/docs/getting-started/pro/license#在预设模式中使用)进行配置。
...
如果你拥有 Univer 的商业许可证，请参考[在客户端使用许可证](/zh-CN/guides/docs/getting-started/pro/license#在插件模式中使用)进行配置。
