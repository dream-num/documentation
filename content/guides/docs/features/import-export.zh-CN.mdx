...
<Callout title="为什么 Univer SDK 通过后端服务实现导入导出？">
  ...
  DOCX 解析库将文件解析为符合 `IDocumentData` 接口的数据结构，然后通过 Facade API 将数据导入到 Univer 中。
</Callout>
...
| -------------------- | ------------ | --------------------------------------- | ------------------------------------------------------- |
| 通过 `unitId`        | 需要         | 已启用协同、需要多人共享/继续协作的文档 | `importDocToUnitIdAsync` / `exportDocByUnitIdAsync`     |
| 通过快照（snapshot） | 不需要       | 本地/单人/离线导入导出，或仅做格式转换  | `importDocToSnapshotAsync` / `exportDocBySnapshotAsync` |
...
<Callout>
  @univerjs/preset-docs-advanced 的 `UniverDocsAdvancedPreset` 预设在运行时依赖 `UniverDocsDrawingPreset` 预设，请先安装
...
`UniverDocsAdvancedPreset` 配置项：

```typescript
interface IUniverDocsAdvancedPresetConfig {
  // Univer Server 的端点地址
  universerEndpoint?: string
  // Univer SDK Pro 商业许可证
  license?: string
}
```
...
import DocsExchangeClientZhCN from '@univerjs-pro/docs-exchange-client/locale/zh-CN' // [!code ++]
...
// 或者调用API: univerAPI.loadServerUnit(unitId, 1) 加载文档，具体参数参考 /reference/facade/univer#loadserverunit
```
