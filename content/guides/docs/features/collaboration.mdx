...
<MetaData
  ...
  meta={{
    ...
    plugins: [{
      ...
      locale: '@univerjs-pro/collaboration-client/locales/en-US',
    }, {
...
<Callout type="warning" title="Caution">
  The collaborative editing functionality requires support from the Univer server. Please ensure that you have correctly installed and configured the Univer server. For details, please refer to: [Upgrading to Pro](/en-US/guides/pro)
</Callout>
...
<Callout>
  The `UniverDocsCollaborationPreset` preset from `@univerjs/preset-docs-collaboration` depends on the `UniverDocsDrawingPreset` and `UniverDocsAdvancedPreset` presets at runtime. Please install `@univerjs/preset-docs-drawing` and `@univerjs/preset-docs-advanced` first.
</Callout>
...
- You can create a blank document using the [Create Document](/en-US/guides/pro/api#create-document) API.
- You can import a `.docx` file into the Univer server using the `FUniver.importDOCXToUnitIdAsync` method provided by the [Import Plugin](/en-US/guides/docs/features/import-export).
...
- [OT Algorithm and Univer’s Collaborative Editing Design](/en-US/blog/ot)
