---
title: Collaboration
icon: 'CloudCheck'
---

<MetaData
  lang="en-US"
  meta={{
    preset: [
      {
        client: '@univerjs/preset-docs-collaboration',
        locale: '@univerjs/preset-docs-collaboration/locales/en-US',
        style: '@univerjs/preset-docs-collaboration/lib/index.css',
      },
    ],
    plugins: [
      {
        client: '@univerjs-pro/collaboration',
      },
      {
        client: '@univerjs-pro/collaboration-client',
        facade: '@univerjs-pro/collaboration-client/facade',
        locale: '@univerjs-pro/collaboration-client/locale/en-US',
      },
      {
        client: '@univerjs-pro/collaboration-client-ui',
        facade: '@univerjs-pro/collaboration-client-ui/facade',
        locale: '@univerjs-pro/collaboration-client-ui/locale/en-US',
        style: '@univerjs-pro/collaboration-client-ui/lib/index.css',
      },
    ],
    license: true,
    server: true,
  }}
/>


Collaborative editing connects your browser editor to a backend. Start with [Collaboration integration](/server/collaboration/browser-integration) to run the example and connect your application's identity, permissions, and storage. See [Collaboration Quick Start](/server/collaboration/quick-start) for the browser and server configuration.

Create the document through an authenticated application API, then load the returned Unit identifier with the method below. Reuse that identifier when opening the same document again.

## Facade API [#loading-collaborative-documents]

```ts
import '@univerjs-pro/collaboration-client/facade'

const collaboration = univerAPI.getCollaboration()
const unit = await collaboration.loadDocAsync('your-unit-id')
```

[Node.js / AI SDK](/ai/content-operations) · [OT](/blog/ot)
