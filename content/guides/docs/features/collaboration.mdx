...
<Callout>
  The `UniverDocsCollaborationPreset` preset from `@univerjs/preset-docs-collaboration` depends on the
  `UniverDocsDrawingPreset` and `UniverDocsAdvancedPreset` presets at runtime. Please install
  `@univerjs/preset-docs-drawing` and `@univerjs/preset-docs-advanced` first.
</Callout>
...
- You can import a `.docx` file into the Univer server using the `FUniver.importDocToUnitIdAsync` method provided by the [Import Plugin](/guides/docs/features/import-export).
