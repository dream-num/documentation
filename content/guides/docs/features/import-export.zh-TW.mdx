...
<Callout title="為什麼 Univer SDK 透過後端服務實現匯入匯出？">
  ...
  DOCX 解析庫將檔案解析為符合 `IDocumentData` 介面的資料結構，然後透過 Facade API 將資料匯入到 Univer 中。
</Callout>
...
| -------------------- | ------------ | -------------------------------------- | ------------------------------------------------------- |
| 透過 `unitId`        | 需要         | 已啟用協同、需多人共享/持續協作的文件  | `importDocToUnitIdAsync` / `exportDocByUnitIdAsync`     |
| 透過快照（snapshot） | 不需要       | 本機/單人/離線匯入匯出，或僅做格式轉換 | `importDocToSnapshotAsync` / `exportDocBySnapshotAsync` |
...
<Callout>
  @univerjs/preset-docs-advanced 的 `UniverDocsAdvancedPreset` 預設在執行時依賴 `UniverDocsDrawingPreset` 預設，請先安裝
...
`UniverDocsAdvancedPreset` 配置項：

```typescript
interface IUniverDocsAdvancedPresetConfig {
  // Univer Server 的端點位址
  universerEndpoint?: string
  // Univer SDK Pro 商業許可證
  license?: string
}
```
