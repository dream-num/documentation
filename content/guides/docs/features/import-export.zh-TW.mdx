...
在啟用協同編輯的時候，每個文件都有一個唯一的 `unitId`。使用 `univerAPI.importDOCXToUnitIdAsync` 傳入 `file` 引數會返回 `unitId`，可以透過 `unitId` 來訪問文件。 `file` 引數可以是一個 `File` 物件，也可以是遠端檔案的 URL。
...
// 或者呼叫API: univerAPI.loadServerUnit(unitId, 1) 載入文件，具體引數參考 https://reference.univer.ai/zh-TW/classes/FUniver#loadserverunit
```
...
在啟用協同編輯的時候，每個文件都有一個唯一的 `unitId`。使用 `univerAPI.exportDOCXByUnitIdAsync` 傳入 `unitId` 引數會返回 `File` 物件。
