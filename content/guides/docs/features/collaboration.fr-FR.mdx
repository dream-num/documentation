...
<Callout type="warning" title="Attention">
  L’édition collaborative nécessite la prise en charge du serveur Univer. Assurez-vous d’avoir correctement installé et configuré le serveur Univer. Pour plus de détails, consultez la page [Passer à Pro](/guides/pro).
</Callout>
...
<Callout>
  Le preset `UniverDocsCollaborationPreset` du package `@univerjs/preset-docs-collaboration` dépend des presets `UniverDocsDrawingPreset` et `UniverDocsAdvancedPreset` lors de l’exécution. Installez d’abord `@univerjs/preset-docs-drawing` et `@univerjs/preset-docs-advanced`.
</Callout>
...
- Vous pouvez importer un fichier `.docx` sur le serveur Univer à l’aide de la méthode `FUniver.importDOCXToUnitIdAsync` fournie par le [plugin d’importation](/guides/docs/features/import-export).
