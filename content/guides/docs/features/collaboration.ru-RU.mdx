...
<Callout type="warning" title="Внимание">
  Для совместного редактирования требуется сервер Univer. Убедитесь, что вы правильно установили и настроили сервер Univer. Подробнее см. в разделе [Переход на Pro](/guides/pro).
</Callout>
...
<Callout>
  Предустановка `UniverDocsCollaborationPreset` из `@univerjs/preset-docs-collaboration` во время выполнения зависит от предустановок `UniverDocsDrawingPreset` и `UniverDocsAdvancedPreset`. Сначала установите `@univerjs/preset-docs-drawing` и `@univerjs/preset-docs-advanced`.
</Callout>
...
- Импортировать файл `.docx` на сервер Univer с помощью метода `FUniver.importDOCXToUnitIdAsync`, предоставляемого [плагином импорта](/guides/docs/features/import-export).
