...
<Callout>
  `@univerjs/preset-docs-collaboration` の `UniverDocsCollaborationPreset` は実行時に `UniverDocsDrawingPreset` と
  `UniverDocsAdvancedPreset` に依存します。先に `@univerjs/preset-docs-drawing` と `@univerjs/preset-docs-advanced`
  をインストールしてください。
</Callout>
...
- [インポートプラグイン](/guides/docs/features/import-export) の `FUniver.importDocToUnitIdAsync` で `.docx` を取り込む。
