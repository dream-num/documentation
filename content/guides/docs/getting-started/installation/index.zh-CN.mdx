...
与预设模式不同，插件模式默认并不包含 Facade API[^facade-entry]。下面的示例会使用 Facade API，因此需要在创建 API 实例前引入对应的 facade 包。
...
<Callout type="warning">
  ...
  `1.0.0-rc.0` 尚未导出 `UniverDocsMobileUIPlugin`。使用以下示例前，需要选择已包含该移动端入口的构建版本。
</Callout>

在 Univer Docs 中使用**移动端 UI 插件**提供触摸操作界面。[^mobile-ui-mode] 请在创建编辑器前确定 UI 模式。
...
* 在 [功能](/guides/docs/features/core) 相关章节中了解修改文档内容的所有操作。
