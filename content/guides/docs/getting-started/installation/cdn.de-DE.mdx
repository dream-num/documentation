---
title: Univer über CDN importieren
---

Wenn Sie keinen Paketmanager verwenden, können Sie Univer auch über `<script>`-Tags importieren.

#### Namespace Konvention

Der globale Build (UMD) von Univer stellt einen Namespace für das globale Objekt bereit und folgt dabei der Namenskonvention unten:
...
## Plugin-Modus [#plugin-mode]

### Verwendung von Global Build

Univer stellt UMD-Builds für Plugins bereit. Wenn Sie CDN-Skript-Tags verwenden, müssen Sie die UMD-Builds jedes Plugins und deren Abhängigkeiten manuell einbinden. Dieser Prozess ist umständlich, daher empfehlen wir, ihn erst zu verwenden, nachdem Sie den Plugin-Mechanismus, einschließlich der Plugin-Abhängigkeiten und der Reihenfolge der Stildateien, vollständig verstanden haben.
...
unpkg und jsDeliver unterstützen beide die Angabe einer bestimmten Version der Ressource. Wenn Sie beispielsweise die Version `x.y.z` von Univer verwenden möchten, können Sie einfach `@<version>` zur URL hinzufügen:
