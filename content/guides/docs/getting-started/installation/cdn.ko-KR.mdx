...
패키지 관리자를 사용하지 않는 경우에도 `<script>` 태그를 통해 Univer를 가져올 수 있습니다.
...
Univer는 플러그인용 UMD 빌드를 제공합니다. CDN 스크립트 태그를 사용할 때는 각 플러그인과 그 종속 항목의 UMD 빌드를 직접 포함해야 합니다. 이 과정은 번거로우므로 플러그인 간 종속 관계와 스타일 파일의 순서를 비롯한 플러그인 메커니즘을 충분히 이해한 후에만 사용하는 것이 좋습니다.
...
unpkg와 jsDeliver는 모두 리소스의 특정 버전을 지정할 수 있습니다. 예를 들어 Univer의 `x.y.z` 버전을 사용하려면 URL에 `@<version>`을 추가하면 됩니다.
