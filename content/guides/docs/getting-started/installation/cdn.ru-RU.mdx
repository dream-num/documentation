...
Если вы не используете менеджер пакетов, Univer можно подключить с помощью тегов `<script>`.
...
Univer предоставляет UMD-сборки для плагинов. При подключении через CDN необходимо вручную добавить UMD-сборки каждого плагина и его зависимостей. Этот процесс довольно сложен, поэтому рекомендуем использовать такой способ, только если вы хорошо понимаете механизм работы плагинов, включая их зависимости и порядок подключения файлов стилей.
...
И unpkg, и jsDelivr позволяют указывать конкретную версию ресурса. Например, чтобы использовать Univer версии `x.y.z`, достаточно добавить `@<version>` в URL:
