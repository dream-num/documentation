...
透過 `@univerjs-pro/boards-exchange-client`，畫板可以匯入 Mermaid 原始碼並匯出 PPTX 檔案。
...
匯出 PPTX 時，請透過 [Univer Server SDK](/server/import-export) 整合轉換後端，並確認其支援 Board 快照。[用戶端授權](/server/license)需另外設定；註冊前端外掛不會自動提供轉換介面。
