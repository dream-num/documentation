...
PPTX 내보내기는 [Univer Server SDK](/server/import-export)로 변환 백엔드를 통합하고 Board 스냅샷 지원을 확인해야 합니다. [클라이언트 라이선스](/server/license)는 별도로 설정합니다. 브라우저 플러그인 등록만으로 변환 API가 제공되지는 않습니다.
