...
For PPTX export, integrate a conversion backend using [Univer Server SDK](/server/import-export) and verify support for Board snapshots. Configure the [client license](/server/license) separately; registering a browser plugin does not provide conversion endpoints.
