  ...
  meta={{
    ...
    plugins: [
      ...
      },
      {
        ...
        locale: '@univerjs-pro/boards-history-ui/locale/en-US',
...
After you move elements, edit text, or change connectors, open history to inspect earlier versions, compare changes, and restore a version when permitted. History reads server-persisted versions. Undo and redo apply to the current editing session and do not require a history service.


## Prepare collaboration and history services

Complete [collaboration integration](/server/collaboration/browser-integration) and [license configuration](/server/license). Verify that two sessions can edit the same Boards document and that changes survive a reload.
...
To verify the integration, move elements, edit text, or change connectors, wait for collaboration confirmation, and inspect the history list and version content. Reopen the application and restart the service to check persistence. Restore an earlier version and verify that another browser session receives the restored state.
