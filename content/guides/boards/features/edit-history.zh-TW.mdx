  ...
  meta={{
    ...
    plugins: [
      ...
      },
      {
        ...
        locale: '@univerjs-pro/boards-history-ui/locale/zh-TW',
...
修改 元素位置、文字或連接線 後，可開啟歷史記錄檢視較早的版本、比較變更，並在權限允許時還原版本。歷史記錄讀取的是伺服器持久化的版本；復原與重做只作用於目前的編輯工作階段，不需要歷史服務。

## 準備協同與歷史服務

完成[協同整合](/server/collaboration/browser-integration)與[授權設定](/server/license)。確認兩個工作階段可以共同編輯同一份 Boards 文件，且重新載入後修改仍然保留。
...
驗證整合時，修改 元素位置、文字或連接線，等待協同確認後，檢查歷史清單與版本內容。重新開啟應用程式並重啟服務，以確認資料持久化。還原較早的版本，並確認另一個瀏覽器工作階段收到還原後的狀態。
