...
const board = univerAPI.createBoard({ name: '规划画板' })
const snapshot = board.save()
```

模型 Facade 提供 `createBoard`、`getActiveBoard`、`getBoard`，以及元素和容器操作。UI Facade 则为 `FBoard` 添加设置、搜索、聚焦和视口 API。接下来可阅读 [Boards Facade API 指南](/guides/boards/getting-started/facade)和[画板数据结构](/guides/boards/model/board-data)。
