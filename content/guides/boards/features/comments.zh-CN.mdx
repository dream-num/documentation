...
先配置[移动端编辑器](/guides/boards/getting-started/installation#mobile-support)。在本页插件模式的初始化代码中，将下列桌面端注册替换为同一个包导出的移动端插件，并在**打开文档前**完成。保留模型插件、样式、语言包和 Facade 导入，不要同时注册两个版本。
