...
PPTX のエクスポートには [Univer Server SDK](/server/import-export) で変換バックエンドを組み込み、Board スナップショットへの対応を確認します。[クライアントライセンス](/server/license)は別途設定します。ブラウザープラグインの登録だけでは変換 API は提供されません。
