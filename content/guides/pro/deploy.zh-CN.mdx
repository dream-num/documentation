...
- `v0.13.0`版本开始，新增了`collaboration-helper`服务，用于处理需要异步执行的CPU密集型任务，如生成快照等
- `v0.15.0`版本开始，引入了协同编辑分层调度，将不同规模sheet的操作调度到不同的协同服务集群，尽量降低大表编辑影响普通规模表格
...
Univer 的安装配置默认是未启用的，你需要根据你的使用需求决定是否需要启用，相关说明请查看[通过 USIP 与你的系统集成](/guides/pro/usip)
...
另外，`v0.15.0`版本开始，引入了协同编辑分层调度，你需要统计下在你们的使用场景中，表格的规模分布（doc和slide不会参与分层调度），以此来判断是否需要配置分层调度，如果需要，还要评估好需要多少个等级的集群、每个等级处理什么规模的表格（单元格总数量、导入耗时等指标）。具体配置在后续章节中说明。

#### 协同编辑资源需求计算

如果绝大部分都是小文档，可以选择 2c4g 规格的协同服务；否则建议选择 4c8g 规格的协同服务配置。不建议继续增加协同服务单实例的规格，会导致 CPU 资源过多的浪费。

- 单实例协同服务 2c4g/4c8g 能支撑 1100/1200 篇小规模文档的同时编辑，按编辑保存 QPS 算是 360/400
- 单 universer 服务 2c4g/4c8g 能支撑 1200/2200 篇小规模文档的同时编辑，按编辑保存 QPS 算是 400/730

#### 多人协同资源需求计算

多人协同时，除了编辑保存消耗资源外，更多的是协同消息广播对 CPU 资源的需求。协同消息广播都在 universer 服务中完成，不影响 node 服务的资源占用。

2c4g × 2 能支持向 1400 人每秒推送 10 次编辑操作。平均 1core 最高能支持每秒 3500 次编辑协同广播。

协同广播 QPS = 编辑保存 QPS × (活跃用户数/活跃文档数 − 1)，减去 1 是因为向自己的推送已经算在编辑操作中了。

#### 容量需求计算器

你可以使用下方的计算器，根据你的实际业务参数快速估算所需的资源配置。输入活跃文档数、保存频率、协同比例等参数后，计算器会自动给出编辑 QPS、协同广播 QPS 以及推荐的 Node 服务、Universer 和数据库配置。
...
在后续运维 Univer 的后端服务时，你希望使用 Univer 集成的可观测组件还是接入自己的可观测系统？Univer 目前提供了 Prometheus 指标及 grafana 看板、服务日志。接入详情请阅读[运维手册](/guides/pro/sre-manual)。
...
在[通过 USIP 与你的系统集成](/guides/pro/usip)中已描述配置项的意义，这里不再重复，如果你需要启用，可如下配置：
...
这个在[通过 Univer 事件同步机制与你的系统集成](/guides/pro/event-sync)中已描述配置项的意义，这里不再重复，如果你需要启用，可如下配置：
...
`v0.15.0`版本开始引入，默认配置下不会生效，需要手动配置开启。目前会通过workbook的单元格总数、导入实际消耗的时间来决定应该调度到哪个等级的协同服务集群。针对刚通过导入创建的workbook，会根据导入实际耗时来决定；其他方式创建或触发新快照生成后，则通过单元格总数来决定。每次快照创建后，会将相关指标数据持久化到数据库并更新调度。

分层调度实际上就是将协同服务部署了多个集群，然后配置规则指定什么规模的表格应该调度到哪个集群处理。规则配置数据结构如下：
...
- `tiers`: 描述每一个集群信息，理论上可以有任意多个集群
- `enable`: 设置是否启用此集群，设置为`false`则此集群会被忽略
- `upgradeCellsThreshold`: 指定当sheet单元格总数达到此规模时才能调度到此集群
- `downgradeCellsThreshold`: 则指定：如果sheet当前正被调度到此集群，只有当单元格总数低于此值时才能被调度到更低等级的集群。这样可以避免两个等级交界处规模的sheet被频繁地切换调度
- `upgradeImportTimeThreshold`: 新导入的sheet暂时没有单元格总数统计，此时利用此配置来决定调度，作用机制同`upgradeCellsThreshold`，只是指标用的是导入的实际耗时

可以看出，必须得有一个集群的指标要求全是0，才能保证每个sheet都能被调度到。

按上述示例配置参数下的调度case：

- workbook1创建后的初始单元格数量为0，调度到normal集群
- workbook1随后被编辑到总单元格数量为1000时，调度到large集群
- workbook1总单元格数量又降至999时，由于大于设置的降级线900，仍然调度到large集群
- workbook1总单元格数量继续降至899时，则降级调度回normal集群
- workbook2被导入了，实际耗时61秒，已经超过huge集群的条件60秒，将被调度到huge集群处理

Univer 的 Docker Compose 部署脚本提供了选项，可选部署1个 or 2个等级的集群（分离普通表格和大表格）。
集群配置中的 `ENABLE_LARGE_TIER_COLLABORATION_SERVER` 参数控制调度时是否使用大表格集群，以及控制Docker Compose是否实际部署大表格集群服务。
如果你需要更精细的集群等级控制或使用 K8s 部署，请调整docker compose/charts的编排来实际部署对应等级的`collaboration-server`服务集群，然后按上述说明配置好调度路由表（`tiers`数组）

如果你直接使用univer提供的docker compose编排，可调整如下参数来配置是否启用第二个等级集群，以及什么规模的表格算作大表格需要分离处理。

<Tabs items={['docker compose', 'K8s']}>
  <Tab label="docker compose">
    默认部署只配置了两个等级：`normal`和`large`

    ```properties title=".env.custom"
    # 配置开启/关闭large集群，true-开启，false-关闭，默认关闭
    ENABLE_LARGE_TIER_COLLABORATION_SERVER=false

    # 如果开启large集群，当总单元格数达到此规模后将调度到large集群，默认2M
    LARGE_TIER_MIN_CELLS_COUNT=2000000

    # 如果sheet当前调度在large集群，则只有当总单元格数降至此值以下时才会降级回normal集群，默认1.95M
    LARGE_TIER_DOWNGRADE_CELLS_COUNT=1950000

    # 如果开启large集群，当workbook导入耗时超过配置后将调度到large集群，默认10s
    LARGE_TIER_MIN_IMPORT_SECONDS=10

    # large集群协同服务实例数，默认2
    LARGE_TIER_COLLABORATION_SERVER_REPLICATION_CNT=2

    # large集群协同服务实例内存限制，建议至少配置8GB，默认8GB
...
<Tabs items={['docker compose', 'K8s']}>
  ...
  <Tab label="K8s">
    使用 K8s 部署时的 values.yaml，注意：当你要修改多处配置时，需要你对最终的 yaml 进行合并。
