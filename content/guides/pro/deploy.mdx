...
- **Identity & Permissions**: determine whether USIP integration is required
...
If most documents are small, use the 2c4g specification for collaboration services; otherwise, choose 4c8g. Increasing the specification of a single collaboration-server instance further is not recommended, as it leads to CPU waste.

- A single collaboration-server instance (2c4g / 4c8g) supports approximately 1100 / 1200 concurrent edits of small documents, which translates to approximately 360 / 400 edit-save QPS.
- A single universer instance (2c4g / 4c8g) supports approximately 1200 / 2200 concurrent edits of small documents, which translates to approximately 400 / 730 edit-save QPS.

### Multi-user collaboration

In multi-user scenarios, collaborative message broadcasting consumes significant CPU. Broadcasting is handled entirely by universer and does not affect collaboration-server resource utilization.

A 2c4g × 2 configuration can push 10 edit operations per second to 1400 users. On average, one core can support up to 3500 collaborative broadcasts per second.

Broadcast QPS = edit-save QPS × (active users / active documents − 1). The minus 1 accounts for the fact that the user's own edit operation is already counted in edit-save QPS.

### Capacity calculator

Use the calculator below to quickly estimate resource requirements based on actual workload. Enter parameters such as active documents, save frequency, and collaboration ratio to obtain edit QPS, broadcast QPS, and recommended Node server, Universer, and database configurations.

<CapacityCalculator locale="en" />
