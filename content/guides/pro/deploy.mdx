...
If most of your documents are small, use the 2c4g spec for collaboration services; otherwise, choose 4c8g. We do not recommend increasing the spec of a single collaboration-server instance further, as it leads to CPU waste.

- A single collaboration-server instance (2c4g/4c8g) supports ~1100/1200 small-document concurrent edits, which translates to ~360/400 edit-save QPS.
- A single universer instance (2c4g/4c8g) supports ~1200/2200 small-document concurrent edits, which translates to ~400/730 edit-save QPS.

### Multi-user collaboration

In multi-user scenarios, collaborative message broadcasting consumes significant CPU. Broadcasting is handled entirely by universer and does not affect collaboration-server resources.

2c4g × 2 can push 10 edit operations per second to 1400 users. On average, 1 core can support up to 3500 collaborative broadcasts per second.

Broadcast QPS = edit-save QPS × (active users / active documents − 1). The minus 1 accounts for the fact that your own edit operation is already counted in edit-save QPS.

### Capacity calculator

Use the calculator below to quickly estimate resource requirements based on your actual workload. Enter parameters such as active documents, save frequency, and collaboration ratio to get edit QPS, broadcast QPS, and recommended Node server, Universer, and database configurations.
