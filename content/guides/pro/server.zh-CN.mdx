...
1. 下载离线安装包：//releases/univer-server/download
2. 上传到服务器解压
...
前往 [Demo UI 仓库](https://github.com/dream-num/univer-pro-sheet-start-kit) 拉取最新的 Demo UI 代码并启动服务。

访问服务链接后会自动创建空白文档，如果需要协作体验，可在另一个浏览器或无痕窗口打开同一链接。
...
如果你希望进一步将 Univer SDK Pro 的服务端集成到自己的系统中，以实现类似以下功能：

- 用户登录后才可访问 Univer SDK Pro 的服务
- 按角色或用户来管理文档的访问权限
- 获取文档评论的通知
- 操作文档资源（如创建、删除、修改文档等）

Univer SDK Pro 设计了两种集成方式，分别是：
...
![Univer SDK Pro 集成架构](./server/integration-arch.png)
