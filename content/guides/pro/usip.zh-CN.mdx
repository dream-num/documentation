...
目前 Univer Pro 定义了三类场景的 USIP 接口：
...
提供编辑文档的修改时间给自己的业务用，比如文档列表的时间显示
...
**请注意**，一旦你配置了开启 USIP，上述的所有 4 个 USIP API 的 url 都需要配置，并且需要你的服务正确实现。如何在部署的时候应用这些配置请参考[生产部署](/guides/pro/deploy)。
...
如果使用了[历史记录](/guides/sheets/features/edit-history)，你需要在[获取 HistoryUniver 的实例](/guides/sheets/features/edit-history#get-edit-history-univer-instance)后为其添加相同的拦截器：
