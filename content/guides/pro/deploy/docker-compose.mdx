...
4. Obtain Univer SDK Pro back-end services:
   - If the deployment server can access the public internet:
     ...
     - The CLI will attempt to automatically open your browser to retrieve an auth token from [//cli-auth](//cli-auth). If the browser does not open automatically, please visit this URL manually to obtain your token, then paste it into the terminal when prompted.
   - If the deployment server cannot access the public internet:
     - [Click here to download](//releases/univer-server/download) the All-in-one offline installation package provided by Univer.
