...
在准备升级前，你应该查看我们的版本介绍，确定你想要升级的版本，并查看我们的[生产部署](/zh-CN/guides/pro/deploy)，其中会介绍从哪个版本开始加入了新的配置项，从哪个版本开始旧的配置项不再支持。根据文档，查看自己的旧版本自定义配置文件 `.env.custom` 有哪些需要修改、有哪些新增配置需要自定义。确定好后，准备一份新的自定义配置文件 `.env.custom`，并且保留旧版本的 `.env.custom`以便后续能安全回滚。
...
在准备升级前，你应该查看我们的版本介绍，确定你想要升级的版本，并查看我们的[生产部署](/zh-CN/guides/pro/deploy)，其中会介绍从哪个版本开始加入了新的配置项，从哪个版本开始旧的配置项不再支持。根据文档，查看自己的旧版本自定义配置文件 `values.yaml` 有哪些需要修改、有哪些新增配置需要自定义。确定好后，准备一份新的自定义配置文件 `values.yaml`，并且保留旧版本的 `values.yaml`，以便后续能安全回滚。
