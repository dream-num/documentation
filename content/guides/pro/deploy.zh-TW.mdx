...
若絕大多數為小規模文件，協同服務可選擇 2c4g 規格；否則建議選擇 4c8g 規格。不建議持續提升單一協同服務實例的規格，以免造成 CPU 資源浪費。

- 單一協同服務實例（2c4g / 4c8g）可支援約 1100 / 1200 篇小規模文件的同時編輯，換算為編輯儲存 QPS 約為 360 / 400。
- 單一 universer 服務（2c4g / 4c8g）可支援約 1200 / 2200 篇小規模文件的同時編輯，換算為編輯儲存 QPS 約為 400 / 730。

### 多人協同資源需求計算

多人協同時，除編輯儲存消耗資源外，協同訊息廣播對 CPU 資源的需求更為顯著。協同訊息廣播都在 universer 服務中完成，不影響 Node 服務的資源占用。

2c4g × 2 的配置可支援向 1400 人每秒推送 10 次編輯操作。平均而言，1 核心最高可支援每秒 3500 次協同廣播。

協同廣播 QPS = 編輯儲存 QPS × (活躍使用者數 / 活躍文件數 − 1)，減去 1 是因為向自身的推送已計入編輯儲存 QPS 中。

### 容量需求計算器

可使用下方的計算器，依據實際業務參數快速估算所需的資源配置。輸入活躍文件數、儲存頻率、協同比例等參數後，計算器將自動輸出編輯 QPS、協同廣播 QPS 以及推薦的 Node 服務、Universer 與資料庫配置。
