...
如果絕大部分都是小文件，可以選擇 2c4g 規格的協同服務；否則建議選擇 4c8g 規格的協同服務配置。不建議繼續增加協同服務單實例的規格，會導致 CPU 資源過多的浪費。

- 單實例協同服務 2c4g/4c8g 能支撐 1100/1200 篇小規模文件的同時編輯，按編輯儲存 QPS 算是 360/400。
- 單 universer 服務 2c4g/4c8g 能支撐 1200/2200 篇小規模文件的同時編輯，按編輯儲存 QPS 算是 400/730。

### 多人協同資源需求計算

多人協同時，除了編輯儲存消耗資源外，更多的是協同訊息廣播對 CPU 資源的需求。協同訊息廣播都在 universer 服務中完成，不影響 node 服務的資源占用。

2c4g × 2 能支持向 1400 人每秒推送 10 次編輯操作。平均 1core 最高能支持每秒 3500 次編同廣播。

協同廣播 QPS = 編輯儲存 QPS × (活躍使用者數/活躍文件數 − 1)，減去 1 是因為向自己的推送已經算在編輯操作中了。

### 容量需求計算器

你可以使用下方的計算器，根據你的實際業務參數快速估算所需的資源配置。輸入活躍文件數、儲存頻率、協同比例等參數後，計算器會自動給出編輯 QPS、協同廣播 QPS 以及推薦的 Node 服務、Universer 和資料庫配置。
