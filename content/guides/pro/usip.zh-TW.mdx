...
Univer Pro 預設不內建帳號系統。若需要身分驗證、權限控制或與業務系統整合，請使用 USIP（Univer Server Integration Protocol）。USIP 是一種 SPI：由 Univer 定義介面，你的系統實作並回應。
...
完整範例可參考 [Univer Pro Sheet 起步套件](https://github.com/dream-num/univer-pro-sheet-start-kit/blob/main/src/setup-univer.ts)。
