...
Currently, Univer Pro defines USIP interfaces for three types of scenarios:
...
- 📄 Document-related
  - Notify of the document’s last edit timestamp
...
Provide the document's last edit time for internal business use, such as displaying it in the document list.
...
If you are using [Edit History](/guides/sheets/features/edit-history), you need to add the same interceptor to the `HistoryUniver` instance after [getting the instance](/guides/sheets/features/edit-history#get-edit-history-univer-instance):
