...
本ページは本番環境向けのデプロイ/設定です。評価目的は[クイックスタート](/guides/pro/server)を参照してください。
...
<Callout>
  collaboration-server はステートフルです。同一ドキュメントの編集は同一インスタンスにルーティングされます。
</Callout>
...
ほとんどのドキュメントが小規模であれば、協調サービスに 2c4g のスペックを選択してください。そうでない場合は 4c8g を選択することをお勧めします。協調サービスの単一インスタンスのスペックをさらに増やすことはお勧めしません。CPU リソースの無駄が増えます。

- 単一 collaboration-server インスタンス（2c4g/4c8g）は、小規模ドキュメントの同時編集をそれぞれ約 1100/1200 件まで処理でき、編集保存 QPS に換算すると約 360/400 です。
- 単一 universer サービス（2c4g/4c8g）は、小規模ドキュメントの同時編集をそれぞれ約 1200/2200 件まで処理でき、編集保存 QPS に換算すると約 400/730 です。

### 多人協調のリソース見積もり

多人協調時には、編集保存以外に協調メッセージのブロードキャストが CPU リソースを大きく消費します。ブロードキャストは universer サービス内で完結し、node サービスのリソースには影響しません。

2c4g × 2 の構成であれば、1400 人に対して毎秒 10 回の編集操作を配信できます。平均して 1 コアは毎秒最大 3500 回の協調ブロードキャストを処理できます。

ブロードキャスト QPS = 編集保存 QPS × (アクティブユーザー数 / アクティブドキュメント数 − 1)。マイナス 1 は、自分自身へのプッシュが既に編集操作に含まれているためです。

### 容量見積もりツール

以下の計算機を使って、実際の業務パラメータから必要なリソース構成を素早く見積もれます。アクティブドキュメント数、保存間隔、協調比率などを入力すると、編集 QPS、ブロードキャスト QPS、および推奨の Node サービス、Universer、データベース構成が自動的に表示されます。
...
`.env.custom` を作成して `.env` を上書きします。

### K8s

独自の `values.yaml` で Helm chart を上書きします。
...
- 有効化後はイベントの Consumer が必要です（[イベント同期](/guides/pro/event-sync)）。

### 自前インフラ利用

<Callout>
  安定性とデータ保護のため、自前またはクラウド管理コンポーネントを推奨します。
</Callout>
...
- RDS：PostgreSQL 16.1、MySQL 8.0、GaussDB、DamengDB。分散 DB は未対応の場合あり。
- Object Storage：AWS S3 互換。内蔵イメージ：`bitnami/minio:2024.8.3-debian-12-r1`。

#### RDS

**注意**：Temporal は GaussDB/DamengDB 非対応。Temporal は別 PostgreSQL を使用します。
...
<Tabs items={['docker compose', 'K8s']}>
  ...
  <Tab label="K8s">
    K8s は別途インストールが必要です（[運用手冊](/guides/pro/sre-manual)）。
  </Tab>
</Tabs>
...
Helm chart はデフォルトでデモ UI をデプロイします。本番では無効化しドメインを設定します：
