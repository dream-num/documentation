...
[`FWorksheet.addSparkline`](https://docs.univer.ai/reference/facade/worksheet#addsparkline) 方法用于在指定区域添加迷你图，返回一个 `FSparkline` 实例。
...
[`FWorksheet.composeSparkline`](https://docs.univer.ai/reference/facade/worksheet#composesparkline) 和 [`FWorksheet.unComposeSparkline`](https://docs.univer.ai/reference/facade/worksheet#uncomposesparkline) 方法用于组合和取消组合迷你图。
...
[`FWorksheet.getSparklineByCell`](https://docs.univer.ai/reference/facade/worksheet#getsparklinebycell) 用于获取指定单元格内的迷你图实例，返回一个 `FSparkline` 实例。
...
[`FWorksheet.getSparklineGroupByCell`](https://docs.univer.ai/reference/facade/worksheet#getsparklinegroupbycell) 用于获取指定单元格内的迷你图组实例，返回一个 `FSparklineGroup` 实例。
...
[`FSparkline`](https://docs.univer.ai/reference/facade/sparkline) 实例有以下方法用来操作迷你图：
...
[`FSparklineGroup`](/reference/facade/sparkline-group) 实例有以下方法用来操作迷你图组：
...
迷你图颜色等视觉选项通过 `FSparklineGroup.setConfig()` 设置，并不是独立的 `FSparkline.setColor()` 方法。配置作用于整个迷你图组。自定义颜色需要同时设置 `seriesColor` 和 `themeType: SparklineThemeTypeEnum.CUSTOM`。颜色、标记点、坐标轴、最大/最小值、空值、隐藏单元格、线宽等完整选项及示例见 [迷你图组配置](/reference/facade/sparkline-group)。
