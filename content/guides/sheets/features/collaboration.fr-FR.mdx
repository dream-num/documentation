...
<Callout type="warning" title="Attention">
  La fonctionnalité d’édition collaborative nécessite le serveur Univer. Vérifiez que celui-ci est correctement installé
  et configuré. Pour en savoir plus, consultez la page [Passer à Pro](/guides/pro).
</Callout>
...
<Callout>
  Le preset `UniveSheetsCollaborationPreset` du package `@univerjs/preset-sheets-collaboration` dépend des presets
  `UniverSheetsDrawingPreset` et `UniverSheetsAdvancedPreset` lors de l’exécution. Veuillez d’abord installer
  `@univerjs/preset-sheets-drawing` et `@univerjs/preset-sheets-advanced`.
</Callout>
...
- Vous pouvez importer un fichier `.xlsx` sur le serveur Univer à l’aide de la méthode `FUniver.importSheetToUnitIdAsync` fournie par le [plugin d’importation](/guides/sheets/features/import-export).
