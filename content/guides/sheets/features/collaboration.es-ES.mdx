...
<Callout>
El ajuste preestablecido `UniveSheetsCollaborationPreset` de `@univerjs/preset-sheets-collaboration` depende de los ajustes preestablecidos `UniverSheetsDrawingPreset` y `UniverSheetsAdvancedPreset` en tiempo de ejecución. Instale `@univerjs/preset-sheets-drawing` y `@univerjs/preset-sheets-advanced` primero.
</Callout>
...
- Puedes importar un archivo `.xlsx` al servidor Univer usando el método `FUniver.importXLSXToUnitIdAsync` proporcionado por [Importar Complemento](/guides/sheets/features/import-export).
