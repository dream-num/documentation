...
<Callout type="warning" title="Caution">
  La funcionalidad de edición colaborativa requiere soporte del servidor Univer. Asegúrese de haber instalado y
...
<Callout>
  El ajuste preestablecido `UniveSheetsCollaborationPreset` de `@univerjs/preset-sheets-collaboration` depende de los
  ajustes preestablecidos `UniverSheetsDrawingPreset` y `UniverSheetsAdvancedPreset` en tiempo de ejecución. Instale
  `@univerjs/preset-sheets-drawing` y `@univerjs/preset-sheets-advanced` primero.
</Callout>
...
Si tiene una licencia comercial de Univer, consulte [Usar la licencia en el cliente](/guides/pro/license#in-preset-mode) para configurarla.
...
Si tiene una licencia comercial de Univer, consulte [Usar la licencia en el cliente](/guides/pro/license#in-plugin-mode) para configurarla.
...
El complemento de edición colaborativa se basa en el servidor Univer y los datos de los documentos colaborativos se almacenan en el servidor Univer.
...
- Puedes importar un archivo `.xlsx` al servidor Univer usando el método `FUniver.importSheetToUnitIdAsync` proporcionado por [Importar Complemento](/guides/sheets/features/import-export).
...
Si Si desea utilizar esta función, debe modificar la lógica de carga de datos existente de manera adecuada y agregar los parámetros `unit` y `type` a la URL, como se muestra a continuación:
...
### Carga de documentos colaborativos

Si no desea utilizar parámetros de URL para cargar documentos colaborativos, también puede cargar documentos colaborativos a través de la API de Facade.
...
## Edición colaborativa en el entorno Node.js [#node-collaboration-example]
