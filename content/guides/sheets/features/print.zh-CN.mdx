...
该 API 仅在拥有 [许可证](/guides/pro/license) 的情况下可用，未持有许可证时将受到使用限制，保存操作将返回 `false`。
...
该 API 仅在持有 [许可证](/guides/pro/license) 时可用，未持证用户将受到使用限制，保存失败时返回 `false`，成功时返回图片的 base64 字符串。
