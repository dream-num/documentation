...
使用 [`FWorkbook.updatePrintConfig(config: ISheetPrintLayoutConfig)`](https://docs.univer.ai/reference/facade/workbook#updateprintconfig) 方法更新打印布局配置。
...
使用 [`FWorkbook.updatePrintRenderConfig(config: ISheetPrintRenderConfig)`](https://docs.univer.ai/reference/facade/workbook#updateprintrenderconfig) 方法更新打印渲染配置。
...
该 API 仅在拥有 [许可证](/server/license) 的情况下可用，未持有许可证时将受到使用限制，保存操作将返回 `false`。
...
该 API 仅在持有 [许可证](/server/license) 时可用，未持证用户将受到使用限制，保存失败时返回 `false`，成功时返回图片的 base64 字符串。
...
fRange.getScreenshot({ includeHeaders: true }) // 包含行列标题
```
