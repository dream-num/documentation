...
<Callout>
Die Voreinstellung `UniverSheetsAdvancedPreset` von `@univerjs/preset-sheets-advanced` hängt von der ab `UniverSheetsDrawingPreset` zur Laufzeit voreingestellt. Bitte installieren Sie zuerst `@univerjs/preset-sheets-drawing`.
</Callout>
...
Nur verfügbar mit einer [Lizenz](en-US/guides/pro/license), für Benutzer ohne Lizenz gelten Nutzungsbeschränkungen und der Speichervorgang gibt `false` zurück.
...
Nur verfügbar mit einer [Lizenz](en-US/guides/pro/license), für Benutzer ohne Lizenz gelten Nutzungsbeschränkungen und der Speichervorgang wird zurückgegeben `false` bei Fehler oder eine Base64-Zeichenfolge des Bildes bei Erfolg.
