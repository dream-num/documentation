...
<Callout>
  Во время выполнения пресету `UniverSheetsAdvancedPreset` из `@univerjs/preset-sheets-advanced` требуется пресет `UniverSheetsDrawingPreset`. Сначала установите `@univerjs/preset-sheets-drawing`.
</Callout>
...
<Callout type="info" title="Примечание о режиме плагинов">
  Импортировать пакет Facade вручную нужно только в режиме плагинов. В режиме пресетов соответствующий пакет Facade уже включён, поэтому дополнительный импорт не требуется.
</Callout>
...
Эта возможность доступна только при наличии [лицензии](en-US/guides/pro/license). Для пользователей без лицензии действуют ограничения, а операция сохранения возвращает `false`.
...
Эта возможность доступна только при наличии [лицензии](en-US/guides/pro/license). Для пользователей без лицензии действуют ограничения. При ошибке операция сохранения возвращает `false`, а при успехе — строку изображения в формате base64.
