...
<Callout type="warning" title="注意事项">
  导入导出功能需要 Univer 服务端支持，请确保你已经正确安装并配置了 Univer 服务端。具体请参考：[升级到 Pro](/guides/sheets/getting-started/pro)
</Callout>
...
| --- | --- | --- | --- |
| 通过 `unitId` | 需要 | 已启用协同、需要多人共享/继续协作的文档 | `importXLSXToUnitIdAsync` / `exportXLSXByUnitIdAsync` |
| 通过快照（snapshot） | 不需要 | 本地/单人/离线导入导出，或仅做格式转换 | `importXLSXToSnapshotAsync` / `exportXLSXBySnapshotAsync` |
...
<Callout>
  快照导入不依赖协同服务，适合本地/单人/离线场景。需要协同文档请使用 `unitId` 导入。
</Callout>
...
<Callout type="warning" title="迁移说明">
  包根目录导出的 `transformSnapshotJsonToWorkbookData` 和 `transformWorkbookDataToSnapshotJson` 已移除。请导入 `@univerjs-pro/exchange-client/facade` 并使用下方的 Facade API。
</Callout>
