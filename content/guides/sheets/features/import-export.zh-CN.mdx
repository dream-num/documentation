...
<Callout type="warning" title="注意事项">
  导入导出功能需要 Univer 服务端支持，请确保你已经正确安装并配置了 Univer 服务端。具体请参考：[升级到
  Pro](/guides/sheets/getting-started/pro)
</Callout>

导入和导出功能允许用户通过 Univer 服务端将 `.xlsx` 文件导入到 Univer 文档中，或将文档内容导出为 `.xlsx` 文件，便于与其他办公软件或平台进行交互。

<PlaygroundFrame lang="zh-CN" slug="sheets/univer-pro-import-export" clickToShow />

<Callout title="为什么 Univer SDK 通过后端服务实现导入导出？">
  仅通过前端实现的导入导出无论是从性能还是效果上来说都无法满足企业需求，因此我们提供了后端服务来实现导入导出功能。你可以通过一些开源的
  XLSX 解析库将文件解析为符合 [`IWorksheetData`](https://reference.univer.ai/zh-CN/interfaces/IWorkbookData)
  接口的数据结构，然后通过 Facade API 将数据导入到 Univer 中。
</Callout>

通过菜单栏的导入入口成功导入后，页面左下角会弹出提示框，显示导入成功的信息，并提供访问导入文档的链接。若默认的导入导出行为无法满足你的需求，请查看[《自定义导入文档》](/blog/custom-import)以了解如何自定义导入行为。

## 如何选择导入/导出方式

导入导出有两条路径，区别在于是否依赖协同服务：

| 方式                 | 是否需要协同 | 适用场景                                | 对应 API                                                    |
| -------------------- | ------------ | --------------------------------------- | ----------------------------------------------------------- |
| 通过 `unitId`        | 需要         | 已启用协同、需要多人共享/继续协作的文档 | `importSheetToUnitIdAsync` / `exportSheetByUnitIdAsync`     |
| 通过快照（snapshot） | 不需要       | 本地/单人/离线导入导出，或仅做格式转换  | `importSheetToSnapshotAsync` / `exportSheetBySnapshotAsync` |

如果你的项目已接入协同（服务端可持久化文档并通过 `unitId` 打开），优先使用 `unitId` 路径；如果你不需要协同能力或只想在本地处理数据，使用快照路径。
...
interface IUniverSheetsAdvancedPresetConfig {
  ...
  exchangeClientOptions?: {
     ...
     * 注意，此配置需要在协同编辑环境下使用，服务端配置文件需要配置 SSC_SERVER_ENABLED=true。
     */
    enableServerSideComputing?: boolean
    /**
     * 是否在导出时禁用将单元格图片转换为链接。
     * 如果单元格图片源是一个 URL，默认行为是在导出时将该 URL 转换为单元格的值。
     * 由于服务器会下载所有的资源文件，当图片较多时，会消耗大量资源并且耗时较长。
     * @default true
     */
...
import SheetsExchangeClientZhCN from '@univerjs-pro/sheets-exchange-client/locale/zh-CN' // [!code ++]
...
// 或者调用API: univerAPI.loadServerUnit(unitId, 2) 加载文档，具体参数参考 /reference/facade/univer#loadserverunit
```

#### 导入 `.xlsx` 文件并获取 `IWorkbookData`

<Callout>快照导入不依赖协同服务，适合本地/单人/离线场景。需要协同文档请使用 `unitId` 导入。</Callout>
...
Univer 服务端提供了导入导出的 API，但是服务端转换的数据和 Univer 格式不一致。在 Node.js 进程中初始化 Univer 并注册 `@univerjs-pro/exchange-client` 后，请通过 Facade API 转换数据。
