...
Univer 상용 라이선스가 있다면 [클라이언트에서 라이선스 사용하기](/server/license#in-plugin-mode)를 참고하여 설정하세요.
...
Univer 상용 라이선스가 있다면 [클라이언트에서 라이선스 사용하기](/server/license#in-preset-mode)를 참고하여 설정하세요.
...
| 메서드                                                                                                                         | 설명                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| [remove](/reference/facade/generic-pivot-table#remove)                             | 인스턴스에서 피벗 테이블 제거                                              |
...
| [setValueFilter](/reference/facade/pivot-table#setvaluefilter)             | 값 필터 설정                                                                  |
| [reset](/reference/facade/generic-pivot-table#reset)                               | 피벗 테이블 필드 초기화                                                      |
...
## 값 표시 방식 [#show-values-as]

`setShowDataAs()`로 값 필드를 행, 열 또는 전체 합계의 비율이나 기준 항목과의 차이로 표시할 수 있습니다. 표시 계산만 바뀌며 원본 데이터는 대체되지 않습니다. `getShowDataAs()`는 설정을 읽고 `resetShowDataAs()`는 일반 계산으로 복원합니다.
...
예제의 `fPivotTable`은 기존 피벗 테이블입니다. 원본 열이나 행 필드가 아닌 값 필드 ID를 사용하세요. 차이 및 누계 모드에는 해당 기준 필드와 기준 항목 설정도 필요합니다.
