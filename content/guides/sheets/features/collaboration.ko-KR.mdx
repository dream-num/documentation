...
<Callout type="warning" title="주의">
  공동 편집 기능을 사용하려면 Univer 서버가 필요합니다. Univer 서버가 올바르게 설치되고 구성되어 있는지 확인하세요. 자세한 내용은 [Pro로 업그레이드](/guides/pro)를 참고하세요.
</Callout>
...
<Callout>
  `@univerjs/preset-sheets-collaboration`의 `UniveSheetsCollaborationPreset` 프리셋은 런타임에 `UniverSheetsDrawingPreset` 및 `UniverSheetsAdvancedPreset` 프리셋을 필요로 합니다. 먼저 `@univerjs/preset-sheets-drawing`과 `@univerjs/preset-sheets-advanced`를 설치하세요.
</Callout>
...
- [가져오기 플러그인](/guides/sheets/features/import-export)이 제공하는 `FUniver.importXLSXToUnitIdAsync` 메서드를 사용해 `.xlsx` 파일을 Univer 서버로 가져올 수 있습니다.
