...
## Hidden data and independent categories [#source-mapping]

`setPlotVisibleOnly(false)` includes values from hidden rows and columns; use `getPlotVisibleOnly()` to inspect the setting. For sources with separate category columns, `setSeriesToCategory()` maps each value-field index to its category-field indexes. Indexes refer to the physical source vectors, not the filtered visible series. Missing entries inherit the shared categories; `{}` clears the overrides.

For example, with columns A:D, the value series in D can use C as its category while B keeps the shared category in A.
...
For aggregated interactive reports, see [PivotCharts](/guides/sheets/features/pivot-chart).
