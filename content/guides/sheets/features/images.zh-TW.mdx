...
univer.registerPlugin(UniverDrawingPlugin, {
  ...
  // allowImageSize: 20 * 1024 * 1024, // 設定上傳圖片的最大允許大小為 20MB，非必要
})
```
...
// 這是一個 React 元件。對於 Vue3 元件，第三個參數應該是 `{ framework: 'vue3' }`
...
[`FWorksheet.newOverGridImage()`](https://docs.univer.ai/reference/facade/worksheet#newovergridimage) 建立一個浮動圖片構建器，返回一個 `FOverGridImageBuilder` 例項，可以透過鏈式呼叫生成 `ISheetImage` 物件用於插入浮動圖片。

以下是 [`FOverGridImageBuilder`](/reference/facade/over-grid-image-builder) 上的一些成員方法：
...
`FWorksheet.getImages()` 方法可以獲取工作表中的所有浮動圖片，返回一個 [`FOverGridImage[]`](https://docs.univer.ai/reference/facade/chart) 例項陣列。
...
## 工作表背景圖 [#worksheet-background]

工作表另支援一張平鋪背景圖，繪製於儲存格下方，獨立於浮動與儲存格圖片，儲存在 `IWorksheetData.backgroundImage`。繪圖預設已包含此功能；外掛模式需要 `@univerjs/sheets-drawing` 與其 Facade，並由繪圖 UI 提供渲染及上傳入口。
...
`setBackgroundImage()` 預設使用 `ImageSourceType.URL` 並取代原背景；`deleteBackgroundImage()` 移除背景。兩者透過命令支援復原與重做。UI 上傳使用設定的圖片 I/O 服務。背景僅供螢幕顯示，不參與列印。
