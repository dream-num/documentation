...
import SheetsHistoryUIKoKR from '@univerjs-pro/sheets-history-ui/locale/ko-KR' // [!code ++]
...
`UniverSheetsHistoryUIPlugin`의 구성 옵션은 다음과 같습니다.
...
편집 기록을 활성화하면 `UniverSheetsHistoryUIPlugin`이 새 Univer 인스턴스를 불러와 지정된 DOM 노드에 마운트합니다. 따라서 기록 패널이 기존 Univer 패널을 덮도록 플러그인을 등록할 때 적절한 DOM 노드 ID(예: 기존 Univer 인스턴스의 컨테이너)를 제공해야 합니다. 노드 ID를 지정하지 않으면 기본값으로 `univer-container`가 사용됩니다.

기록 목록의 서버 주소를 변경하려면 `historyServerUrl` 구성 옵션을 설정하세요. 기본값은 `/universer-api/history`입니다.
