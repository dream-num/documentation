...
<Callout type="warning" title="Внимание">
  Для совместного редактирования требуется сервер Univer. Убедитесь, что сервер Univer правильно установлен и настроен.
  Подробнее см. в разделе [Переход на Pro](/guides/pro).
</Callout>

Функция совместного редактирования позволяет нескольким пользователям одновременно работать с одним документом, синхронизируя изменения в реальном времени. Она подходит для командной работы и других сценариев с несколькими редакторами.

<PlaygroundFrame lang="ru-RU" slug="sheets/univer-pro-collaboration" clickToShow />
...
<Callout>
  Предустановка `UniveSheetsCollaborationPreset` из пакета `@univerjs/preset-sheets-collaboration` во время выполнения
  зависит от предустановок `UniverSheetsDrawingPreset` и `UniverSheetsAdvancedPreset`. Сначала установите
  `@univerjs/preset-sheets-drawing` и `@univerjs/preset-sheets-advanced`.
</Callout>
...
Плагин совместного редактирования использует сервер Univer, на котором хранятся данные совместных документов.

### `unitId`

Каждый совместный документ на сервере Univer имеет уникальный `unitId`. Клиент совместной работы Univer (плагин совместного редактирования) использует `unitId`, чтобы получить с сервера Univer данные соответствующего документа для совместного редактирования.

### `type`

`type` — это тип совместного документа, определяющий его исходную структуру данных.
...
- Импортировать файл `.xlsx` на сервер Univer с помощью метода `FUniver.importSheetToUnitIdAsync`, предоставляемого [плагином импорта](/guides/sheets/features/import-export).

## Загрузка совместных документов через параметры URL

Плагин `@univerjs-pro/collaboration-client` умеет автоматически загружать соответствующие данные на основе параметров URL `unit` и `type`. В некоторых сценариях это упрощает логику загрузки данных.

Чтобы использовать эту возможность, соответствующим образом измените существующую логику загрузки данных и добавьте в URL параметры `unit` и `type`, как показано ниже:
...
<Callout type="info" title="Примечание о режиме плагинов">
  Импортировать пакет Facade вручную нужно только в режиме плагинов. В режиме предустановки соответствующий пакет Facade
...
Если вы не хотите загружать совместные документы с помощью параметров URL, их также можно загрузить через Facade API.
...
## Совместное редактирование в среде Node.js [#node-collaboration-example]
