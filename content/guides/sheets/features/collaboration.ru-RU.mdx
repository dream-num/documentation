...
<Callout type="warning" title="Внимание">
  Для совместного редактирования требуется сервер Univer. Убедитесь, что сервер Univer правильно установлен и настроен. Подробнее см. в разделе [Переход на Pro](/guides/pro).
</Callout>
...
<Callout>
  Предустановка `UniveSheetsCollaborationPreset` из пакета `@univerjs/preset-sheets-collaboration` во время выполнения зависит от предустановок `UniverSheetsDrawingPreset` и `UniverSheetsAdvancedPreset`. Сначала установите `@univerjs/preset-sheets-drawing` и `@univerjs/preset-sheets-advanced`.
</Callout>
...
- Импортировать файл `.xlsx` на сервер Univer с помощью метода `FUniver.importXLSXToUnitIdAsync`, предоставляемого [плагином импорта](/guides/sheets/features/import-export).
