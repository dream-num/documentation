...
<MetaData
  ...
  meta={{
    ...
    preset: [{
      ...
      locale: '@univerjs/preset-sheets-thread-comment/locales/ja-JP',
    ...
    plugins: [{
      ...
      locale: '@univerjs/sheets-thread-comment-ui/locale/ja-JP',
...
import SheetsThreadCommentUIJaJP from '@univerjs/sheets-thread-comment-ui/locale/ja-JP' // [!code ++]
...
const univer = new Univer({
  ...
  locales: {
    [LocaleType.JA_JP]: mergeLocales(
      ...
      SheetsThreadCommentUIJaJP, // [!code ++]
    ),
  },
})
...
import UniverPresetSheetsThreadCommentJaJP from '@univerjs/preset-sheets-thread-comment/locales/ja-JP' // [!code ++]
...
const { univerAPI } = createUniver({
  ...
  locales: {
    [LocaleType.JA_JP]: mergeLocales(
      ...
      UniverPresetSheetsThreadCommentJaJP, // [!code ++]
    ),
  },
  presets: [
...
Facade API の完全な型定義は、[FacadeAPI](/reference/facade/thread-comment) を参照してください。
...
以下は [`FTheadCommentBuilder`](/reference/facade/thead-comment-builder) の主なメソッドです:
...
- `FfWorksheet.getComments()`: ワークシート内のすべてのコメントを取得します
- `FRange.getComment()`: 範囲の左上セルのコメントを取得します
- `FRange.getComments()`: 範囲内のすべてのコメントを取得します

以下は [`FThreadComment`](https://docs.univer.ai/reference/facade/thread-comment) の主なメソッドです:
...
イベント型の完全な定義は [Events](/reference/facade/events) を参照してください。

コメントモジュールは、コメントの追加・更新・削除・解決状態の変更を監視するためのイベント群を提供します。すべてのイベントは `univerAPI.addEvent()` で購読できます。

#### コメント追加イベント

`univerAPI.Event.CommentAdded`: コメントが追加された後にトリガーします
...
`univerAPI.Event.BeforeCommentAdd`: コメントが追加される前にトリガーします
...
`univerAPI.Event.CommentUpdated`: コメントが更新された後にトリガーします
...
`univerAPI.Event.BeforeCommentUpdate`: コメントが更新される前にトリガーします
...
`univerAPI.Event.CommentDeleted`: コメントが削除された後にトリガーします
...
`univerAPI.Event.BeforeCommentDelete`: コメントが削除される前にトリガーします
...
`univerAPI.Event.CommentResolved`: コメントの解決ステータスが変更された後にトリガーします
...
`univerAPI.Event.BeforeCommentResolve`: コメントの解決ステータスが変更される前にトリガーします
