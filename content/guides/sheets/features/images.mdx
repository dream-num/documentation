...
{/* 如使用[协同编辑](/en-US/guides/sheets/features/collaboration)功能，请确保在 `UniverSheetsDrawingPreset` 中传入 `collaboration: true` 选项。 */}
If you are using the [collaboration](/en-US/guides/sheets/features/collaboration) feature, make sure to pass the `collaboration: true` option in `UniverSheetsDrawingPreset`:
