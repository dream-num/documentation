...
<Callout type="warning" title="Caution">
  The collaborative editing functionality requires support from the Univer server. Please ensure that you have correctly
  installed and configured the Univer server. For details, please refer to: [Upgrading to Pro](/guides/pro)
</Callout>
...
<Callout>
  The `UniveSheetsCollaborationPreset` preset from `@univerjs/preset-sheets-collaboration` depends on the
  `UniverSheetsDrawingPreset` and `UniverSheetsAdvancedPreset` presets at runtime. Please install
  `@univerjs/preset-sheets-drawing` and `@univerjs/preset-sheets-advanced` first.
</Callout>
...
- You can import a `.xlsx` file into the Univer server using the `FUniver.importSheetToUnitIdAsync` method provided by the [Import Plugin](/guides/sheets/features/import-export).
