...
<MetaData
  ...
  meta={{
    ...
    plugins: [{
      ...
      locale: '@univerjs/data-validation/locale/zh-CN',
    }, {
      client: '@univerjs/sheets-data-validation',
      facade: '@univerjs/sheets-data-validation/facade',
      locale: '@univerjs/sheets-data-validation/locale/zh-CN',
    }, {
...
完整的配置项参考 [`IUniverSheetsDataValidationUIConfig`](https://github.com/dream-num/univer/blob/dev/packages/sheets-data-validation-ui/src/config/config.ts)。
...
[`univerAPI.newDataValidation()`](https://docs.univer.ai/reference/facade/univer#newdatavalidation) 可以创建一个新的数据验证构建器，返回一个 `FDataValidationBuilder` 实例，可以通过链式调用生成数据验证规则。

以下是 [`FDataValidationBuilder`](/reference/facade/data-validation-builder) 上的一些成员方法：
...
| requireValueInList | 设置数据验证规则，要求用户从特定值列表中输入一个值。支持单选/多选，支持显示/隐藏下拉框 |
| requireValueInRange | 设置数据验证规则，要求用户输入特定范围内的值。支持单选/多选，支持显示/隐藏下拉框 |
...
以下是 [`FDataValidation`](https://docs.univer.ai/reference/facade/data-validation) 上的一些成员方法：

| 方法 | 描述 |
| ---- | ---- |
| copy | 使用当前规则对象创建一个新的 FDataValidationBuilder 实例 |
...
校验状态是 `DataValidationStatus` 的枚举值，包括：
- `valid`：单元格值通过验证
- `invalid`：单元格值未通过验证
- `validating`：单元格正在验证中
...
`errorStyle` 选项控制输入无效数据时的行为。它是 `DataValidationErrorStyle` 的枚举值，包括：
- `STOP`：阻止用户输入无效数据
- `WARNING`：允许用户输入无效数据，但显示警告
- `INFO`：允许用户输入无效数据，但显示信息提示
