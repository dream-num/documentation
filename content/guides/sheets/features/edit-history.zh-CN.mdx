...
<Callout type="warning" title="0.25.x 用户">
  如果使用 **0.25.x**，请阅读 [0.25.x 旧版 Pro 文档](https://dream-num.github.io/documentation/v0.25/zh-CN/guides/pro/)。
</Callout>

## 先准备协同和历史服务

先按[协同集成](/guides/server-integration/collaboration)连接编辑器，并完成[许可证配置](/guides/server-integration/license)。确认两端能编辑同一份 Sheets 文档，刷新后内容仍然保留。

然后按 [Office 协同扩展模块](https://office.univer.ai/zh-CN/collaboration/extensions)和 [History 示例](https://office.univer.ai/zh-CN/collaboration/examples#history)接入历史服务、存储和权限。核心协同不会自动启用历史记录；后端还需提供历史列表、版本内容和恢复操作所需的能力。
