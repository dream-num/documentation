...
Using [`FWorkbook.updatePrintConfig(config: ISheetPrintLayoutConfig)`](https://docs.univer.ai/reference/facade/workbook#updateprintconfig) method to update the print layout configuration.
...
Using [`FWorkbook.updatePrintRenderConfig(config: ISheetPrintRenderConfig)`](https://docs.univer.ai/reference/facade/workbook#updateprintrenderconfig) method to update the print render configuration.
...
Only available with a [license](/server/license), users without a license will face usage restrictions, and the save operation will return `false`.
...
Only available with a [license](/server/license), users without a license will face usage restrictions, and the save operation will return `false` on failure, or a base64 string of the image on success.
