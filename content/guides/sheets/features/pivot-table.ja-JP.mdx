...
Univer Sheets では、ピボットテーブルのデータソースはワークシートまたはデータ範囲のいずれかを選択できます。必要に応じて異なるデータソースを選択し、データをより適切に分析できます。
データソースの最初の行をデフォルトの列ラベルとして、残りをデータソースとして扱います。したがって、データソースはできるだけ整理された状態であることが求められ、空の行や列はできるだけ避けるようにしてください。
同じ列のデータについては、ピボットテーブルの機能をより適切にサポートするために、データ型を自動的に認識します。列内のデータ型が不一致の場合、その列のデータ型は文字列にデフォルト設定されます。

Univer Sheets では、ピボットテーブルはデータの変更に自動的に対応します。データソースが変更されると、ピボットテーブルは自動的に更新され、データをより適切に分析できるようになります。
...
Univer の商用ライセンスをお持ちの場合は、設定方法について [クライアントでのライセンス](/server/license#in-plugin-mode) を参照してください。
...
interface IUniverSheetsPivotConfig {
  /**
   * 単一のピボットテーブル フィールドで表示できる最大項目数を定義します。
   * @default 1000
   */
...
Univer の商用ライセンスをお持ちの場合は、設定方法について [クライアントでのライセンス](/server/license#in-preset-mode) を参照してください。
...
[`FWorkbook.addPivotTable`](https://docs.univer.ai/reference/facade/workbook#addpivottable) メソッドはピボットテーブルを追加するために使用され、成功すると `FPivotTable` インスタンスを返します。
...
[`FWorkbook.getPivotTableByCell`](https://docs.univer.ai/reference/facade/workbook#getpivottablebycell) メソッドは、アンカーセルからピボットテーブルのインスタンスを取得します。対象セルがピボットテーブル内にある場合、`FPivotTable` インスタンスを返します。
...
[`FPivotTable`](https://docs.univer.ai/reference/facade/pivot-table) インスタンスには、ピボットテーブルを操作するためのいくつかのメソッドがあります。

| メソッド                                                                                                                       | 説明                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| [remove](/reference/facade/generic-pivot-table#remove)                             | ピボットテーブルをインスタンスから削除します                                               |
| [getConfig](https://docs.univer.ai/reference/facade/pivot-table#getconfig)                       | ピボットテーブルの設定（ソース範囲情報、アンカーセル情報、設定）を返します                 |
| [addField](https://docs.univer.ai/reference/facade/pivot-table#addfield)                         | ピボットテーブルにフィールドを追加します                                                   |
| [removeField](https://docs.univer.ai/reference/facade/pivot-table#removefield)                   | ピボットテーブルからフィールドを削除します                                                 |
| [updateFieldPosition](https://docs.univer.ai/reference/facade/pivot-table#updatefieldposition)   | ピボットテーブル内のフィールド位置を更新します                                             |
| [updateValuePosition](https://docs.univer.ai/reference/facade/pivot-table#updatevalueposition)   | ∑ 値の位置を制御します                                                                     |
| [setSubtotalType](https://docs.univer.ai/reference/facade/pivot-table#setsubtotaltype)           | フィールドの小計タイプを設定します                                                         |
| [setLabelSort](https://docs.univer.ai/reference/facade/pivot-table#setlabelsort)                 | フィールドのラベル ソートを設定します                                                       |
| [setLabelManualFilter](https://docs.univer.ai/reference/facade/pivot-table#setlabelmanualfilter) | フィールドのラベル フィルターを設定します                                                   |
| [renameField](https://docs.univer.ai/reference/facade/pivot-table#renamefield)                   | フィールド名を変更します                                                                   |
| [setValueFilter](/reference/facade/pivot-table#setvaluefilter)             | 値フィルターを設定します                                                                    |
| [reset](/reference/facade/generic-pivot-table#reset)                               | ピボットテーブルのフィールドをリセットします                                               |
| [setFieldsConfig](https://docs.univer.ai/reference/facade/pivot-table#setfieldsconfig)           | ピボットテーブルのフィールド構成を設定します                                               |
| [getValueFilters](https://docs.univer.ai/reference/facade/pivot-table#getvaluefilters)           | ピボットテーブルのすべての値フィルターを取得します                                         |
...
## 値の表示方法 [#show-values-as]

`setShowDataAs()` で値フィールドを行・列・総計に対する割合や基準項目との差として表示できます。表示上の計算を変更し、元データは置き換えません。`getShowDataAs()` で設定を取得し、`resetShowDataAs()` で通常の計算に戻します。
...
例の `fPivotTable` は既存のピボットテーブルです。元データの列や行フィールドではなく、値フィールドの ID を指定します。差分や累計のモードでは、対応する基準フィールドと基準項目の設定も必要です。
