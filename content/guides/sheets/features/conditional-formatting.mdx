...
Conditional formatting allows users to automatically change the format of cells based on specific conditions, highlighting important data or trends. It supports various conditions and formatting options, helping users understand data more intuitively.
...
[`FWorksheet.newConditionalFormattingRule()`](https://docs.univer.ai/reference/facade/worksheet#newconditionalformattingrule) creates a conditional formatting builder, returning an instance of `FConditionalFormattingBuilder`, which can be used to generate conditional formatting rules through method chaining.

Here are some member methods of [`FConditionalFormattingBuilder`](/reference/facade/conditional-formatting-builder):

| Method | Description |
| ---- | ---- |
| build | Builds the conditional formatting rule |
| copy | Deep clone a current builder |
| createCfId | Create a conditional format ID |
| getRanges | Gets the scope of the current conditional format |
