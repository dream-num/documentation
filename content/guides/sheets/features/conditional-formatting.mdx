...
[`FWorksheet.newConditionalFormattingRule()`](https://reference.univer.ai/en-US/classes/FWorksheet#newconditionalformattingrule) 创建一个条件格式构建器，返回一个 `FConditionalFormattingBuilder` 实例，可以通过链式调用生成条件格式规则。

以下是 [`FConditionalFormattingBuilder`](https://reference.univer.ai/en-US/classes/FConditionalFormattingBuilder) 上的一些成员方法：
