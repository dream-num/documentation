...
| 方法                                                                                                                           | 描述                                                                    |
| ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| [remove](/reference/facade/generic-pivot-table#remove)                             | 刪除透視表例項                                                          |
...
| [setValueFilter](/reference/facade/pivot-table#setvaluefilter)             | 設定值維度                                                              |
| [reset](/reference/facade/generic-pivot-table#reset)                               | 重置透視表的維度配置                                                    |
...
## 值顯示方式 [#show-values-as]

使用 `setShowDataAs()` 將值欄位顯示為列、欄或總計的百分比，也可顯示相對於基準項目的差異。這只改變顯示計算，不會取代來源資料。`getShowDataAs()` 讀取設定，`resetShowDataAs()` 還原一般計算。
...
範例中的 `fPivotTable` 是既有樞紐分析表。請使用值欄位 ID，而非來源欄或列欄位 ID。差異與累計等模式還需要設定對應的基準欄位與基準項目。
