...
[`FWorksheet.newOverGridImage()`](https://docs.univer.ai/reference/facade/worksheet#newovergridimage) 创建一个浮动图片构建器，返回一个 `FOverGridImageBuilder` 实例，可以通过链式调用生成 `ISheetImage` 对象用于插入浮动图片。

以下是 [`FOverGridImageBuilder`](/reference/facade/over-grid-image-builder) 上的一些成员方法：
...
| setCropTop | 设置 image 的裁剪区域，定义顶部边缘，从而显示你想要的图像的特定部分 |
| setCropLeft | 设置 image 的裁剪区域，定义左边缘，从而显示你想要的图像的特定部分 |
| setCropBottom | 设置 image 的裁剪区域，定义底部边缘，从而显示你想要的图像的特定部分 |
| setCropRight | 设置 image 的裁剪区域，定义右边缘，从而显示你想要的图像的特定部分 |
...
`FWorksheet.getImages()` 方法可以获取工作表中的所有浮动图片，返回一个 [`FOverGridImage[]`](https://docs.univer.ai/reference/facade/chart) 实例数组。
...
## 工作表背景图 [#worksheet-background]

工作表还支持一张平铺背景图，绘制在单元格下方，独立于浮动图片和单元格图片，保存在 `IWorksheetData.backgroundImage`。绘图预设已包含此功能；插件模式需要 `@univerjs/sheets-drawing` 及其 Facade，并通过绘图 UI 提供渲染和上传入口。
...
`setBackgroundImage()` 默认使用 `ImageSourceType.URL`，会替换原背景；`deleteBackgroundImage()` 删除背景。两者通过命令支持撤销/重做。UI 上传经配置的图片 I/O 服务处理。背景仅用于屏幕展示，不参与打印。
