...
| --- | --- | --- | --- |
| By `unitId` | Yes | Collaborative documents that are persisted on the server | `importXLSXToUnitIdAsync` / `exportXLSXByUnitIdAsync` |
| By snapshot | No | Local or offline workflows, or format conversion only | `importXLSXToSnapshotAsync` / `exportXLSXBySnapshotAsync` |
...
<Callout type="warning" title="Migration note">
  The package-root helpers `transformSnapshotJsonToWorkbookData` and `transformWorkbookDataToSnapshotJson` remain available for compatibility, but are deprecated and planned for removal in v1.1.0. Use the Facade APIs below in new code.
</Callout>
