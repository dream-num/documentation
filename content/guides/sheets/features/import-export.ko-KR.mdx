...
<Callout type="warning" title="Caution">
  가져오기 및 내보내기 기능을 사용하려면 Univer 서버가 필요합니다. Univer 서버가 올바르게 설치되고 구성되었는지
  확인하세요. 자세한 내용은 [Pro로 업그레이드](/guides/pro)를 참고하세요.
</Callout>
...
<Callout title="Why does Univer SDK implement import and export through backend services?">
  ...
  백엔드 서비스를 통해 가져오기 및 내보내기 기능을 제공합니다. 오픈 소스 DOCX 파싱 라이브러리로 파일을 `IDocumentData`
  인터페이스에 맞는 데이터 구조로 파싱한 다음 Facade API를 사용하여 Univer로 데이터를 가져올 수 있습니다.
</Callout>

메뉴 표시줄의 가져오기 항목을 통해 가져오기가 완료되면 페이지 왼쪽 아래에 성공 알림과 가져온 문서로 이동할 수 있는 링크가 표시됩니다. 기본 가져오기 및 내보내기 동작이 요구 사항에 맞지 않는다면 [문서 가져오기 사용자 지정](/blog/custom-import)에서 가져오기 동작을 사용자 지정하는 방법을 확인하세요.
...
| ------------- | ------------------- | ----------------------------------------------- | ----------------------------------------------------------- |
| `unitId` 사용 | 예                  | 서버에 영구 저장되는 공동 작업 문서             | `importSheetToUnitIdAsync` / `exportSheetByUnitIdAsync`     |
| 스냅샷 사용   | 아니요              | 로컬 또는 오프라인 워크플로 또는 단순 형식 변환 | `importSheetToSnapshotAsync` / `exportSheetBySnapshotAsync` |
