...
<Callout type="warning" title="Migration note">
  패키지 루트의 `transformSnapshotJsonToWorkbookData` 및 `transformWorkbookDataToSnapshotJson` 헬퍼는 호환성을 위해 계속 제공되지만 더 이상 권장되지 않으며 v1.1.0에서 제거될 예정입니다. 새 코드에서는 아래의 Facade API를 사용하세요.
</Callout>
