...
<MetaData
  ...
  meta={{
    preset: [{
      ...
      locale: '@univerjs/preset-sheets-conditional-formatting/locales/ja-JP',
    ...
    plugins: [{
      ...
      locale: '@univerjs/sheets-conditional-formatting-ui/locale/ja-JP',
...
import SheetsConditionalFormattingUIJaJP from '@univerjs/sheets-conditional-formatting-ui/locale/ja-JP' // [!code ++]
...
const univer = new Univer({
  ...
  locales: {
    [LocaleType.JA_JP]: mergeLocales(
      SheetsConditionalFormattingUIJaJP, // [!code ++]
    ),
  },
})
...
import UniverPresetSheetsConditionalFormattingJaJP from '@univerjs/preset-sheets-conditional-formatting/locales/ja-JP' // [!code ++]
...
const { univerAPI } = createUniver({
  ...
  locales: {
    [LocaleType.JA_JP]: mergeLocales(
      ...
      UniverPresetSheetsConditionalFormattingJaJP, // [!code ++]
    ),
  },
  presets: [
...
[`FWorksheet.newConditionalFormattingRule()`](https://docs.univer.ai/reference/facade/worksheet#newconditionalformattingrule) は条件付き書式ビルダーを生成し、`FConditionalFormattingBuilder` インスタンスを返します。返されたビルダーをメソッドチェーンで操作して条件付き書式ルールを作成できます。

以下は [`FConditionalFormattingBuilder`](/reference/facade/conditional-formatting-builder) のメソッドの一部です:

| Method | Description |
| ---- | ---- |
| build | 条件付き書式ルールを構築します |
| copy | 現在のビルダーを深くクローンします |
| createCfId | 条件付き書式 ID を作成します |
| getRanges | 現在の条件付き書式の範囲を取得します |
