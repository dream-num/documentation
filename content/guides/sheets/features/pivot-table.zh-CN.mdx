...
- 汇总数据：数据透视表可以帮助你快速汇总数据，以便更好地了解数据的整体情况。
- 分析数据：数据透视表可以帮助你分析数据，找出数据中的模式和关系。
- 探索数据：数据透视表可以帮助你探索数据，发现数据中的规律和趋势。
- 可视化数据：数据透视表可以帮助你将数据可视化，以便更好地理解数据。
...
在数据透视表中，你可以根据需要对行标签、列标签和值进行调整，以便更好地分析数据。

![Missing Locale](./pivot-table/pivot.png)

### 数据源

在 Univer Sheets 中，数据透视表的数据源可以是一个工作表（Worksheet），也可以是一个数据区域。你可以根据需要选择不同的数据源，以便更好地分析数据。
...
import SheetsPivotTableUIZhCN from '@univerjs-pro/sheets-pivot-ui/locale/zh-CN' // [!code ++]
import SheetsPivotTableZhCN from '@univerjs-pro/sheets-pivot/locale/zh-CN' // [!code ++]
...
const univer = new Univer({
  ...
  locales: {
    [LocaleType.ZH_CN]: mergeLocales(
      SheetsPivotTableZhCN, // [!code ++]
      SheetsPivotTableUIZhCN, // [!code ++]
    ),
  },
})
```
...
先配置[移动端编辑器](/guides/sheets/getting-started/installation#mobile-support)。在本页插件模式的初始化代码中，将下列桌面端注册替换为同一个包导出的移动端插件，并在**打开文档前**完成。保留模型插件、样式、语言包和 Facade 导入，不要同时注册两个版本。
...
如果你拥有 Univer 的商业许可证，请参考[在客户端使用许可证](/server/license#in-preset-mode)进行配置。
...
[`FWorkbook.addPivotTable`](https://docs.univer.ai/reference/facade/workbook#addpivottable) 方法用于添加透视表，成功会返回 `FPivotTable` 实例。
...
[`FWorkbook.getPivotTableByCell`](https://docs.univer.ai/reference/facade/workbook#getpivottablebycell) 方法通过锚定单元格获取透视表实例。如果目标单元格在透视表中，它将返回 `FPivotTable` 实例。
...
[`FPivotTable`](https://docs.univer.ai/reference/facade/pivot-table) 实例中有一些方法，可以用来操作透视表。

| 方法                                                                                                                           | 描述                                                                    |
| ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| [remove](/reference/facade/generic-pivot-table#remove)                             | 删除透视表实例                                                          |
| [getConfig](https://docs.univer.ai/reference/facade/pivot-table#getconfig)                       | 获取当前透视表的配置信息，其中包括数据源范围，锚定单元格， 维度配置信息      |
| [addField](https://docs.univer.ai/reference/facade/pivot-table#addfield)                         | 添加一个维度                                                            |
| [removeField](https://docs.univer.ai/reference/facade/pivot-table#removefield)                   | 删除一个维度                                                            |
| [updateFieldPosition](https://docs.univer.ai/reference/facade/pivot-table#updatefieldposition)   | 更新维度的所在区域以及顺序                                               |
| [updateValuePosition](https://docs.univer.ai/reference/facade/pivot-table#updatevalueposition)   | 更新 ∑Value 的位置                                                      |
| [setSubtotalType](https://docs.univer.ai/reference/facade/pivot-table#setsubtotaltype)           | 设置汇总方式                                                            |
| [setLabelSort](https://docs.univer.ai/reference/facade/pivot-table#setlabelsort)                 | 设置标签排序                                                            |
| [setLabelManualFilter](https://docs.univer.ai/reference/facade/pivot-table#setlabelmanualfilter) | 设置标签筛选                                                            |
| [renameField](https://docs.univer.ai/reference/facade/pivot-table#renamefield)                   | 重命名维度                                                              |
| [setValueFilter](/reference/facade/pivot-table#setvaluefilter)             | 设置值维度                                                              |
| [reset](/reference/facade/generic-pivot-table#reset)                               | 重置透视表的维度配置                                                    |
| [setFieldsConfig](https://docs.univer.ai/reference/facade/pivot-table#setfieldsconfig)           | 设置透视表的维度配置                                                    |
| [getValueFilters](https://docs.univer.ai/reference/facade/pivot-table#getvaluefilters)           | 获取透视表的值维度配置                                                  |
...
## 值显示方式 [#show-values-as]

使用 `setShowDataAs()` 将值字段显示为行、列或总计的百分比，也可以显示相对于基准项的差异。这只改变展示计算，不会替换源数据。`getShowDataAs()` 读取设置，`resetShowDataAs()` 恢复常规计算。
...
示例中的 `fPivotTable` 是已有透视表。这里需要值字段 ID，不能使用源列或行字段 ID。差异、累计等模式还需要设置对应的基准字段与基准项。
