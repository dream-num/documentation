...
import SheetsHistoryUIEnUS from '@univerjs-pro/sheets-history-ui/locale/en-US' // [!code ++]
...
When enabling edit history, the `UniverSheetsHistoryUIPlugin` will load a new Univer instance and mount it on the specified DOM node. Therefore, when registering, you need to provide a suitable DOM node id (such as the original Univer instance's container) to achieve the purpose of the history panel covering the original Univer panel. If the node id is not specified, it defaults to `univer-container`.
