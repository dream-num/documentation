...
[`FWorksheet.newConditionalFormattingRule()`](https://docs.univer.ai/reference/facade/worksheet#newconditionalformattingrule) 创建一个条件格式构建器，返回一个 `FConditionalFormattingBuilder` 实例，可以通过链式调用生成条件格式规则。

以下是 [`FConditionalFormattingBuilder`](/reference/facade/conditional-formatting-builder) 上的一些成员方法：

| 方法 | 描述 |
| ---- | ---- |
| build | 构建条件格式规则 |
| copy | 深拷贝当前构建器 |
| createCfId | 创建条件格式 ID |
| getRanges | 获取当前条件格式的范围 |
