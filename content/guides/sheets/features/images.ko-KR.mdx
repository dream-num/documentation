...
[`FOverGridImageBuilder`](/reference/facade/over-grid-image-builder)의 주요 멤버 메서드는 다음과 같습니다.
...
## 워크시트 배경 이미지 [#worksheet-background]

워크시트는 셀 뒤에 이미지 한 장을 반복해서 표시할 수 있습니다. 떠 있는 이미지 및 셀 이미지와 별도로 `IWorksheetData.backgroundImage`에 저장됩니다. Drawing 프리셋에 포함되며 플러그인 모드에는 `@univerjs/sheets-drawing`, Facade 및 렌더링·업로드용 Drawing UI가 필요합니다.
...
`setBackgroundImage()`는 기본적으로 `ImageSourceType.URL`을 사용하여 배경을 교체하고 `deleteBackgroundImage()`는 배경을 제거합니다. 둘 다 실행 취소/다시 실행을 지원합니다. UI 업로드는 설정된 이미지 I/O 서비스를 사용합니다. 배경은 화면 표시용이며 인쇄에서 제외됩니다.
