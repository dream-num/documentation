    ...
    plugins: [
      {
        ...
        locale: '@univerjs-pro/sheets-print/locale/ja-JP',
...
import SheetsPrintPluginJaJP from '@univerjs-pro/sheets-print/locale/ja-JP' // [!code ++]
...
### プラグインと設定 [#plugins-and-configuration]
...
[`FWorkbook.updatePrintConfig(config: ISheetPrintLayoutConfig)`](https://docs.univer.ai/reference/facade/workbook#updateprintconfig) メソッドを使用して、印刷レイアウト設定を更新します。
...
[`FWorkbook.updatePrintRenderConfig(config: ISheetPrintRenderConfig)`](https://docs.univer.ai/reference/facade/workbook#updateprintrenderconfig) メソッドを使用して、印刷レンダー設定を更新します。
...
[ライセンス](/server/license) をお持ちの方のみ利用可能です。ライセンスがない場合は使用制限があり、保存処理は `false` を返します。
...
[ライセンス](/server/license) をお持ちの方のみ利用可能です。ライセンスがない場合は使用制限があり、失敗時は `false` を返し、成功時は画像の base64 文字列を返します。
...
fRange.getScreenshot({ includeHeaders: true }) // 行列ヘッダーを含む
```
