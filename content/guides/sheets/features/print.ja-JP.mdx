...
[ライセンス](/guides/license) をお持ちの方のみ利用可能です。ライセンスがない場合は使用制限があり、保存処理は `false` を返します。
...
[ライセンス](/guides/license) をお持ちの方のみ利用可能です。ライセンスがない場合は使用制限があり、失敗時は `false` を返し、成功時は画像の base64 文字列を返します。
