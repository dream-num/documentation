...
[`univerAPI.newDataValidation()`](https://docs.univer.ai/reference/facade/univer#newdatavalidation) can create a new data validation builder, which returns an `FDataValidationBuilder` instance that can generate data validation rules through chained calls.

Here are some member methods on [`FDataValidationBuilder`](/reference/facade/data-validation-builder):
...
Here are some member methods on [`FDataValidation`](https://docs.univer.ai/reference/facade/data-validation):
