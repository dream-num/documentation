...
[`univerAPI.newDataValidation()`](https://reference.univer.ai/en-US/classes/FUniver#newdatavalidation) 可以创建一个新的数据验证构建器，返回一个 `FDataValidationBuilder` 实例，可以通过链式调用生成数据验证规则。

以下是 [`FDataValidationBuilder`](https://reference.univer.ai/en-US/classes/FDataValidationBuilder) 上的一些成员方法：
...
以下是 [`FDataValidation`](https://reference.univer.ai/en-US/classes/FDataValidation) 上的一些成员方法：
