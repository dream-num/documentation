...
[`FWorksheet.addSparkline`](https://docs.univer.ai/reference/facade/worksheet#addsparkline) 方法用於在指定區域新增迷你圖，返回一個 `FSparkline` 例項。
...
[`FWorksheet.composeSparkline`](https://docs.univer.ai/reference/facade/worksheet#composesparkline) 和 [`FWorksheet.unComposeSparkline`](https://docs.univer.ai/reference/facade/worksheet#uncomposesparkline) 方法用於組合和取消組合迷你圖。
...
[`FWorksheet.getSparklineByCell`](https://docs.univer.ai/reference/facade/worksheet#getsparklinebycell) 用於獲取指定單元格內的迷你圖例項，返回一個 `FSparkline` 例項。
...
[`FWorksheet.getSparklineGroupByCell`](https://docs.univer.ai/reference/facade/worksheet#getsparklinegroupbycell) 用於獲取指定單元格內的迷你圖組例項，返回一個 `FSparklineGroup` 例項。
...
[`FSparkline`](https://docs.univer.ai/reference/facade/sparkline) 例項有以下方法用來操作迷你圖：
...
[`FSparklineGroup`](/reference/facade/sparkline-group) 例項有以下方法用來操作迷你圖組：
...
迷你圖的顏色等視覺選項透過 `FSparklineGroup.setConfig()` 設定，並非獨立的 `FSparkline.setColor()` 方法。設定會套用至整個迷你圖群組。自訂顏色時，需同時設定 `seriesColor` 與 `themeType: SparklineThemeTypeEnum.CUSTOM`。顏色、標記點、座標軸、最大／最小值、空值、隱藏儲存格、線寬等完整選項及範例，請參閱[迷你圖群組設定](/reference/facade/sparkline-group)。
