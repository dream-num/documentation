...
<Callout type="warning" title="注意事項">
  協同編輯功能需要 Univer 服務端支援，請確保你已經正確安裝並配置了 Univer 服務端。具體請參考：[升級到
  Pro](/guides/sheets/getting-started/pro)
</Callout>

協同編輯功能允許多個使用者同時編輯同一文件，實時同步更改，適用於團隊協作和多人編輯場景。

<PlaygroundFrame lang="zh-TW" slug="sheets/univer-pro-collaboration" clickToShow />
...
<Callout>
  @univerjs/preset-sheets-collaboration 的 `UniveSheetsCollaborationPreset` 預設在執行時依賴 `UniverSheetsDrawingPreset`
  和 `UniverSheetsAdvancedPreset` 預設，請先安裝 @univerjs/preset-sheets-drawing 和 @univerjs/preset-sheets-advanced。
</Callout>
...
- 透過[匯入外掛](/guides/sheets/features/import-export)提供的 `FUniver.importSheetToUnitIdAsync` 方法，將 `.xlsx` 檔案匯入到 Univer 服務。
