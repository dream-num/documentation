...
@univerjs-pro/collaboration-client 外掛內部提供了根據 URL 引數 `unit` 和 `type` 自動載入對應的資料的功能，這可以簡化一些場景下的資料載入邏輯。

如果你想使用該特性，你需要適當地修改一下原有的載入資料邏輯，並將 `unit` 和 `type` 引數新增到 URL 中，如下所示：
...
如果你不想使用 URL 引數來載入協同文件，你也可以透過 Facade API 來載入協同文件。
