...
```typescript
import ChartUIZhCN from '@univerjs-pro/chart-ui/locale/zh-CN' // [!code ++]
...
import SheetsChartUIZhCN from '@univerjs-pro/sheets-chart-ui/locale/zh-CN' // [!code ++]
...
const univer = new Univer({
  ...
  locales: {
    [LocaleType.ZH_CN]: mergeLocales(
      ChartUIZhCN, // [!code ++]
      SheetsChartZhCN, // [!code ++]
      SheetsChartUIZhCN, // [!code ++]
    ),
  },
})
...
Univer Sheets 支持以下图表类型：

| 图表类型 | 说明 |
| --- | --- |
| Column | 柱状图 |
| ColumnStacked | 堆积柱状图 |
| ColumnPercentStacked | 百分比堆积柱状图 |
| Bar | 条形图 |
| BarStacked | 堆积条形图 |
...
目前我们支持以下的配置项，你可以通过查阅 IChartBuildOptions 来获取对应的配置：
