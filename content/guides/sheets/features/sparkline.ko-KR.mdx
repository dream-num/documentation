...
Univer 상용 라이선스가 있다면 구성 방법은 [클라이언트에서 라이선스 사용하기](/server/license#in-plugin-mode)를 참고하세요.
...
Univer 상용 라이선스가 있다면 구성 방법은 [클라이언트에서 라이선스 사용하기](/server/license#in-preset-mode)를 참고하세요.
...
[`FSparklineGroup`](/reference/facade/sparkline-group) 인스턴스에는 스파크라인 그룹을 조작하는 다음 메서드가 있습니다.
...
색상과 표시 옵션은 `FSparklineGroup.setConfig()`로 설정하며 그룹 전체에 적용됩니다. 별도의 `FSparkline.setColor()` 메서드는 없습니다. 사용자 지정 색상을 사용하려면 `seriesColor`와 `themeType: SparklineThemeTypeEnum.CUSTOM`을 함께 설정하세요. 마커, 축, 최댓값과 최솟값, 빈 셀, 숨겨진 셀, 선 두께를 포함한 전체 옵션과 예제는 [스파크라인 그룹 설정](/reference/facade/sparkline-group)을 참고하세요.
