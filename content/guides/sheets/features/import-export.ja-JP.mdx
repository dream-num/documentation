...
<Callout type="warning" title="移行について">
  パッケージルートの `transformSnapshotJsonToWorkbookData` と `transformWorkbookDataToSnapshotJson` は削除されました。`@univerjs-pro/exchange-client/facade` をインポートし、以下の Facade API を使用してください。
</Callout>
