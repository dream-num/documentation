...
<Callout type="warning" title="移行について">
  パッケージルートからエクスポートされる `transformSnapshotJsonToWorkbookData` と `transformWorkbookDataToSnapshotJson` は互換性のために残されていますが、非推奨であり、v1.1.0 で削除される予定です。新しいコードでは以下の Facade API を使用してください。
</Callout>
