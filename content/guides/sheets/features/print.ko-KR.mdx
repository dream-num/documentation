...
이 기능은 [라이선스](/guides/server-integration/license)가 있어야 사용할 수 있습니다. 라이선스가 없는 사용자는 이용에 제한을 받으며 저장 작업은 `false`를 반환합니다.
...
이 기능은 [라이선스](/guides/server-integration/license)가 있어야 사용할 수 있습니다. 라이선스가 없는 사용자는 이용에 제한을 받으며 작업에 실패하면 `false`, 성공하면 이미지의 base64 문자열을 반환합니다.
