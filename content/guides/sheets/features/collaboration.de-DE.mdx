...
<Callout>
Die Voreinstellung `UniveSheetsCollaborationPreset` von `@univerjs/preset-sheets-collaboration` hängt von den Voreinstellungen `UniverSheetsDrawingPreset` und `UniverSheetsAdvancedPreset` zur Laufzeit ab. Bitte installieren Sie zuerst `@univerjs/preset-sheets-drawing` und `@univerjs/preset-sheets-advanced`.
</Callout>
...
- Sie können eine `.xlsx`-Datei mit der von der bereitgestellten Methode `FUniver.importXLSXToUnitIdAsync` in den Univer-Server importieren [Import-Plugin](/guides/sheets/features/import-export).
...
Nur der Plugin-Modus erfordert eine manuelle Eingabe Importieren des Facade-Pakets. Der Preset-Modus beinhaltet bereits das entsprechende Facade-Paket, sodass kein zusätzlicher Import erforderlich ist.
</Callout>
