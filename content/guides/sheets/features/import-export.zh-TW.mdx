...
使用下方瀏覽器 API 前，請先完成[匯入匯出整合](/server/import-export)。應用程式需要實作與用戶端匹配的上傳、轉換、任務查詢和受權限保護的下載契約。快照 API 仍需要轉換後端，只是不要求持久化協同 Unit。[Exchange 範例](/server/collaboration/examples#exchange)示範 Sheets；其他產品和格式需核對目標版本的支援範圍。
