...
透過選單欄的匯入入口成功匯入後，頁面左下角會彈出提示框，顯示匯入成功的資訊，並提供訪問匯入文件的連結。若預設的匯入匯出行為無法滿足你的需求，請檢視[《自定義匯入文件》](/guides/recipes/tutorials/custom-import)以瞭解如何自定義匯入行為。
...
在啟用協同編輯的時候，每個文件都有一個唯一的 `unitId`。使用 `univerAPI.importXLSXToUnitIdAsync` 傳入 `file` 引數會返回 `unitId`，可以透過 `unitId` 來訪問文件。 `file` 引數可以是一個 `File` 物件，也可以是遠端檔案的 URL。
...
// 或者呼叫API: univerAPI.loadServerUnit(unitId, 2) 載入文件，具體引數參考 https://reference.univer.ai/zh-TW/classes/FUniver#loadserverunit
```
...
在啟用協同編輯的時候，每個文件都有一個唯一的 `unitId`。使用 `univerAPI.exportXLSXByUnitIdAsync` 傳入 `unitId` 引數會返回 `File` 物件。
