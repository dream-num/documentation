...
<Callout title="為什麼 Univer 透過後端服務實現匯入匯出？">
  僅透過前端實現的匯入匯出無論是從效能還是效果上來說都無法滿足企業需求，因此我們提供了後端服務來實現匯入匯出功能。你可以透過一些開源的 XLSX 解析庫將檔案解析為符合 [`IWorksheetData`](https://reference.univer.ai/zh-TW/interfaces/IWorkbookData) 介面的資料結構，然後透過 Facade API 將資料匯入到 Univer 中。
</Callout>
