...
<Callout type="warning" title="注意事項">
  匯入匯出功能需要 Univer 服務端支援，請確保你已經正確安裝並配置了 Univer 服務端。具體請參考：[升級到 Pro](/guides/sheets/getting-started/pro)
</Callout>
...
<Callout title="為什麼 Univer 透過後端服務實現匯入匯出？">
  僅透過前端實現的匯入匯出無論是從效能還是效果上來說都無法滿足企業需求，因此我們提供了後端服務來實現匯入匯出功能。你可以透過一些開源的 XLSX 解析庫將檔案解析為符合 [`IWorksheetData`](/guides/sheets/model/workbook-data) 介面的資料結構，然後透過 Facade API 將資料匯入到 Univer 中。
</Callout>
...
| --- | --- | --- | --- |
| 透過 `unitId` | 需要 | 已啟用協同、需多人共享/持續協作的文件 | `importXLSXToUnitIdAsync` / `exportXLSXByUnitIdAsync` |
| 透過快照（snapshot） | 不需要 | 本機/單人/離線匯入匯出，或僅做格式轉換 | `importXLSXToSnapshotAsync` / `exportXLSXBySnapshotAsync` |
...
在啟用協同編輯的時候，每個文件都有一個唯一的 `unitId`。使用 `univerAPI.importXLSXToUnitIdAsync` 傳入 `file` 參數會返回 `unitId`，可以透過 `unitId` 來訪問文件。 `file` 參數可以是一個 `File` 物件，也可以是遠端檔案的 URL。
...
使用 `univerAPI.importXLSXToSnapshotAsync` 匯入 `.xlsx` 檔案，會返回 `IWorkbookData` 格式的文件資料。
...
在啟用協同編輯的時候，每個文件都有一個唯一的 `unitId`。使用 `univerAPI.exportXLSXByUnitIdAsync` 傳入 `unitId` 參數會返回 `File` 物件。
...
使用 `univerAPI.exportXLSXBySnapshotAsync` 傳入 `IWorkbookData` 會返回 `File` 物件。
...
<Callout type="warning" title="遷移說明">
  套件根目錄匯出的 `transformSnapshotJsonToWorkbookData` 和 `transformWorkbookDataToSnapshotJson` 已移除。請匯入 `@univerjs-pro/exchange-client/facade` 並使用下方的 Facade API。
</Callout>
