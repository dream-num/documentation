...
<Callout type="warning" title="注意事項">
  匯入匯出功能需要 Univer 服務端支援，請確保你已經正確安裝並配置了 Univer 服務端。具體請參考：[升級到
  Pro](/guides/sheets/getting-started/pro)
</Callout>

匯入和匯出功能允許使用者透過 Univer 服務端將 `.xlsx` 檔案匯入到 Univer 文件中，或將文件內容匯出為 `.xlsx` 檔案，便於與其他辦公軟體或平臺進行互動。

<PlaygroundFrame lang="zh-TW" slug="sheets/univer-pro-import-export" clickToShow />

<Callout title="為什麼 Univer 透過後端服務實現匯入匯出？">
  僅透過前端實現的匯入匯出無論是從效能還是效果上來說都無法滿足企業需求，因此我們提供了後端服務來實現匯入匯出功能。你可以透過一些開源的
  XLSX 解析庫將檔案解析為符合 [`IWorksheetData`](/guides/sheets/model/workbook-data) 介面的資料結構，然後透過 Facade API
  將資料匯入到 Univer 中。
</Callout>
...
| -------------------- | ------------ | -------------------------------------- | ----------------------------------------------------------- |
| 透過 `unitId`        | 需要         | 已啟用協同、需多人共享/持續協作的文件  | `importSheetToUnitIdAsync` / `exportSheetByUnitIdAsync`     |
| 透過快照（snapshot） | 不需要       | 本機/單人/離線匯入匯出，或僅做格式轉換 | `importSheetToSnapshotAsync` / `exportSheetBySnapshotAsync` |
...
<Callout>
  @univerjs/preset-sheets-advanced 的 `UniverSheetsAdvancedPreset` 預設在執行時依賴 `UniverSheetsDrawingPreset`
  預設，請先安裝 @univerjs/preset-sheets-drawing。
</Callout>
...
interface IUniverSheetsAdvancedPresetConfig {
  ...
  exchangeClientOptions?: {
    ...
    /**
     * 匯出是否啟用伺服器端公式計算。啟用後，每次匯出都會觸發服務端公式計算。解決如下問題：
     * 1. WPS 等辦公軟體開啟檔案時公式無預設值
     * 2. EXCEL 不啟用編輯，無法看到公式計算結果
     * 3. 陣列公式存在字首@，導致無法展開計算結果
     * 注意，此配置需要在協同編輯環境下使用，服務端配置檔案需要配置 SSC_SERVER_ENABLED=true。
     */
    enableServerSideComputing?: boolean
    /**
     * 是否在匯出時禁用將單元格圖片轉換為連結。
     * 如果單元格圖片源是一個 URL，預設行為是在匯出時將該 URL 轉換為單元格的值。
     * 由於伺服器會下載所有的資原始檔，當圖片較多時，會消耗大量資源並且耗時較長。
     * @default true
     */
...
import SheetsExchangeClientZhTW from '@univerjs-pro/sheets-exchange-client/locale/zh-TW' // [!code ++]
...
// 配合協同編輯自動載入資料一同使用 https://docs.univer.ai/zh-TW/guides/sheets/features/collaboration#loading-collaborative-documents
...
// 或者呼叫API: univerAPI.loadServerUnit(unitId, 2) 載入文件，具體參數參考 /reference/facade/univer#loadserverunit
```

#### 匯入 `.xlsx` 檔案並獲取 `IWorkbookData`

<Callout>快照匯入不依賴協同服務，適合本機/單人/離線場景。需要協同文件請使用 `unitId` 匯入。</Callout>
...
Univer 服務端提供了匯入匯出的 API，但是服務端轉換的資料和 Univer 格式不一致。在 Node.js 執行環境中初始化 Univer 並註冊 `@univerjs-pro/exchange-client` 後，請透過 Facade API 轉換資料。

### Snapshot 轉化為 `IWorkbookData`

匯入後得到的 Snapshot JSON 資料需要轉化為 `IWorkbookData` 格式的資料才能在 Univer 中使用。

```typescript
// json 資料包含 snapshot 和 sheetBlocks
const workbookData = await univerAPI.transformSnapshotJsonToWorkbookDataAsync(json)
```

### `IWorkbookData` 轉化為 Snapshot

匯出前需要將 `IWorkbookData` 格式的資料轉化為 Snapshot JSON 資料，才能傳給服務端進行匯出。
