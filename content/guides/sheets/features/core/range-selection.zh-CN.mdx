...
范围完整 Facade API 类型定义，请查看 [FRange Facade API](https://docs.univer.ai/reference/facade/range)
...
如果只想设置范围左上角的单元格值，可以使用 [`FRange.setValueForCell`](https://docs.univer.ai/reference/facade/range#setvalueforcell) 方法：
...
- [`FRange.clear`](https://docs.univer.ai/reference/facade/range#clear) 方法可以清理范围的内容和格式信息。
- [`FRange.clearContent`](https://docs.univer.ai/reference/facade/range#clearcontent) 方法可以清理范围的内容信息。
- [`FRange.clearFormat`](https://docs.univer.ai/reference/facade/range#clearformat) 方法可以清理范围的格式信息。
...
[`FRange.insertCells`](https://docs.univer.ai/reference/facade/range#insertcells) 方法可以在此范围内插入空单元格。工作表中沿指定维度的现有数据将移离插入的范围。
...
[`FRange.deleteCells`](https://docs.univer.ai/reference/facade/range#deletecells) 方法可以在范围内删除单元格。工作表中沿指定维度的现有数据将向已删除的范围移动。
...
[`FRange.highlight`](https://docs.univer.ai/reference/facade/range#highlight) 方法可以使用指定样式和主单元格高亮范围。
...
[`FRange.splitTextToColumns`](https://docs.univer.ai/reference/facade/range#splittexttocolumns) 方法可以将单元格中的文本按指定分隔符分列。
...
选区完整 Facade API 类型定义，请查看 [FSelection Facade API](https://docs.univer.ai/reference/facade/selection)
...
2. 插件还会将拓展的单元格属性存储在 `Workbook` 的 `resources` 属性中，详细请阅读 [插件自定义模型](/blog/custom-model/)。
...
| `BeforeSheetEditStart` | 单元格开始编辑前触发       | `IBeforeSheetEditStartEventParams` | `const { worksheet, workbook, row, column, eventType, keycode } = params`                   |
| `SheetEditStarted`     | 单元格开始编辑时触发       | `ISheetEditStartedEventParams`     | `const { worksheet, workbook, row, column, eventType, keycode } = params`                   |
| `SheetEditChanging`    | 单元格编辑内容变化时触发   | `ISheetEditChangingEventParams`    | `const { worksheet, workbook, row, column, value } = params`                                |
| `BeforeSheetEditEnd`   | 单元格结束编辑前触发       | `IBeforeSheetEditEndEventParams`   | `const { worksheet, workbook, row, column, value, eventType, keycode, isConfirm } = params` |
| `SheetEditEnded`       | 单元格结束编辑后触发       | `ISheetEditEndedEventParams`       | `const { worksheet, workbook, row, column, eventType, keycode, isConfirm } = params`        |
...
`registerCellCustomRender` 用于注册单元格自定义渲染器。
...
`BeforeClipboardChange` 事件在剪贴板内容改变之前触发。你可以使用此事件在内容改变之前监控或修改剪贴板内容。
...
`BeforeClipboardPaste` 事件在内容粘贴之前触发。你可以使用此事件在粘贴之前监控或修改内容。
