...
<Callout>
  对单元格的操作可以看作对行高 1 、列宽 1 的范围进行操作，操作范围请阅读 [范围-range](/guides/sheets/features/core/range-selection#range)。
</Callout>

<Callout>
  插件还会将拓展的单元格属性存储在 `Workbook` 的 `resources` 属性中，详细请阅读 [插件自定义模型](/guides/recipes/tutorials/custom-model/)。
</Callout>
