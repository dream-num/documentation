...
In the UI interface, you can set the number format of the cell through the menu bar. Or set the [`s.n.pattern`](/guides/sheets/model/cell-data#number-format)[^number-format-value] field in the cell data [`ICellData`](/guides/sheets/model/cell-data) object.
...
You can use the [`FWorkbook.setNumfmtLocal(local)`](https://docs.univer.ai/reference/facade/workbook#setnumfmtlocal) method to set the locale for number formatting.
...
[^number-format-value]: Number formatting changes display, not the stored numeric value. For example, formatting `0.25` as `0%` displays `25%`; formulas still calculate with `0.25`.
