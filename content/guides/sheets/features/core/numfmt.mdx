...
<Callout type="warning" emoji="Caution">
  The **DBNum syntax** is not supported.
</Callout>

## Usage

In the UI interface, you can set the number format of the cell through the menu bar. Or set the [`s.n.pattern`](/en-US/guides/sheets/model/cell-data#number-format) field in the cell data [`ICellData`](/en-US/guides/sheets/model/cell-data) object.
...
2. Add a single quote `'` before entering the number (this is known as [forcing text](/en-US/guides/sheets/model/cell-data#cell-type)).
