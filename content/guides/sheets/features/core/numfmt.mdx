...
UI 界面中，可以通过菜单栏设置单元格的数字格式。或者在单元格数据 [`ICellData`](/en-US/guides/sheets/getting-started/cell-data) 对象中设置 [`s.n.pattern`](/en-US/guides/sheets/getting-started/cell-data#number-format) 字段。
...
使用 [`FWorkbook.setNumfmtLocal(local)`](https://reference.univer.ai/en-US/classes/FWorkbook#setnumfmtlocal) 方法设置数字格式的区域设置。
...
可将单元格数字格式设置为文本格式，或在输入数字前加上单引号 `'`（[强制文本](/en-US/guides/sheets/getting-started/cell-data#cell-type)）。
