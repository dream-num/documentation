...
[`univerAPI.newDefinedName()`](https://reference.univer.ai/en-US/classes/FUniver#newdefinedname) 创建一个定义名称构建器，返回一个 `FDefinedNameBuilder` 实例，可以通过链式调用生成 `ISetDefinedNameMutationParam` 对象用于创建定义名称。

以下是 [`FDefinedNameBuilder`](https://reference.univer.ai/en-US/classes/FDefinedNameBuilder) 上的一些成员方法：
...
通过 [`FWorkbook.insertDefinedNameBuilder()`](https://reference.univer.ai/en-US/classes/FWorkbook#insertdefinednamebuilder) 方法可以创建整个工作簿可用的定义名称。
...
也可以通过 [`FWorkbook.insertDefinedName()`](https://reference.univer.ai/en-US/classes/FWorkbook#insertdefinedname) 方法快速创建定义名称。
...
其他方法：
- [`FWorksheet.insertDefinedName()`](https://reference.univer.ai/en-US/classes/FWorksheet#insertdefinedname)：创建工作表可用的定义名称。

### 获取定义名称

[`FWorkbook.getDefinedName()`](https://reference.univer.ai/en-US/classes/FWorkbook#getdefinedname) 方法可以获取工作簿中已定义的名称，返回一个 `FDefinedName` 实例。
...
其他方法：
- [`FWorkbook.getDefinedNames()`](https://reference.univer.ai/en-US/classes/FWorkbook#getdefinednames) 方法可以获取工作簿中所有已定义的名称，返回一个 `FDefinedName[]` 数组。
- [`FWorksheet.getDefinedNames()`](https://reference.univer.ai/en-US/classes/FWorksheet#getdefinednames) 方法可以获取所有此工作表可用的定义名称，返回一个 `FDefinedName[]` 数组。
...
也可以通过返回的 [`FDefinedName`](https://reference.univer.ai/en-US/classes/FDefinedName) 实例直接修改定义名称配置。
...
[`FWorkbook.deleteDefinedName()`](https://reference.univer.ai/en-US/classes/FWorkbook#deletedefinedname) 方法可以删除工作簿中已定义的名称。
...
也可以通过返回的 [`FDefinedName`](https://reference.univer.ai/en-US/classes/FDefinedName) 实例直接删除定义名称。
