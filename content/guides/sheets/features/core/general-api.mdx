...
<Callout type="info" emoji="ℹ️">
  如需了解更多设计细节请阅读 [Univer 命令系统](/en-US/introduction/architecture/univer#command-system) 。
</Callout>
...
完整事件列表请参考：https://reference.univer.ai/en-US/classes/FEventName#%E5%B1%9E%E6%80%A7
...
从 `0.2.12` 开始，使用 [`FUniver.copy()`](https://reference.univer.ai/en-US/classes/FUniver#copy) 和 [`FUniver.paste()`](https://reference.univer.ai/en-US/classes/FUniver#paste) 方法读取和写入系统剪贴板。

<Callout type="info" emoji="ℹ️">
  复制与粘贴依赖浏览器原生 API ，当[环境条件或者权限](https://developer.mozilla.org/en-US/docs/Web/API/Clipboard/writeText)不满足时复制与粘贴功能将无法工作。
</Callout>
...
- [扩展 Canvas](/en-US/guides/recipes/practices/custom-canvas)
- [自定义主题](/en-US/guides/recipes/practices/custom-theme)
...
如下案例所示，使用 [`registerFunction`](https://reference.univer.ai/en-US/classes/FFormula#registerfunction) 将一个 `CUSTOMSUM` 公式所需要的算法、名称、描述一次性注册到公式插件，执行之后就可以使用公式了。在任一空白单元格输入 `=CUSTOMSUM` 可以看到提示。
...
- `locales` 下可以设置多种语言，命名规则参考 [LocaleType](https://reference.univer.ai/en-US/globals#localetype)。可以在 `functionList` 下添加多个公式的翻译。详细的字段说明请参考 [如何在 Formula Engine 中添加公式](/en-US/guides/recipes/practices/custom-formula#how-to-add-formulas-in-formula-engine) 的部分。

<Callout type="info" emoji="ℹ️">
  如果想复用公式系统提供的算法，增强公式算法的能力，可以通过插件配置的方式注册自定义公式，详细教程请参考 [自定义公式](/en-US/guides/recipes/practices/custom-formula/)。
</Callout>
