...
<Callout>For more design details, please read [Univer Command System](/blog/univer#command-system).</Callout>
...
<Callout>
  Commands can be simply understood as unique "events" within Univer. For more details on the design, please refer to
  [command-system](/blog/univer#command-system).
</Callout>
...
- `Event.BeforeCommandExecute`: Executes custom logic before the command is executed.
- `Event.CommandExecuted`: Executes custom logic after the command is executed.

#### Listening Commands Before Execution

Before the command is executed, pass `Event.BeforeCommandExecute` to `FUniver.addEvent` to register a custom preprocessing listener.
...
To prevent the command from executing, set `event.cancel` to `true` in the listener callback.
...
After the command is executed, pass `Event.CommandExecuted` to `FUniver.addEvent` to register a custom post-processing listener.
...
Full event list: /reference/facade/events#available-event-names
...
<Callout>
  Copy and paste rely on the browser's native API. When environmental conditions or permissions are insufficient, the
  copy and paste functions will not work. For more information, see [MDN
...
- [Extending Canvas](/blog/custom-canvas)
...
Use piecemeal installation, you need to install the [`@univerjs/network`](/reference/packages/plugins/univerjs/network) dependency package and register the `UniverNetworkPlugin` plugin.
...
<Callout>
  If you want to reuse the algorithm provided by the formula system and enhance the capability of the formula algorithm,
  you can register a custom formula through plugin configuration. For detailed tutorials, please refer to [Custom
  Formula](/blog/custom-formula).
</Callout>
