...
<Callout>如需了解更多设计细节请阅读 [Univer 命令系统](/blog/univer#command-system)。</Callout>
...
<Callout>
  每一个命令对应了一个唯一的 ID。如果你正在寻找某个特定的命令 ID，可以参考 [如何查找命令
  ID](/blog/find-the-command-id)。
</Callout>
...
- `Event.BeforeCommandExecute`：在命令执行前执行自定义逻辑。
- `Event.CommandExecuted`：在命令执行后执行自定义逻辑。

#### 监听命令执行前

命令执行前，将 `Event.BeforeCommandExecute` 传给 `FUniver.addEvent` 来注册预处理监听器。
...
如需阻止命令执行，在监听器中将 `event.cancel` 设为 `true`。
...
命令执行后，将 `Event.CommandExecuted` 传给 `FUniver.addEvent` 来注册后处理监听器。
...
完整事件列表请参考：/reference/facade/events#%E5%B1%9E%E6%80%A7
...
使用 [`FUniver.copy()`](https://docs.univer.ai/reference/facade/univer#copy) 和 [`FUniver.paste()`](https://docs.univer.ai/reference/facade/univer#paste) 方法可以读取和写入系统剪贴板。

<Callout>
  复制与粘贴依赖浏览器原生 API
  ，当[环境条件或者权限](https://developer.mozilla.org/zh-CN/docs/Web/API/Clipboard/writeText)不满足时复制与粘贴功能将无法工作。
</Callout>
...
使用手动组合安装，需要安装 [`@univerjs/network`](/reference/packages/plugins/univerjs/network) 依赖包，并注册 `UniverNetworkPlugin` 插件。
...
如下案例所示，使用 [`registerFunction`](https://docs.univer.ai/reference/facade/formula#registerfunction) 将一个 `CUSTOMSUM` 公式所需要的算法、名称、描述一次性注册到公式插件，执行之后就可以使用公式了。在任一空白单元格输入 `=CUSTOMSUM` 可以看到提示。
...
- `locales` 下可以设置多种语言，命名规则参考 [LocaleType](https://reference.univer.ai/zh-CN/globals#localetype)。可以在 `functionList` 下添加多个公式的翻译。详细的字段说明请参考 [如何在 Formula Engine 中添加公式](/blog/custom-formula#how-to-add-formulas-in-formula-engine) 的部分。

<Callout>
  如果想复用公式系统提供的算法，增强公式算法的能力，可以通过插件配置的方式注册自定义公式，详细教程请参考
  [自定义公式](/blog/custom-formula/)。
</Callout>
