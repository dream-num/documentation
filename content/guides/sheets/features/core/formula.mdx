    ...
    plugins: [
      {
        ...
        locale: '@univerjs/engine-formula/locale/en-US',
      },
      {
        client: '@univerjs/sheets-formula',
        facade: '@univerjs/sheets-formula/facade',
        locale: '@univerjs/sheets-formula/locale/en-US',
      },
      {
...
## Cross-workbook and 3D references

Formulas can reference loaded workbooks by display name and span an inclusive range of worksheets in workbook order:
...
Workbook names are matched case-insensitively, with common Excel extensions ignored as a fallback. A missing or ambiguous workbook name returns `#REF!`.
