...
[`deleteRowsByPoints(rowPoints)`](https://reference.univer.ai/zh-CN/classes/FWorksheet#deleterowsbypoints): 指定された行位置の配列に基づいて、複数の行（不連続）を削除します。
...
[`deleteColumnsByPoints(columnPoints)`](https://reference.univer.ai/zh-CN/classes/FWorksheet#deletecolumnsbypoints): 指定された列位置の配列に基づいて、複数の列（不連続）を削除します。
