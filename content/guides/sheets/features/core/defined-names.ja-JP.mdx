...
### 定義名を作成する [#create-defined-names]

`FWorkbook.newDefinedNameBuilder` は定義名ビルダーを生成し、`FDefinedNameBuilder` インスタンスを返します。メソッドチェーンで `ISetDefinedNameMutationParam` オブジェクトを構築し、定義名を挿入できます。

[`FDefinedNameBuilder`](https://docs.univer.ai/reference/facade/defined-name) の主なメンバー:
...
[`FWorkbook.insertDefinedNameBuilder()`](https://docs.univer.ai/reference/facade/workbook#insertdefinednamebuilder) はブック全体で利用可能な定義名を作成します。
...
[`FWorkbook.insertDefinedName()`](https://docs.univer.ai/reference/facade/workbook#insertdefinedname) で簡易作成も可能です。
...
その他:
- [`FWorksheet.insertDefinedName()`](https://docs.univer.ai/reference/facade/worksheet#insertdefinedname): 特定ワークシートのみ有効な定義名を作成。

### 定義名の取得

[`FWorkbook.getDefinedName()`](https://docs.univer.ai/reference/facade/workbook#getdefinedname) はブック内の特定定義名を取得し、`FDefinedName` インスタンスを返します。
...
その他:
- [`FWorkbook.getDefinedNames()`](https://docs.univer.ai/reference/facade/workbook#getdefinednames): ブック内すべての定義名配列 (`FDefinedName[]`) を取得。
- [`FWorksheet.getDefinedNames()`](https://docs.univer.ai/reference/facade/worksheet#getdefinednames): そのワークシートで利用可能な定義名配列を取得。

### 定義名の変更

[`FWorkbook.updateDefinedNameBuilder()`](https://docs.univer.ai/reference/facade/workbook#updatedefinednamebuilder) は既存定義名を変更します。
...
[`FDefinedName`](https://docs.univer.ai/reference/facade/defined-name) インスタンスから直接更新も可能です。
...
[`FWorkbook.deleteDefinedName()`](https://docs.univer.ai/reference/facade/workbook#deletedefinedname) で定義名を削除します。
...
取得済みの [`FDefinedName`](https://docs.univer.ai/reference/facade/defined-name) インスタンスから直接削除することも可能です。
