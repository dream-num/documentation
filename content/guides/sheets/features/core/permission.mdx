...
Similar to hiding the permission background shadow, you can add `UNIVER_SHEET_PERMISSION_USER_PART` in `customComponents`, which represents a custom permission user component. The custom user component can be registered by yourself, and if not registered, the default user component will be hidden.
