...
To distinguish protected areas that the current user can edit from those they cannot, use `non-editable`. Editable protected areas have no shadow; protected areas with editing disabled keep their shadow. Non-viewable protected ranges remain masked.

This setting controls the display of range and worksheet protection. It does not create protection rules or change edit/view permissions, and it does not add shadows to unprotected areas. Configure the protection rules and the current user's permissions first, as shown above.

| Strategy | Display behavior |
| --- | --- |
| `true` / `'always'` | Show shadows for all protected areas (default). |
| `'non-editable'` | Hide shadows on editable protected areas; keep them on non-editable areas. |
| `'non-viewable'` | Show shadows only on non-viewable protected areas. |
| `false` / `'none'` | Hide all protection shadows; permission checks still apply. |

<PermissionShadowDemo />

Set the strategy during initialization:
...
The Facade API below updates existing workbooks in the current Univer instance. In plugin mode, first import `@univerjs/sheets-ui/facade`; the Sheets Core preset already includes it.
...
<Callout>
  Collaborator management belongs to your application and permission service. Connect your user/ACL model and enforce server-side read, JOIN, and edit policies; see [identity and authorization](/server/collaboration/identity-and-authorization). The frontend APIs below require a matching permission-service implementation and do not automatically configure server authorization.
</Callout>
