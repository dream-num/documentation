...
UI 界面中，可以通过菜单栏设置单元格的数字格式。或者在单元格数据 [`ICellData`](/zh-CN/guides/sheets/model/cell-data) 对象中设置 [`s.n.pattern`](/zh-CN/guides/sheets/model/cell-data#number-format) 字段。
...
2. 在输入数字前加上单引号 `'`（[强制文本](/zh-CN/guides/sheets/model/cell-data#cell-type)）
