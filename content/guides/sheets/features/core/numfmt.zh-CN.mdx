...
工具栏的货币操作会根据 Univer 的 `region` 选择货币符号和格式。未显式配置时，`region` 会跟随 `locale`：
...
UI 界面中，可以通过菜单栏设置单元格的数字格式。或者在单元格数据 [`ICellData`](/guides/sheets/model/cell-data) 对象中设置 [`s.n.pattern`](/guides/sheets/model/cell-data#number-format)[^number-format-value] 字段。
...
2. 在输入数字前加上单引号 `'`（[强制文本](/guides/sheets/model/cell-data#cell-type)）
...
使用 [`FWorkbook.setNumfmtLocal(local)`](https://docs.univer.ai/reference/facade/workbook#setnumfmtlocal) 方法设置数字格式的区域设置。
...
[^number-format-value]: 数字格式只改变显示，不改变存储的数值。例如将 `0.25` 设置为 `0%` 会显示 `25%`，公式仍使用 `0.25` 参与计算。
