...
範囲関連 Facade API の完全な定義は [FRange FacadeAPI](https://docs.univer.ai/reference/facade/range) を参照してください。
...
範囲左上セルのみ設定したい場合は [`FRange.setValueForCell`](https://docs.univer.ai/reference/facade/range#setvalueforcell)：
...
- [`FRange.clear`](https://docs.univer.ai/reference/facade/range#clear)： コンテンツと書式をすべてクリア
- [`FRange.clearContent`](https://docs.univer.ai/reference/facade/range#clearcontent)： コンテンツのみクリア
- [`FRange.clearFormat`](https://docs.univer.ai/reference/facade/range#clearformat)： 書式のみクリア
...
[`FRange.insertCells`](https://docs.univer.ai/reference/facade/range#insertcells) は指定範囲内に空セルを挿入し、指定次元方向に既存データを押し出します。
...
[`FRange.deleteCells`](https://docs.univer.ai/reference/facade/range#deletecells) は指定範囲を削除し、指定次元方向へデータを詰めます。
...
[`FRange.highlight`](https://docs.univer.ai/reference/facade/range#highlight) は指定スタイルと主セル（primary cell）で範囲を強調表示します。
...
[`FRange.splitTextToColumns`](https://docs.univer.ai/reference/facade/range#splittexttocolumns) は指定した区切り文字に従いテキストを複数列へ分割します。
...
選択関連 Facade API の完全な定義は [FSelection FacadeAPI](https://docs.univer.ai/reference/facade/selection) を参照してください。
...
2. Univer API プラグインは拡張セル属性を `Workbook` の `resources` プロパティにも格納します。詳細は [Plugin Custom Model](/blog/custom-model) を参照。
...
| `BeforeSheetEditStart` | セル編集開始前               | `IBeforeSheetEditStartEventParams` | `const { worksheet, workbook, row, column, eventType, keycode } = params`                   |
| `SheetEditStarted`     | セル編集開始時               | `ISheetEditStartedEventParams`     | 同上                                                                                        |
| `SheetEditChanging`    | 編集中内容変更               | `ISheetEditChangingEventParams`    | `const { worksheet, workbook, row, column, value } = params`                                |
| `BeforeSheetEditEnd`   | セル編集終了直前             | `IBeforeSheetEditEndEventParams`   | `const { worksheet, workbook, row, column, value, eventType, keycode, isConfirm } = params` |
...
`registerCellCustomRender` はセル用のカスタムレンダラーを登録します。
...
`BeforeSheetEditEnd` イベントは、セルの編集が終了する前にトリガーされます。
...
`SheetEditEnded` イベントは、セルの編集が終了した後にトリガーされます。
