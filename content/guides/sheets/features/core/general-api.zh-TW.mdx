...
<Callout>如需了解更多設計細節請閱讀 [Univer 命令系統](/blog/univer#command-system)。</Callout>
...
<Callout>
  每一個命令對應了一個唯一的 ID。如果你正在尋找某個特定的命令 ID，可以參考 [如何查找命令
...
- `Event.BeforeCommandExecute`：在命令執行前執行自訂邏輯。
- `Event.CommandExecuted`：在命令執行後執行自訂邏輯。

#### 監聽命令執行前

命令執行前，將 `Event.BeforeCommandExecute` 傳給 `FUniver.addEvent` 來註冊預處理監聽器。
...
如需阻止命令執行，在監聽器中將 `event.cancel` 設為 `true`。
...
命令執行後，將 `Event.CommandExecuted` 傳給 `FUniver.addEvent` 來註冊後處理監聽器。
...
完整事件列表請參考：/reference/facade/events#%E5%B1%9E%E6%80%A7
...
使用 [`FUniver.copy()`](https://docs.univer.ai/reference/facade/univer#copy) 和 [`FUniver.paste()`](https://docs.univer.ai/reference/facade/univer#paste) 方法可以讀取和寫入系統剪貼板。

<Callout>
  複製與貼上依賴瀏覽器原生 API
  ，當[環境條件或者權限](https://developer.mozilla.org/zh-CN/docs/Web/API/Clipboard/writeText)不滿足時複製與貼上功能將無法工作。
</Callout>
...
使用手動組合安裝，需要安裝 [`@univerjs/network`](/reference/packages/plugins/univerjs/network) 依賴包，並註冊 `UniverNetworkPlugin` 插件。
...
如下案例所示，使用 [`registerFunction`](https://docs.univer.ai/reference/facade/formula#registerfunction) 將一個 `CUSTOMSUM` 公式所需的演算法、名稱、描述一次性註冊到公式插件，執行之後就可以使用公式了。在任一空白儲存格輸入 `=CUSTOMSUM` 可以看到提示。
          ...
          CUSTOMSUM: {
            ...
            functionParameter: {
              number1: {
                name: '數值1',
                detail:
                  '要相加的第一個數字。 該數字可以是 4 之類的數字，B6 之類的儲存格引用或 B2:B8 之類的儲存格範圍。',
              },
              number2: {
                name: '數值2',
...
- `locales` 下可以設定多種語言，命名規則參考 [LocaleType](https://reference.univer.ai/zh-CN/globals#localetype)。可以在 `functionList` 下添加多個公式的翻譯。詳細的欄位說明請參考 [如何在 Formula Engine 中添加公式](/blog/custom-formula#how-to-add-formulas-in-formula-engine) 的部分。

<Callout>
  如果想重用公式系統提供的演算法，增強公式演算法的能力，可以通過插件配置的方式註冊自訂公式，詳細教程請參考
  [自訂公式](/blog/custom-formula/)。
</Callout>
