...
<Callout>
  协作者管理由业务应用和权限服务负责。请接入自己的用户与 ACL 模型，并在服务端分别校验读取、JOIN 和修改权限，见[身份与权限](https://office.univer.ai/zh-CN/collaboration/identity-and-authorization)。下方前端 API 需要匹配的权限服务实现，不会自动配置服务端权限。
</Callout>
