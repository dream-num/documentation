...
[`univerAPI.createWorkbook(data, options)`](https://reference.univer.ai/en-US/classes/FUniver#createworkbook) 方法会创建并返回 `FWorkbook` 实例。
...
此外，`univer` 和 `univerAPI` 是两个不同的实例，关于如何获取它们的实例，请参考 [安装和基本使用](/en-US/guides/sheets/getting-started/installation)。
</Callout>
...
下面例子展示通过 [`FWorkbook.create`](https://reference.univer.ai/en-US/classes/FWorkbook#create) 方法创建一个工作表。
...
参考：[权限控制](/en-US/guides/sheets/features/core/permission)

### 公式

参考：[公式](/en-US/guides/sheets/features/core/formula)

### 行列操作

参考：[行列操作](/en-US/guides/sheets/features/core/row-col)

### 范围Range

参考：[范围 Range](/en-US/guides/sheets/features/core/range-selection#range)

### 选区 Selection

参考：[选区 Selection](/en-US/guides/sheets/features/core/range-selection#selection)

### 单元格 Cell

参考：[单元格 Cell](/en-US/guides/sheets/features/core/range-selection#cell)

### 冻结

参考：[冻结](/en-US/guides/sheets/features/core/freeze)

### 集成自定义组件

参考：[集成自定义组件](/en-US/guides/sheets/features/core/dom)
