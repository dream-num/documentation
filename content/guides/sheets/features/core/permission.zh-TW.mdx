...
<Callout type="warning" title="使用前須知">
  Univer 提供的是可擴充的基礎能力，而不是完整的權限系統。
  如果你需要權限持久化、組織架構、使用者資訊等客製化能力，需要自行實現權限規則儲存與組織架構接入，通常透過自訂外掛來完成。
...
<Tabs items={['使用 Facade API', '使用命令系統']}>
  ...
  <Tab>
    ...
    const permissionService = accessor.get(IPermissionService)
    // 這裡 RangeProtectionPermissionEditPoint 的第三個參數就是上面生成的 permissionId，false 表示不可編輯
...
你也可以擴充修改別的權限點位來實現對不同功能的控制，具體點位列表請參考文章底部。
...
<Callout type="warning" title="注意事項">
  已經開始接入自訂權限，可以直接操作更細粒度的權限點位，建議不要和 Permission Facade API 混用。
</Callout>
...
完成自訂人員的設定後，請將其同步到 `sheetPermissionUserManagerService` 服務中的 `_selectUserList`，以便後續使用。
...
<Callout type="warning" title="注意事項">
  ...
  使用自訂權限服務時，請參考上方的“第三方權限服務接入”章節自行實現協作者管理邏輯。
</Callout>
