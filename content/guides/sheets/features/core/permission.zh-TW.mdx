...
<Callout type="info" title="權限點位說明">
  本機非持久化（非協同編輯環境）權限控制可直接透過修改權限點位實現，持久化權限請參考下方的“第三方權限服務接入”。
...
<Callout>
  協作者管理由業務應用程式和權限服務負責。請接入自己的使用者與 ACL 模型，在伺服器端分別檢查讀取、JOIN 和修改權限，見[身分與權限](/zh-CN/server/collaboration/identity-and-authorization)。下方前端 API 需要匹配的權限服務實作，不會自動設定伺服器端權限。
</Callout>
...
- `FWorksheetPermission.canView(): boolean`：檢查工作表是否可檢視
- `FWorksheetPermission.canViewCell(row: number, col: number): boolean`：檢查指定單元格是否可檢視
...
// 或透過 `FRangePermission.unprotect()` 删除保護規則
...
以下是 `FRangeProtectionRule` 一些成員方法：

| 方法 | 描述 |
| --- | --- |
| `id: string` | 保護規則的唯一標識符 |
| `permissionId: string` | 保護規則關聯的權限 ID |
| `ranges: FRange[]` | 保護規則作用的範圍列表 |
| `options: IRangeProtectionOptions` | 保護規則的選項配置 |
| `updateRanges(ranges: FRange[]): Promise<boolean>` | 更新保護規則的範圍列表 |
| `remove(): Promise<boolean>` | 刪除保護規則 |
| `setPoint(point: RangePermissionPoint, value: boolean): Promise<void>` | 設定保護規則的權限點位值 |
| `getPoint(point: RangePermissionPoint): boolean` | 獲取保護規則的權限點位值 |
| `canEdit(): boolean` | 檢查保護規則是否允許編輯 |
| `canView(): boolean` | 檢查保護規則是否允許檢視 |
| `canDelete(): boolean` | 檢查保護規則是否允許刪除 |
| `canManageCollaborator(): boolean` | 檢查保護規則是否允許管理協作者 |
| `getSnapshot(): RangePermissionSnapshot` | 獲取保護規則的快照資訊 |
...
<MigrationTable headers={['API 方法描述', '遷移指南']}>
  <MigrationRow method="取得權限例項">
    ...
    <MigrationCell>
      <strong>新 API：</strong>
      <br />
      <div>工作簿：<code>FWorkbook.getWorkbookPermission()</code></div>
 <div>工作表：<code>FWorksheet.getWorksheetPermission()</code></div>
 <div>選區：<code>FRange.getRangePermission()</code></div>
    </MigrationCell>
  </MigrationRow>
  <MigrationRow method="設定工作簿權限點位">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
  ...
  <MigrationRow method="獲取工作簿權限點位狀態">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
  ...
  <MigrationRow method="獲取工作表權限點位狀態">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
  ...
  <MigrationRow method="獲取指定單元格的權限除錯資訊">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
  ...
  <MigrationRow method="設定範圍保護">
    ...
    <MigrationCell>
      <strong>新 API：</strong>
      <br />
      <div>工作表：<code>FWorksheetPermission.protectRanges(configs)</code></div>
 <div>選區：<code>FRangePermission.protect(options?: IRangeProtectionOptions)</code></div>
    </MigrationCell>
  </MigrationRow>
  <MigrationRow method="刪除範圍保護">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
    ...
    <MigrationCell>
      <strong>新 API：</strong>
      <br />
      <div>工作表：<code>FWorksheetPermission.unprotectRules(ruleIds: string[])</code></div>
 <div>選區：<code>FRangePermission.unprotect()</code></div>
 <div>范围保护规则：<code>FRangeProtectionRule.remove()</code></div>
    </MigrationCell>
  </MigrationRow>
  <MigrationRow method="設定選區權限點位">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
  ...
  <MigrationRow method="更新保護規則的範圍列表">
    <MigrationCell className="mb-2.5">
      <strong>舊 API：</strong>
      <br />
