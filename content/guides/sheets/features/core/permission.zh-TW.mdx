...
<Callout>
  協作者管理由業務應用程式和權限服務負責。請接入自己的使用者與 ACL 模型，在伺服器端分別檢查讀取、JOIN 和修改權限，見[身分與權限](https://office.univer.ai/zh-CN/collaboration/identity-and-authorization)。下方前端 API 需要匹配的權限服務實作，不會自動設定伺服器端權限。
</Callout>
