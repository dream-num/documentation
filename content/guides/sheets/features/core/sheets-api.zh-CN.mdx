...
[`univerAPI.createWorkbook(data, options)`](https://docs.univer.ai/reference/facade/univer#createworkbook) 方法会创建并返回 `FWorkbook` 实例。
...
下面例子展示通过 [`FWorkbook.create`](https://docs.univer.ai/reference/facade/workbook#create) 方法创建一个工作表。
...
// 创建一个名为 'MyNewSheetWithData' 的新工作表，包含 10 行和 10 列，并带有一些数据，且将其设置为第一个工作表
const sheetData = {
  // ... 省略其他属性
  cellData: {
    0: {
