...
To get full definition of range related facade api, please refer to [FRange FacadeAPI](https://docs.univer.ai/reference/facade/range)
...
If you only want to set the value of the top-left cell in the range, you can use the [`FRange.setValueForCell`](https://docs.univer.ai/reference/facade/range#setvalueforcell) method:
...
- [`FRange.clear`](https://docs.univer.ai/reference/facade/range#clear) method can clear the content and format information of the range.
- [`FRange.clearContent`](https://docs.univer.ai/reference/facade/range#clearcontent) method can clear the content information of the range.
- [`FRange.clearFormat`](https://docs.univer.ai/reference/facade/range#clearformat) method can clear the format information of the range.
...
[`FRange.insertCells`](https://docs.univer.ai/reference/facade/range#insertcells) method can inserts empty cells into this range. Existing data in the sheet along the provided dimension is shifted away from the inserted range.
...
[`FRange.deleteCells`](https://docs.univer.ai/reference/facade/range#deletecells) method can deletes this range of cells. Existing data in the sheet along the provided dimension is shifted towards the deleted range.
...
[`FRange.highlight`](https://docs.univer.ai/reference/facade/range#highlight) method can highlight the range with the specified style and primary cell.
...
[`FRange.splitTextToColumns`](https://docs.univer.ai/reference/facade/range#splittexttocolumns) method can split the text in the range into columns accrording to the specified delimiter.
...
To get full definition of selection related facade api, please refer to [FSelection FacadeAPI](https://docs.univer.ai/reference/facade/selection)
...
When disabled, selection becomes unresponsive. `hideSelection` makes the selection invisible—unlike `disableSelection`, selection still works; you just cannot see it.
...
2. The Univer API plugin will also store the extended cell attributes in the `resources` property of the `Workbook`, please refer to [Plugin Custom Model](/blog/custom-model).
...
| `BeforeSheetEditStart` | Triggered before cell editing begins         | `IBeforeSheetEditStartEventParams` | `const { worksheet, workbook, row, column, eventType, keycode } = params`                   |
| `SheetEditStarted`     | Triggered when cell editing starts           | `ISheetEditStartedEventParams`     | `const { worksheet, workbook, row, column, eventType, keycode } = params`                   |
| `SheetEditChanging`    | Triggered when cell content is being changed | `ISheetEditChangingEventParams`    | `const { worksheet, workbook, row, column, value } = params`                                |
| `BeforeSheetEditEnd`   | Triggered before cell editing ends           | `IBeforeSheetEditEndEventParams`   | `const { worksheet, workbook, row, column, value, eventType, keycode, isConfirm } = params` |
| `SheetEditEnded`       | Triggered after cell editing ends            | `ISheetEditEndedEventParams`       | `const { worksheet, workbook, row, column, eventType, keycode, isConfirm } = params`        |
...
`registerCellCustomRender` installs custom renderers used during cell rendering.
