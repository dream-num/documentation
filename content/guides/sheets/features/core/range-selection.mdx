...
范围完整 Facade API 类型定义，请查看 [FRange FacadeAPI](https://reference.univer.ai/en-US/classes/FRange)
...
如果只想设置范围左上角的单元格值，可以使用 [`FRange.setValueForCell`](https://reference.univer.ai/en-US/classes/FRange#setvalueforcell) 方法：
```typescript
...
- [`FRange.clear`](https://reference.univer.ai/en-US/classes/FRange#clear) 方法可以清理范围的内容和格式信息。
- [`FRange.clearContent`](https://reference.univer.ai/en-US/classes/FRange#clearcontent) 方法可以清理范围的内容信息。
- [`FRange.clearFormat`](https://reference.univer.ai/en-US/classes/FRange#clearformat) 方法可以清理范围的格式信息。
...
[`FRange.insertCells`](https://reference.univer.ai/en-US/classes/FRange#insertcells) 方法可以在此范围内插入空单元格。工作表中沿指定维度的现有数据将移离插入的范围。
...
[`FRange.deleteCells`](https://reference.univer.ai/en-US/classes/FRange#deletecells) 方法可以在范围内删除单元格。工作表中沿指定维度的现有数据将向已删除的范围移动。
...
[`FRange.splitTextToColumns`](https://reference.univer.ai/en-US/classes/FRange#splittexttocolumns) 方法可以将单元格中的文本按指定分隔符分列。
...
选区完整 Facade API 类型定义，请查看 [FSelection FacadeAPI](https://reference.univer.ai/en-US/classes/FSelection)
...
<Callout type="info" emoji="ℹ️">
  对单元格的操作可以看作对行高 1 、列宽 1 的范围进行操作，操作范围请阅读 [范围-range](/en-US/guides/sheets/features/core/range-selection#range)。
</Callout>

<Callout type="info" emoji="ℹ️">
  插件还会将拓展的单元格属性存储在 `Workbook` 的 `resources` 属性中，详细请阅读 [插件自定义模型](/en-US/guides/recipes/practices/custom-model/)。
</Callout>
