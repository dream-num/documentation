...
2. Плагин Univer API также сохраняет расширенные атрибуты ячеек в свойстве `resources` объекта `Workbook`. Подробнее см. в разделе [Пользовательская модель плагина](/guides/recipes/tutorials/custom-model).
