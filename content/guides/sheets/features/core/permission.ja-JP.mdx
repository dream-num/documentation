...
<MigrationTable headers={['API メソッドの説明', '移行ガイド']}>
  <MigrationRow method="権限インスタンスの取得">
    ...
    <MigrationCell>
      ...
      <br />
      ワークブック：<code>FWorkbook.getWorkbookPermission()</code>
      <br />
      ワークシート：<code>FWorksheet.getWorksheetPermission()</code>
      <br />
      範囲：<code>FRange.getRangePermission()</code>
    </MigrationCell>
  </MigrationRow>
  ...
  <MigrationRow method="範囲保護の設定">
    ...
    <MigrationCell>
      <strong>新しい API：</strong>
      <br />
      ワークシート：<code>FWorksheetPermission.protectRanges(configs)</code>
      <br />
      範囲：<code>FRangePermission.protect(options?: IRangeProtectionOptions)</code>
    </MigrationCell>
  </MigrationRow>
  <MigrationRow method="範囲保護の解除">
    ...
    <MigrationCell>
      <strong>新しい API：</strong>
      <br />
      ワークシート：<code>FWorksheetPermission.unprotectRules(ruleIds: string[])</code>
      <br />
      範囲：<code>FRangePermission.unprotect()</code>
      <br />
      範囲権限ルール：<code>FRangeProtectionRule.remove()</code>
    </MigrationCell>
  </MigrationRow>
