...
| univerAPI.Enum.WorksheetPermissionPoint.EditExtraObject | WorksheetEditExtraObjectPermission | 追加オブジェクト編集可能 |
| univerAPI.Enum.WorksheetPermissionPoint.SelectProtectedCells | WorksheetSelectProtectedCellsPermission | 保護されたセル選択可能 |
| univerAPI.Enum.WorksheetPermissionPoint.SelectUnProtectedCells | WorksheetSelectUnProtectedCellsPermission | 保護されていないセル選択可能 |
...
| univerAPI.Enum.RangePermissionPoint.ManageCollaborator | RangeProtectionPermissionManageCollaPoint | 保護範囲の共同編集者管理可能 |

## 設定

### 権限保護範囲シャドウ戦略

現在のユーザーが編集できる保護領域と編集できない保護領域を区別するには、`non-editable` を使用します。編集可能な保護領域のシャドウは非表示になり、編集権限が無効な保護領域にはシャドウが残ります。閲覧できない保護範囲のマスクも維持されます。

この設定は範囲保護とワークシート保護の表示を制御します。保護ルールの作成や編集・閲覧権限の変更は行わず、保護されていない領域にシャドウを追加することもありません。先に上記の手順で保護ルールと現在のユーザーの権限を設定してください。

| 戦略 | 表示動作 |
| --- | --- |
| `true` / `'always'` | すべての保護領域にシャドウを表示します（デフォルト）。 |
| `'non-editable'` | 編集可能な保護領域のシャドウを非表示にし、編集不可の領域では維持します。 |
| `'non-viewable'` | 閲覧できない保護領域にのみシャドウを表示します。 |
| `false` / `'none'` | すべての保護シャドウを非表示にします。権限チェックは引き続き有効です。 |
...
以下の Facade API は、現在の Univer インスタンス内の既存のワークブックを更新します。プラグインモードでは、事前に `@univerjs/sheets-ui/facade` をインポートしてください。Sheets Core プリセットにはこのインポートが含まれています。
...
<Callout>
  共同編集者の管理はアプリのユーザー・ACL モデルと権限サービスで実装します。[Identity and authorization](/server/collaboration/identity-and-authorization) に沿って読み取り、JOIN、編集を保護してください。以下のフロントエンド API には対応する権限サービスの実装が必要です。
</Callout>
...
| `canManageCollaborator(): boolean` | チェック保護ルールが共同編集者管理を許可するかどうか |
