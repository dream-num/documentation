...
| univerAPI.Enum.WorkbookPermissionPoint.ViewHistory | WorksheetViewHistoryPermission | 履歴閲覧可能 |
| univerAPI.Enum.WorkbookPermissionPoint.ManageHistory | WorkbookHistoryPermission | 履歴管理可能 |
| univerAPI.Enum.WorkbookPermissionPoint.RecoverHistory | WorksheetRecoverHistoryPermission | 履歴復元可能 |
...
// すべての範囲権限ポイントステータスを取得
const snapshot = permission.getSnapshot()
```

ショートカットメソッドの利用例：

- `FRangePermission.isProtected(): boolean`：範囲が保護されているか確認
- `FRangePermission.canEdit(): boolean`：範囲が編集可能か確認
- `FRangePermission.canView(): boolean`：範囲が閲覧可能か確認
- `FRangePermission.canDelete(): boolean`：選択範囲が保護削除可能か確認
