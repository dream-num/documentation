...
<Callout>
  共同編集者の管理はアプリのユーザー・ACL モデルと権限サービスで実装します。[Identity and authorization](https://office.univer.ai/collaboration/identity-and-authorization) に沿って読み取り、JOIN、編集を保護してください。以下のフロントエンド API には対応する権限サービスの実装が必要です。
</Callout>
