...
[`univerAPI.createWorkbook(data, options)`](https://docs.univer.ai/reference/facade/univer#createworkbook) メソッドは `FWorkbook` オブジェクトを作成して返します。
...
以下の例は [`FWorkbook.create`](https://docs.univer.ai/reference/facade/workbook#create) メソッドを使用してワークシートを作成する方法を示しています。
...
// 10 行 10 列の 'MyNewSheetWithData' という名前の新しいシートを作成し、いくつかのデータを含め、最初のシートとして設定します
const sheetData = {
  // ... その他のプロパティは省略
  cellData: {
    0: {
...
参考：[カスタムコンポーネントの Univer への統合](/guides/sheets/ui/components)
