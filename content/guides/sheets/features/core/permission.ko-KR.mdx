...
현재 사용자가 편집할 수 있는 보호 영역과 편집할 수 없는 보호 영역을 구분하려면 `non-editable`을 사용하세요. 편집 가능한 보호 영역의 음영은 숨겨지고, 편집 권한이 비활성화된 보호 영역의 음영은 유지됩니다. 볼 수 없는 보호 범위의 마스크도 유지됩니다.

이 설정은 범위 보호와 워크시트 보호의 표시를 제어합니다. 보호 규칙을 만들거나 편집·보기 권한을 변경하지 않으며, 보호되지 않은 영역에 음영을 추가하지도 않습니다. 먼저 위의 설명에 따라 보호 규칙과 현재 사용자의 권한을 설정하세요.

| 전략 | 표시 동작 |
| --- | --- |
| `true` / `'always'` | 모든 보호 영역에 음영을 표시합니다(기본값). |
| `'non-editable'` | 편집 가능한 보호 영역의 음영을 숨기고 편집 불가능한 영역의 음영은 유지합니다. |
| `'non-viewable'` | 볼 수 없는 보호 영역에만 음영을 표시합니다. |
| `false` / `'none'` | 모든 보호 음영을 숨깁니다. 권한 검사는 계속 적용됩니다. |

<PermissionShadowDemo />

초기화 시 설정하세요:

<Tabs items={['플러그인 모드', '프리셋 모드']}>
  <Tab>
    ```typescript
...
아래 Facade API는 현재 Univer 인스턴스의 기존 워크북에 표시 전략을 적용합니다. 플러그인 모드에서는 먼저 `@univerjs/sheets-ui/facade`를 가져오세요. Sheets Core 프리셋에는 이 가져오기가 포함되어 있습니다.
...
<Callout>
  사용자 정의 컴포넌트를 만드는 방법은 [사용자 지정 컴포넌트](/guides/sheets/ui/components)를 참고하세요.
</Callout>
...
<Callout>
  공동 작업자 관리는 앱의 사용자·ACL 모델과 권한 서비스에서 구현합니다. [Identity and authorization](/server/collaboration/identity-and-authorization)에 따라 읽기, JOIN, 수정을 보호하세요. 아래 프런트엔드 API에는 대응하는 권한 서비스 구현이 필요합니다.
</Callout>
