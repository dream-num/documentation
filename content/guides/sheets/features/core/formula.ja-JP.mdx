    ...
    plugins: [
      {
        ...
        locale: '@univerjs/engine-formula/locale/ja-JP',
      },
      {
        client: '@univerjs/sheets-formula',
        facade: '@univerjs/sheets-formula/facade',
        locale: '@univerjs/sheets-formula/locale/ja-JP',
      },
      {
...
数式では、読み込まれたブックを表示名で参照したり、ブック内のシート順に従って両端を含む連続範囲を参照したりできます。
...
ブック名は大文字と小文字を区別せず、フォールバック時は一般的な Excel 拡張子を無視して照合されます。名前が見つからない場合や曖昧な場合は `#REF!` を返します。
