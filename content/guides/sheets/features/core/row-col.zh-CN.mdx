...
[`deleteRowsByPoints(rowPoints)`](https://reference.univer.ai/zh-CN/classes/FWorksheet#deleterowsbypoints): 删除由行位置数组指定的多行（不连续）。
...
[`deleteColumnsByPoints(columnPoints)`](https://reference.univer.ai/zh-CN/classes/FWorksheet#deletecolumnsbypoints): 删除由列位置数组指定的多列（不连续）。
