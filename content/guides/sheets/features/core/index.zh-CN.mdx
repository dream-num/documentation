...
参考：[`Univer`](https://docs.univer.ai/reference/classes/univer)

#### @univerjs/engine-render

本插件是 Univer 的渲染引擎，负责渲染、滚动条、光标和选区等的绘制。其提供了 `UniverRenderEnginePlugin` 插件。

参考：[@univerjs/engine-render](https://docs.univer.ai/reference/packages/plugins/univerjs/engine-render)

#### @univerjs/engine-formula

本插件是 Univer 的公式引擎，负责公式的计算和处理。其提供了 `UniverFormulaEnginePlugin` 插件。

参考：[@univerjs/engine-formula](https://docs.univer.ai/reference/packages/plugins/univerjs/engine-formula)
...
interface IUniverUIConfig {
  ...
  // 是否显示头部工具栏
  toolbar?: boolean
  // 头部工具栏的类型。'classic': 默认经典模式；'grid': 两行网格模式；'collapsed': 紧凑模式；'simple'：不分组模式
...
参考：[@univerjs/ui](https://docs.univer.ai/reference/packages/plugins/univerjs/ui)

#### @univerjs/docs

本插件是 Univer Docs 的核心插件。其提供了 `UniverDocsPlugin` 插件。

参考：[@univerjs/docs](https://docs.univer.ai/reference/packages/plugins/univerjs/docs)

#### @univerjs/docs-ui

本插件是 Univer Docs 的 UI 插件，包含了文档编辑器的 UI 组件和交互。其提供了 `UniverDocsUIPlugin` 插件。

参考：[@univerjs/docs-ui](https://docs.univer.ai/reference/packages/plugins/univerjs/docs-ui)
...
参考：[@univerjs/sheets](https://docs.univer.ai/reference/packages/plugins/univerjs/sheets)
...
参考：[@univerjs/sheets-ui](https://docs.univer.ai/reference/packages/plugins/univerjs/sheets-ui)
...
参考：[@univerjs/preset-sheets-core](https://docs.univer.ai/reference/packages/presets/univerjs/preset-sheets-core)
...
初始化配置和运行时的 [`univerAPI.updateMenuConfig()`](/reference/facade/univer#updatemenuconfig) 使用同一份 `menu` 结构。键可以是 Ribbon 页签标识（如 `ribbon.insert`）、分组标识（如 `ribbon.start.format`）或菜单项 ID。

`order` 按数值从小到大排列同级节点：Ribbon 中的页签、页签内的分组，以及分组或子菜单内的菜单项。它不会把菜单移到其他分组。页签和分组也可以使用 `@univerjs/ui` 导出的 `RibbonPosition` 及各分组枚举作为键。

### 初始化配置

在 `createUniver()` 的 `presets` 数组中，将配置传给 `UniverSheetsCorePreset`：
...
插件模式下，将同一份 `menu` 和 `ribbonType` 传给已有的 `UniverUIPlugin` 注册配置。

### 运行时更新

公开 Facade 会合并修改并立即刷新 UI。未指定的属性保留已有配置或 Schema 默认值，配置也会应用到后续注册的菜单。插件模式下，需要先导入 `@univerjs/ui/facade`。
...
`gridLayout` 仅在 `ribbonType: 'grid'` 时生效。每个分组固定两行，`row` 和 `column` 均从 1 开始。显式坐标决定按钮位置，因此只修改 `order` 不会移动已指定坐标的按钮；交换位置时应同时修改两个按钮，避免重叠。可选字段包括 `rowSpan`、`columnSpan`、`showLabel`、`width` 和 `iconSize`，尺寸单位为像素。约束和默认值见 [`IRibbonGridLayout`](/reference/packages/plugins/univerjs/ui#iribbongridlayout)。

### 自定义菜单的位置

`createMenu()` 和 `createSubmenu()` 也支持 `order` 与 `gridLayout`。对子菜单来说，这两个字段控制 Ribbon 上的触发入口，不改变弹出菜单内部布局。例如，在历史操作分组的第三列添加按钮：
...
<Callout>你可以通过菜单项 DOM 元素上的 `data-u-command` 属性来获取菜单项的 ID。</Callout>
