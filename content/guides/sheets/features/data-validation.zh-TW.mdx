...
<MetaData
  ...
  meta={{
    ...
    plugins: [{
      ...
      locale: '@univerjs/data-validation/locale/zh-TW',
    }, {
      client: '@univerjs/sheets-data-validation',
      facade: '@univerjs/sheets-data-validation/facade',
      locale: '@univerjs/sheets-data-validation/locale/zh-TW',
    }, {
...
完整的配置項參考 [`IUniverSheetsDataValidationUIConfig`](https://github.com/dream-num/univer/blob/dev/packages/sheets-data-validation-ui/src/config/config.ts)。
...
完整的配置項參考 [`IUniverSheetsDataValidationPresetConfig`](https://github.com/dream-num/univer/tree/dev/presets/packages/preset-sheets-data-validation)。
...
[`univerAPI.newDataValidation()`](https://docs.univer.ai/reference/facade/univer#newdatavalidation) 可以建立一個新的資料驗證構建器，返回一個 `FDataValidationBuilder` 例項，可以透過鏈式呼叫生成資料驗證規則。

以下是 [`FDataValidationBuilder`](/reference/facade/data-validation-builder) 上的一些成員方法：
...
| requireValueInList | 設定資料驗證規則，要求使用者從特定值列表中輸入一個值。支援單選/多選，支援顯示/隱藏下拉框 |
| requireValueInRange | 設定資料驗證規則，要求使用者輸入特定範圍內的值。支援單選/多選，支援顯示/隱藏下拉框 |
...
以下是 [`FDataValidation`](https://docs.univer.ai/reference/facade/data-validation) 上的一些成員方法：

| 方法 | 描述 |
| ---- | ---- |
| copy | 使用當前規則物件建立一個新的 FDataValidationBuilder 實例 |
...
校驗狀態是 `DataValidationStatus` 的列舉值，包括：
- `valid`：儲存格值通過驗證
- `invalid`：儲存格值未通過驗證
- `validating`：儲存格正在驗證中
...
`errorStyle` 選項控制輸入無效資料時的行為。它是 `DataValidationErrorStyle` 的列舉值，包括：
- `STOP`：阻止使用者輸入無效資料
- `WARNING`：允許使用者輸入無效資料，但顯示警告
- `INFO`：允許使用者輸入無效資料，但顯示資訊提示
