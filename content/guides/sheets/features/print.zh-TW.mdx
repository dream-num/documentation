...
如果你擁有 Univer 的商業許可證，請參考[在客戶端使用許可證](/guides/server-integration/license#in-preset-mode)進行配置。
...
如果你擁有 Univer 的商業許可證，請參考[在客戶端使用許可證](/guides/server-integration/license#in-plugin-mode)進行配置。
...
該 API 僅在擁有 [許可證](/guides/server-integration/license) 的情況下可用，未持有許可證時將受到使用限制，儲存操作將返回 `false`。
...
該 API 僅在持有 [許可證](/guides/server-integration/license) 時可用，未持證使用者將受到使用限制，儲存失敗時返回 `false`，成功時返回圖片的 base64 字串。
