...
<MetaData
  ...
  meta={{
    ...
    preset: [{
      ...
      locale: '@univerjs/preset-sheets-drawing/locales/ja-JP',
...
### プリセットと設定 [#presets-and-configuration]
...
[`FWorksheet.newOverGridImage()`](https://docs.univer.ai/reference/facade/worksheet#newovergridimage) は浮動画像ビルダーを作成し、`FOverGridImageBuilder` のインスタンスを返します。チェーン呼び出しで `ISheetImage` オブジェクトを生成して画像を挿入できます。

以下は [`FOverGridImageBuilder`](/reference/facade/over-grid-image-builder) の主なメソッドです:
...
`FWorksheet.getImages()` メソッドはワークシート内のすべての浮動画像を取得し、[`FOverGridImage[]`](https://docs.univer.ai/reference/facade/chart) の配列を返します。
...
## ワークシートの背景画像 [#worksheet-background]

ワークシートはセルの背面に 1 枚の画像をタイル表示できます。浮動画像やセル画像とは独立し、`IWorksheetData.backgroundImage` に保存されます。Drawing プリセットに含まれます。プラグイン方式では `@univerjs/sheets-drawing` と Facade、および描画・アップロード用の Drawing UI が必要です。
...
`setBackgroundImage()` は既定で `ImageSourceType.URL` を使い、既存の背景を置き換えます。`deleteBackgroundImage()` は背景を削除します。どちらも元に戻す・やり直す処理に対応します。UI のアップロードは設定された画像 I/O サービスを使用します。背景は画面表示用で、印刷されません。
