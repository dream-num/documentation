...
在 Univer Sheets 中，您可以使用 Facade API 來建立、配置和管理圖表。Facade API 是一種圖表的程式介面，可以幫助您更方便地使用圖表功能。

### 插入圖表

[`FWorksheet.newChart()`](https://reference.univer.ai/zh-TW/classes/FWorksheet#newchart) 方法用於建立一個圖表建構器，回傳一個 `FChartBuilderBase` 實例，您可以透過鏈式呼叫方法來設定圖表的各種屬性。
...
目前我們支援以下的配置項，您可以透過查閱 IChartBuildOptions 來獲取對應的配置：
...
Univer chart 是基於 echarts 的圖表庫實作的，因此可以借助 echart 的主題建構工具來建構您的[自訂主題](https://echarts.apache.org/zh/theme-builder.html)。
您可以在該網站建構您自己的主題並下載成為配置檔案，使用以下 API 來使用您的自訂主題：
