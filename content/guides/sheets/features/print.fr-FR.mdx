...
<Callout>
  À l’exécution, le préréglage `UniverSheetsAdvancedPreset` de `@univerjs/preset-sheets-advanced` dépend du préréglage `UniverSheetsDrawingPreset`. Installez d’abord `@univerjs/preset-sheets-drawing`.
</Callout>
...
Cette fonctionnalité est uniquement disponible avec une [licence](en-US/guides/pro/license). Sans licence, son utilisation est restreinte et l’opération d’enregistrement renvoie `false`.
...
Cette fonctionnalité est uniquement disponible avec une [licence](en-US/guides/pro/license). Sans licence, son utilisation est restreinte. L’opération d’enregistrement renvoie `false` en cas d’échec, ou une chaîne base64 contenant l’image en cas de réussite.
