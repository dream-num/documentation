...
<Callout type="warning">
  透明度を伴う色値（例: rgba）の使用はできるだけ避けてください。そうしないと、編集状態を有効にしたときに、背景が下のセルの内容を透過してしまいます。
</Callout>
...
[0 で始まる数値や日付フォーマットに変換できる数値を入力した際に、セルがデフォルトで変換されないようにしたい](/guides/sheets/features/core/numfmt#usage)
...
`disableForceStringAlert` と `disableForceStringMark` を設定して、強制テキストの警告とマークを無効にできます。設定項目の場所は以下の通りです。
  - [プリセットモード](/guides/sheets/features/core#univerjspreset-sheets-core)
  - [プラグインモード](/guides/sheets/features/core#univerjssheets-ui)
