---
title: Facade API
description: Univer SDK の使用を簡素化するために Facade API をアプリケーションに追加する方法について学びます。
icon: PlugZap
---
...
<Callout type="warning" title="注意事項">
  すべてのプラグインにfacade パッケージが含まれているわけではありません。各機能のドキュメントでこれを明記します。
</Callout>

### 使い方

Facade API は Univer インスタンスのラッパーであるため、Univer インスタンスを作成した後に `FUniver` を使用してそのインスタンスをラップする必要があります：
...
- [**Facade API リファレンス全体**](https://reference.univer.ai/)
