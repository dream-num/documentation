...
1. [Write a CSV import plugin](/en-US/guides/recipes/practices/csv-import-plugin) to learn how to write a plugin.
2. [Univer Architecture](/en-US/guides/recipes/architecture/univer) to learn more about Univer's architecture design.
...
1. The snapshot format of a spreadsheet is [`IWorkbookData`](/en-US/guides/sheets/model/workbook-data#iworkbookdata).
2. The snapshot format of a document is [`IDocumentData`](https://reference.univer.ai/en-US/interfaces/IDocumentData).

Functional plugins can store resources, such as images, etc., through the `resources` field. For more information, see [Plugin Custom Model](/en-US/guides/recipes/practices/custom-model).
