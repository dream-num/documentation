...
Univer 具備同構特性，不僅可在瀏覽器端執行，也能在 Node.js 環境下使用。你可以基於 Headless Univer 開發資料處理服務，在伺服器端產生或修改文件，並透過 Facade API 與 Univer Sheets 進行互動。

<PlaygroundFrame lang="zh-TW" slug="sheets/node-via-plugin" clickToShow />

Headless Univer 與瀏覽器端的使用方式相似，在預設模式下我們提供開箱即用的預設套件；而在插件模式下，只需排除與 UI 有關的外掛即可。
