...
以下の例では、Univer における Worker[^web-worker]（Web Worker）機能の基本的な利用方法を示します。
...
[^web-worker]: Web Worker はブラウザーのメインスレッド外で JavaScript を実行し、メッセージで通信します。ページの DOM には直接アクセスできません。計算を Worker に移しても、エディター UI は移動しません。
