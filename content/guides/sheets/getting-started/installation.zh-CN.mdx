...
<Playground lang="zh-CN" {...slimViaPreset} options={{ clickToShow: true }} />
...
<Callout>
  你可以在 [API Reference / createUniver](/zh-CN/reference/methods/create-univer) 找到更多关于 `createUniver` 方法的详细信息。
</Callout>
...
<Playground lang="zh-CN" {...slimViaPlugin} options={{ clickToShow: true }} />
...
在前面[预设模式的命名空间规范](/zh-CN/guides/docs/getting-started/installation#命名空间规范)中，我们已经介绍了 Univer 的命名空间规范。这里我们将继续使用相同的命名空间规范来引入插件。
...
* 在 [基础概念](/zh-CN/guides/sheets/getting-started/quickstart#basic-concepts) 章节了解本章中出现的插件、命令和 Facade API 等概念。
* 在 [功能](/zh-CN/guides/sheets/features/core) 相关章节中了解修改电子表格数据的所有操作。
