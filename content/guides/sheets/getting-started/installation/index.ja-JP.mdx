...
<Callout>
  `createUniver` メソッドの詳細情報については、[API リファレンス / createUniver(https://docs.univer.ai/reference/methods/create-univer) で確認できます。
</Callout>
...
<Callout>
  ...
  - yarn を使用している場合は、不足している `peerDependencies` を手動でインストールする必要があります[^3]。以下のインストールコマンドにはすでにこれらの依存関係が含まれています。
</Callout>
