...
<Callout type="info">
  モバイル用とデスクトップ用のプラグインは、完全に別のパッケージではなく、同一パッケージがプラットフォームごとに異なる形で提供されるものです。たとえば
  `@univerjs/sheets-ui` は、デスクトップ用の `UniverSheetsUIPlugin` とモバイル用の `UniverSheetsMobileUIPlugin`
  の両方を提供しています。
</Callout>

<Callout type="warning" title="注意事項">
  - モバイル対応は現在実験段階であり、一部機能が不完全な場合があります。 - モバイル UI プラグインとデスクトップ UI
  プラグインは同時に使用できません。端末種別に応じて適切なプラグインを選択してください。 -
