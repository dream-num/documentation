...
我們已在主流 CDN 服務（例如 [jsDelivr](https://www.jsdelivr.com)、[unpkg](https://unpkg.com)）提供 Univer 的 UMD 版本。你可以於 HTML 檔案中直接引入這些資源（當然也可以先下載後，改由自有伺服器或自建 CDN 佈署）：

<Callout type="warning" title="注意事項">
  透過 `<script>` 標籤引入 Univer 的 UMD 套件，使用方式與透過套件管理工具 + 建置工具並無太大差異。但需特別注意：雖然「預設包」本質上是一組外掛的集合，當你想取得某個特定外掛的物件或方法時，必須從該外掛本身的命名空間存取，而不是從其所被包含的預設包命名空間取得。

  例如：即使 `@univerjs/core` 已包含於 `@univerjs/presets` 之中，你仍需自 `@univerjs/core` 的命名空間 `UniverCore` 取得所需內容。
</Callout>
...
與預設模式類似，Univer 也為每個外掛提供 UMD 套件；不同的是，你需要手動引入每一個外掛及其前置相依。這個流程相當繁瑣，我們**不建議**在未充分理解外掛機制（如外掛間依賴關係與樣式引入順序）前採用此方式。
