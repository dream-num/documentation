...
La version globale d’Univer (UMD) expose un espace de noms sur l’objet global, selon la convention de nommage suivante :
...
Univer fournit des versions UMD pour ses plugins. Lorsque vous utilisez des balises de script CDN, vous devez inclure manuellement les versions UMD de chaque plugin ainsi que leurs dépendances. Ce processus étant fastidieux, nous vous recommandons de ne l’utiliser qu’après avoir bien compris le fonctionnement des plugins, notamment leurs dépendances et l’ordre des fichiers de styles.
...
unpkg et jsDelivr permettent tous deux d’indiquer une version précise de la ressource. Par exemple, pour utiliser la version `x.y.z` d’Univer, ajoutez simplement `@<version>` à l’URL :
