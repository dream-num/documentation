...
Si no utiliza un administrador de paquetes, también puede importar Univer a través de etiquetas `<script>`.

#### Espacio de nombres Convención

La compilación global (UMD) de Univer expone un espacio de nombres en el objeto global, siguiendo la convención de nomenclatura siguiente:
...
## Modo complemento [#plugin-mode]

### Uso de compilación global

Univer proporciona compilaciones UMD para complementos. Cuando utilice etiquetas de script CDN, deberá incluir manualmente las compilaciones UMD de cada complemento y sus dependencias. Este proceso es engorroso, por lo que recomendamos usarlo solo después de comprender completamente el mecanismo del complemento, incluidas las dependencias del complemento y el orden de los archivos de estilo.
...
unpkg y jsDeliver admiten la especificación de una versión específica del recurso. Por ejemplo, si desea utilizar la versión `x.y.z` de Univer, simplemente puede agregar `@<version>` a la URL:
