...
<Callout>
  Если вы хотите использовать компоненты Vue в качестве пользовательских компонентов, обратитесь к [этому разделу](/guides/sheets/ui/components#vue).
</Callout>

Перед интеграцией Univer в проект Vue прочитайте раздел [установки и базовой настройки](/guides/sheets/getting-started/installation) и ознакомьтесь с компонентами Vue и хуками их жизненного цикла.

## Vue 3.x

### Перед интеграцией

- Не используйте прокси для экземпляров Univer и FUniver: это может привести к непредсказуемым ошибкам или проблемам с производительностью.
...
- При установке `@univerjs/presets` с помощью pnpm может потребоваться установить `react` и `react-dom` вручную.
...
- Если в качестве инструмента сборки используется Vue CLI 3.x/4.x (на базе Webpack 4.x), необходимо вручную заменить сопоставленные пути фактическими:
