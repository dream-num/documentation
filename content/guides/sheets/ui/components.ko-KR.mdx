---
title: 사용자 지정 컴포넌트
description: Web SDK는 사용자 지정 컴포넌트를 통합하는 여러 방법을 제공하므로 기능을 확장하고 원하는 대로 조정할 수 있습니다. 이 가이드에서는 자주 사용하는 몇 가지 방법을 소개합니다.
icon: Component
---
...
`@angular/elements`의 `createCustomElement`로 Angular 컴포넌트를 사용자 정의 요소로 변환한 후 Univer의 Web Component 어댑터에 등록하세요. Angular 컴포넌트를 Univer 사이드바나 대화 상자에서 사용할 수 있습니다.

`@angular/core`와 같은 버전의 `@angular/elements`, 다른 Univer 패키지와 같은 버전의 `@univerjs/ui-adapter-web-component`를 추가하세요.

Univer 초기화 후 아래 함수를 호출하세요. 호스트 컴포넌트에서 `private readonly injector = inject(Injector)`로 Angular 주입기를 가져와 `registerAngularPanel(univer, univerAPI, this.injector)`에 전달합니다. 호스트의 `inject`와 `Injector`는 `@angular/core`에서 가져오세요.
...
편집기 렌더링 후 등록한 키를 사이드바 콘텐츠로 사용하세요.
...
반환된 등록 핸들과 사이드바 핸들을 보관하고 더 이상 필요하지 않으면 각각 `dispose()`를 호출하세요. 사용자 정의 요소는 애플리케이션에서 한 번만 등록합니다. 이름에 하이픈이 있어야 하며 Angular 컴포넌트의 selector와 달라야 합니다.

Angular Elements는 입력을 요소 속성으로, 출력을 DOM `CustomEvent`로 변환합니다. Univer 어댑터는 속성을 전달하지만 Angular 출력을 자동 구독하지 않습니다. 필요한 이벤트를 명시적으로 연결하세요.
