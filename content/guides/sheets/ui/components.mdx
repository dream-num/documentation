...
Convert an Angular component into a custom element with `createCustomElement` from `@angular/elements`, then register it with Univer’s Web Component adapter. This lets an Angular component appear in a Univer sidebar or dialog.

Add `@angular/elements` at the same version as `@angular/core`, and add `@univerjs/ui-adapter-web-component` at the same version as the other Univer packages.

Call this function after initializing Univer. Pass the Angular injector from the host component: obtain it with `private readonly injector = inject(Injector)` and call `registerAngularPanel(univer, univerAPI, this.injector)`. Import `inject` and `Injector` from `@angular/core` in that host component.
...
After the editor has rendered, use the registered key as the sidebar content:
...
Keep the returned registration and sidebar handles, and call their `dispose()` methods when they are no longer needed. Register the element once per application: its name must contain a hyphen and differ from the Angular component selector.

Angular Elements maps inputs to element properties and outputs to DOM `CustomEvent` events. Univer’s adapter assigns properties but does not automatically subscribe to Angular outputs; bridge those events explicitly when needed.
