...
The current `@univerjs-pro/bases-ui` package does not export a dedicated mobile UI plugin. Do not invent a `MobileUIPlugin` import or a `mobile: true` option. A narrow container is not evidence of complete touch support; evaluate the existing UI on the target devices.
