...
<Steps>
  ...
  <Step>
    如需使用 Facade API[^facade-entry]，请导入 Facade 入口：
...
当前 `@univerjs-pro/bases-ui` 没有导出独立的移动端 UI 插件。不要使用不存在的 `MobileUIPlugin` 导入或 `mobile: true` 选项。容器变窄不代表已完整支持触摸交互，需要在目标设备上验证现有 UI。
