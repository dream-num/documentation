...
这些 API 需要转换后端。请先阅读[导入导出集成](/guides/server-integration/import-export)，并确认后端支持 Bases 文档类型。前端插件安装成功，不代表服务端已经支持对应格式。
...
`exchangeConfig` 是业务后端的上传、转换、任务查询和下载配置，按[服务端集成指南](/guides/server-integration/import-export)准备。国际化还需将两个包的 `locale/zh-CN` 合并到 `locales`，方法见[国际化](/guides/bases/getting-started/i18n)。
