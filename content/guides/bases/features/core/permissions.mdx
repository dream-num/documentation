...
`setReadOnly()` and `setEditable()` return promises and require `await`. Use `base.getPermission().setObjectPermissions()` for batched changes; see [Bases Facade](/reference/facade/base) for parameters.
