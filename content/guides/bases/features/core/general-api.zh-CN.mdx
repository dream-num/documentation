...
Univer 可用的 Facade API 取决于当前单元类型和已注册的插件。本文介绍 Univer Bases 应用会用到的通用 API。
...
如果已经知道命令 ID 和参数，可以通过 `FUniver.executeCommand` 执行命令。例如，下面的 Bases 命令会重命名当前Base：
...
`univerAPI.Event` 暴露的事件来自 Univer Core 和当前应用已注册的插件。Core 提供通用的命令和单元事件；Bases 插件会补充表格、字段、记录、视图以及其他 Base 变更事件。使用事件前，请先确认提供该事件的插件已经注册。

当前版本可用的事件名称和参数类型请参阅 [Facade Events 参考](/reference/facade/events#available-event-names)。
...
注册 Bases UI 插件后，`copy` 和 `paste` 会作用于Base当前的选区和光标位置。
...
如需扩展菜单、工具栏和其他 Bases 界面区域，请参阅 [Bases UI 组件](/guides/bases/ui/components)。
