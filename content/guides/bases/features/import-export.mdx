...
These APIs need a conversion backend. Start with [file exchange integration](/guides/server-integration/import-export) and confirm that your backend supports the Bases document type and required formats.

<Callout type="warning">
  For **0.25.x**, use the [archived Pro documentation](https://dream-num.github.io/documentation/v0.25/zh-CN/guides/pro/).
</Callout>
...
`exchangeConfig` contains your application's upload, conversion, task-query, and download settings; prepare it using the [integration guide](/guides/server-integration/import-export). Merge both packages' `locale/en-US` resources into `locales` as described in [Internationalization](/guides/bases/getting-started/i18n).
