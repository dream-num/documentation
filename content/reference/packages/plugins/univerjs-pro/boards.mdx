...
The Facade can read, set, and clear the active page background with `getBackground()`, `setImageBackground(options)`, and `clearBackground()`. See the [Boards Facade reference](/reference/facade/board) for an image example.
