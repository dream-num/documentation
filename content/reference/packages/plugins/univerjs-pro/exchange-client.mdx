...
<Callout type="warning" title="Migration note">
  The package-root helpers `downloadFile`, `transformSnapshotJsonToWorkbookData`, and `transformWorkbookDataToSnapshotJson` remain exported for runtime compatibility, but are deprecated and planned for removal in v1.1.0. Use the corresponding Facade APIs in new code.
</Callout>
