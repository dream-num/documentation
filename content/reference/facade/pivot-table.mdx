...
`setFieldSetting()` also accepts `showDataAs`. These settings apply only to value fields. See [Show values as](/guides/sheets/features/pivot-table#show-values-as) for an example.
