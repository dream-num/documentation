...
<Callout type="warning" title="Migration note">
  `FBoard` now represents the active infinite page directly. `getActivePage()`, `getActivePageId()`, and `getBoardModel()` have been removed. Use `FBoard` methods directly. `insertDraft()` has also been removed; start UI import with `beginImport()` or register the relevant exchange-client package.
</Callout>
...
| Category | Methods |
| --- | --- |
| Identity and data | `getId`, `getName`, `getData`, `save` |
| History | `undo`, `redo` |
...
if (!nodes || !board.arrangeElementsInLayers(
  [[nodes[0].getId()], [nodes[1].getId()], [nodes[2].getId()]],
  { direction: 'horizontal', layerGap: 120 },
