...
<Callout type="warning" title="Shape migration">
  `FShapeBuilder` and the Slide methods `newShape()`, `updateShape()`, and `removeShape()` have been removed. Slides now use the shared Shape Facade from `@univerjs-pro/engine-shape`: call `slide.insertShape()`, `getShape()`, or `getShapes()`, then mutate the returned live `FShape` or `FConnectorShape` handle.
</Callout>
