...
`@univerjs-pro/engine-formula/facade` re-exports the Facade API from `@univerjs/engine-formula/facade`; it does not add separate Pro-only Facade methods.
...
| [`whenComputingCompleteAsync`](#whencomputingcompleteasync) | Wait until formula computing is complete |

### @univerjs/sheets-formula

| Method | Description |
|--------|-------------|
| [`calculationResultApplied`](#calculationresultapplied) | Listens for the moment when formula-calculation results are applied |
| [`onCalculationResultApplied`](#oncalculationresultapplied) | Waits for formula-calculation results to be applied |
...
Wait until formula computing is complete.
...
**Returns**
- `Promise<boolean>` — `true` when formula computing completes before timeout.
