...
| Packages | `@univerjs/engine-formula`, `@univerjs-pro/engine-formula`, `@univerjs/sheets-formula` |
| -------- | -------------------------------------------------------------------------------------- |
...
| Method                                                                          | Description                                                                                                    |
| ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| [`calculationEnd`](#calculationend)                                             | Listen for calculation end events                                                                              |
| [`calculationProcessing`](#calculationprocessing)                               | Listen for calculation processing events                                                                       |
| [`calculationStart`](#calculationstart)                                         | Listen for calculation start events                                                                            |
...
| [`getAllDependencyTrees`](#getalldependencytrees)                               | Retrieve all formula dependency trees produced during the latest dependency-analysis run                       |
...
| [`getRangeDependents`](#getrangedependents)                                     | Retrieve the full dependency trees for all formulas that depend on the specified ranges                        |
| [`getRangeDependentsAndInRangeFormulas`](#getrangedependentsandinrangeformulas) | Retrieve both the dependency trees of formulas that depend on the specified ranges and those inside the ranges |
| [`lexerTreeBuilder`](#lexertreebuilder)                                         | Access the formula lexer tree builder                                                                          |
| [`moveFormulaRefOffset`](#moveformularefoffset)                                 | Offsets the formula                                                                                            |
| [`calculationResultApplied`](#calculationresultapplied)                         | Listen after calculation results have been applied to every affected model                                     |
| [`onCalculationResultApplied`](#oncalculationresultapplied)                     | Wait until the latest calculation results have been applied                                                    |
| [`sequenceNodesBuilder`](#sequencenodesbuilder)                                 | Resolves the formula string to a node tree                                                                     |
...
| Method                                                | Description                                                                    |
| ----------------------------------------------------- | ------------------------------------------------------------------------------ |
| [`buildReference`](#buildreference)                   | Build a Sheet-range or Base-table reference and persist its cross-unit binding |
| [`upsertExternalReference`](#upsertexternalreference) | Create or update a host-owned external reference binding                       |
| [`removeExternalReference`](#removeexternalreference) | Remove a host-owned external reference binding                                 |

### @univerjs/sheets-formula

| Method                                                      | Description                                     |
| ----------------------------------------------------------- | ----------------------------------------------- |
| [`registerAsyncFunction`](#registerasyncfunction)           | Register a custom asynchronous formula function |
| [`registerFunction`](#registerfunction)                     | Register a custom synchronous formula function  |
| [`setInitialFormulaComputing`](#setinitialformulacomputing) | Update the calculation mode of the formula      |

## Cross-unit reference authoring

Import `@univerjs-pro/engine-formula/facade` to add these methods to `univerAPI.getFormula()`.
...
The returned string is a reference fragment without a leading `=`. For a cross-unit source, the method also persists the stable source binding on the host unit.

```ts
const host = univerAPI.getActiveWorkbook()
const reference = univerAPI.getFormula().buildReference({
  hostUnitId: host.getId(),
  unit: {
...
host.getActiveSheet().getRange('C1').setFormula(`=SUM(${reference})`)
```
...
Use this before writing hand-authored or imported cross-unit formula text. The options identify the host `unitId`, public `qualifier`, stable `sourceUnitId`, and `sourceUnitType`.
...
Remove a host binding by `referenceId` or `qualifier`. Existing formula text is not rewritten.

<span className="text-xs text-fd-muted-foreground">Source: `@univerjs-pro/engine-formula`</span>
...
Access the formula lexer tree builder.
...
- `LexerTreeBuilder` — The formula lexer tree builder instance.
...
- `timeout` `number` _(optional)_ — The maximum time to wait for the result, in milliseconds. Default is `30_000`.
...
Retrieve the dependency trees of all formulas _inside_ the specified ranges.
...
Retrieve the full dependency trees for all formulas that _depend on_ the
...
- `calculationMode` `CalculationMode` — The calculation mode for formula initialization. See [`CalculationMode`](#calculationmode) for details.
...
- `ignoreAbsolute` `boolean` _(optional)_ — Whether to ignore absolute references (e.g. `$A$1`).
...
- `timeout` `number` _(optional)_ — Maximum wait time in milliseconds.
...
- `options` `string | { locales?: ILocales; description?: string | IFunctionInfo; }` _(optional)_ — A description string, or an object containing `locales` and `description`.
...
Listens after formula-calculation results have been applied to every affected model.
...
- `timeout` `number` _(optional)_ — The maximum time to wait for the calculation to complete, in milliseconds. Default is `30_000`.
