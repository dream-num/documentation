...
See [Footnotes and endnotes](/guides/docs/features/footnotes) for setup and examples. Displayed numbering may change after layout; use the stable note ID with `getParagraphs(noteId)` or `getTextRange()` when addressing note content.
