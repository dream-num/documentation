...
Univer distinguishes between different types of plugins. Product plugins use the type of the instance they extend, while shared plugins use the global type:

- `UniverInstanceType.UNIVER_UNKNOWN` Global plugins, which provide Univer's core capabilities and capabilities shared by other types of plugins.
...
- `UniverInstanceType.UNIVER_PROJECT` Project type plugins
- `UniverInstanceType.UNIVER_BASE` Base type plugins
- `UniverInstanceType.UNIVER_BOARD` Board type plugins
- `UniverInstanceType.UNIVER_PDF` PDF type plugins

<Callout>By distinguishing between plugin types, we better control the lifecycle of plugins.</Callout>
...
Univer topologically sorts these runtime dependencies. If a dependency needs non-default configuration, register that dependency with its configuration before registering a plugin that depends on it; automatically registered dependencies use their default configuration. `DependentOn` only coordinates plugin constructors that are already available at runtime. Package scripts, styles, locale data, Facade entries, and workers still need to be imported separately.
...
export class YourService {
  constructor(@Inject(LifecycleService) private _lifecycleService: LifecycleService) {
    super()
...
Different types of plugins will start their lifecycle at different times. `UniverInstanceType.UNIVER_UNKNOWN` plugins will be built immediately upon registration and enter the `Starting` lifecycle, while other types of plugins will be built and go through the lifecycle when the corresponding type of file is created for the first time, allowing Univer to initialize plugins at the most appropriate time to improve loading performance.
...
<Callout>
  We use the dependency injection tool [redi](https://redi.wzhu.dev/) in Univer, you can refer to the redi documentation
  to understand the basic concepts of dependency injection and how to use it.
</Callout>
...
Some plugins support overriding modules inside the plugin. These plugins have a `override` field in their configuration object, which can be used to override modules inside the plugin, and `null` can be passed to prevent Univer from loading this module. In the [hierarchical dependency injection system](https://redi.wzhu.dev/docs/hierarchy), by overriding the module to `null`, the dependency injection system of Univer can use the implementation of the parent injector.
...
<Callout>If you use Univer CLI to initialize plugins, Univer CLI will help you divide directories.</Callout>
...
- [Formula](/blog/formula)
- [Rendering](/blog/rendering)
- [Web Worker](/blog/web-worker)
