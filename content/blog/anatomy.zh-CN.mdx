...
<Callout>
  Univer 官方文档中有一篇 [教程](/zh-CN/guides/recipes/practices/csv-import-plugin)，帮助读者通过编写一个 CSV 导入插件入门插件开发。建议在阅读博客之前完成该教程。
</Callout>
...
<Callout>
  关于如何划分插件，可以参考架构文档的 [相关章节](/zh-CN/guides/recipes/architecture/univer#如何划分插件)。
</Callout>
...
<Callout>
  了解 Univer 架构的 [分层模型](/zh-CN/guides/recipes/architecture/univer#以分层架构划分模块)。
</Callout>
...
<Callout>
  可参考 [插件自定义模型](/zh-CN/guides/recipes/practices/custom-plugin/custom-model) 了解详情。
</Callout>
...
<Callout>
  阅读 [命令系统](/zh-CN/guides/recipes/architecture/univer#命令系统) 了解什么是命令以及命令的三种类型。
</Callout>

<Callout>
  阅读 [扩展命令](/zh-CN/guides/recipes/practices/custom-command) 了解如何自定义命令。
</Callout>
...
<Callout>
  一些插件需要拓展复制粘贴或下拉填充，[扩展命令](/zh-CN/guides/recipes/practices/custom-plugin/custom-command) 章节也介绍了如何对这两个操作进行拓展。
</Callout>
...
<Callout>
  请阅读 [渲染引擎架构设计](/zh-CN/guides/recipes/architecture/rendering) 了解如何撰写渲染层代码。
</Callout>
...
<Callout>
  请阅读 [自定义 UI](/zh-CN/guides/recipes/practices/custom-plugin/custom-ui) 了解如何自定义包括 mutation 在内的命令。
</Callout>
...
<Callout>
  关于 Univer 的生命周期机制，请查看文档 [插件生命周期](/zh-CN/guides/recipes/architecture/univer#插件生命周期)。
</Callout>

<Callout>
  Univer 使用依赖注入模式来管理模块之间的依赖关系以及实例化模块。关于依赖注入的更多介绍，请参考我们的 [文档](/zh-CN/guides/recipes/architecture/univer#依赖注入)。
</Callout>
