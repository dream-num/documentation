...
- [Plugin 安装](https://github.com/dream-num/univer/blob/0d849012594882307186f33734b8b89d80a729e9/examples/src/sheets/custom/import-csv-button.ts)
- [Presets 安装](https://github.com/dream-num/univer/blob/0d849012594882307186f33734b8b89d80a729e9/examples/src/preset-sheets-core/custom-plugin/import-csv-button.ts)
...
关于插件的生命周期的更多信息，可以查看 [插件生命周期](/blog/univer#plugin-lifecycle) 了解更多。
...
追加操作栏菜单按钮使用 [`IMenuManagerService.mergeMenu`](/reference/packages/plugins/univerjs/ui) 方法。
...
通过 [`ICommandService.registerCommand`](/reference/packages/plugins/univerjs/core) 我们可以注册一个新命令，与 `IMenuManagerService` 同理，通过注入ID `ICommandService` 可以获取对应的对象实例，我们在插件构造函数添加如下代码：
...
[`ICommand.handler`](/reference/packages/plugins/univerjs/core) 是事件处理函数，当命令被触发时，该函数会被调用。
...
接下来，我们需要在点击事件中弹出文件选择框，读取用户选择的 CSV 文件，这块代码不涉 Univer，就不在本文赘述了, 可以前往 [案例介绍](/blog/csv-import-plugin#案例介绍) 自行查看 `waitUserSelectCSVFile` 方法的实现源码。
...
将 CSV 数据设置到当前表格中，可以通过 [`ICommandService.executeCommand`](/reference/packages/plugins/univerjs/core) 方法调用 `SetRangeValuesCommand` 命令来实现。
...
如果你想了解更多关于命令的内容，可以查看 [命令系统](/blog/univer#command-system) 了解更多。
...
插件的完整代码见 [案例介绍](/blog/csv-import-plugin#案例介绍)。

本插件展示了如何通过 Univer 插件系统来扩展 Univer 的 UI 和功能，希望本文可以帮助你快速上手 Univer 插件开发。

随着插件的规模增加，推荐进一步了解[模块分层](/blog/univer#hierarchical-structure)的最佳实践。

Univer 生态还处于起步阶段，如果你有任何问题或者建议，欢迎提交 PR 或者 Issue。

## 参考文档

- [插件生命周期](/blog/univer#plugin-lifecycle)
