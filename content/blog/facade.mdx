---
title: Facade API — How Univer Enhances the Development Experience
author: Wenzhao Hu
date: 2025-01-05
---
...
To achieve these goals, we introduced various [mechanisms](/guides/recipes/architecture/univer), such as the plugin system, dependency injection, and a command system. However, these mechanisms also introduced some side effects: the complexity of directly using Univer's low-level APIs made development less accessible, which conflicted with our goal of providing a "great developer experience."
...
During a recent refactor, we standardized each plugin’s runtime environment, reusing as much code as possible between browsers and Node.js. You can now [run Univer in Node.js](/guides/sheets/getting-started/node) for server-side reading, writing, and computations, or even as a headless collaborative client in Univer’s editing system.
...
Early experiments showed that lightweight custom scripts could make Facade API approachable for more users. However, that approach was insufficient for more advanced needs, such as customizing UIs or writing server-side scripts. To address this, we launched [Univer Go](https://go.univer.ai), a new product that eliminates technical barriers like setting up development environments or deploying code.
