---
title: How to find the command ID
...
For example, if you want to find the ID of the command to set the underline of a range, you can first find it in the [API Reference - Variable: SetRangeUnderlineCommand](/reference/packages/plugins/univerjs/sheets-ui). Then you can import it and get the command ID like this:
...
When creating a Univer instance, you can set the log level through the `logLevel` parameter. There are 5 log levels:
...
If you are familiar with the [architecture of Univer](/blog/univer), you should be able to easily find the ID of a command in the source code.
