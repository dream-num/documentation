---
title: Extending Univer in JavaScript
author: Univer Engineering Team
date: 2025-07-02
---

<Callout>
  If you find that the decorators are not working properly in some projects that integrate TypeScript support (usually
  because the implementation of the babel-related decorator plugin is inconsistent with TypeScript), you can also
  consider using this method to replace the decorators.
</Callout>
...
+  override onStarting() {
-    ([[ConfigService]] as Dependency[]).forEach(d => this._injector.add(d))
+    [[ConfigService]].forEach(d => this._injector.add(d))
  }
}
...
Reference [redi docs](https://redi.wzhu.dev/docs/env)。
