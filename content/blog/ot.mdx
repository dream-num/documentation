...
The architecture of Univer heavily embraces [dependency injection](https://en.wikipedia.org/wiki/Dependency_injection), and the dependency injection framework we use supports [interface-based dependency injection](https://redi.wzhu.dev/docs/identifier#identifierdecorator). During the development of the Univer collaborative module, we abstracted the network interface layer, shielding the specific network layer implementation from the collaborative logic. This abstraction allows us to achieve the following:
...
Collaborative editing is a complex field that represents the collective wisdom of [numerous](https://github.com/Operational-Transformation/ot.js/) [engineers](https://github.com/share/sharedb) and [researchers](https://github.com/ether/etherpad-lite/tree/develop/doc/public/easysync). However, collaborative editing is also intriguing as it brings a new experience to users and enhances collaboration efficiency.
