...
2. 定义国际化
   ...
   定义你所需要的国际化内容，详细的字段说明请参考 [如何在 Formula Engine 中添加公式](/blog/custom-formula#how-to-add-formulas-in-formula-engine) 的部分。同样的，多个公式就用公式名称作为 `key` 值区分。
...
7. 注册公式算法
   ...
   <Callout>
     如果 `UniverFormulaEnginePlugin` 在 `worker` 中有实例化，则需要在 `worker` 中的 `UniverFormulaEnginePlugin`
     注册公式算法，否则无法获取执行自定义公式。
   </Callout>

8. 测试

   到这里就完成了自定义公式的开发，现在可以测试一下。任一空白单元格输入 `=CUSTOMSUM` 预期能得到公式提示。这里提供一个[自定义公式 Demo](https://office.univer.ai/zh-CN/showcase/sheets/custom-formula)，供参考。
...
1. 参考我们的 [贡献指南](https://github.com/dream-num/univer/blob/dev/CONTRIBUTING.md)， fork [Univer 项目](https://github.com/dream-num/univer)，从 dev 切分支开发。分支名称统一为 `feat/formula-[函数名称]`，比如 `feat/formula-sumif`。
...
正式着手写代码之前，推荐大家先学习 [公式的架构](/blog/formula)，对公式系统有一个全面的了解。
...
详细 API 参考 [FunctionType](/reference/packages/plugins/univerjs/engine-formula)
...
2. 国际化文件
   ...
   位置在 [packages/engine-formula/src/locale/function-list/math/zh-CN.ts](https://github.com/dream-num/univer/blob/dev/packages/engine-formula/src/locale/function-list/math/zh-CN.ts#L1009)。
...
3. 公式描述
   ...
   `SUMIF` 属于 `math` 分类，描述信息在 [packages/engine-formula/src/services/formula/function-list/math.ts](https://github.com/dream-num/univer/blob/dev/packages/engine-formula/src/services/formula/function-list/math.ts#L1419)，这个文件负责整个 `math` 分类所有函数。
