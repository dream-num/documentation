...
- [Plugin Installation](https://github.com/dream-num/univer/blob/0d849012594882307186f33734b8b89d80a729e9/examples/src/sheets/custom/import-csv-button.ts)
- [Presets Installation](https://github.com/dream-num/univer/blob/0d849012594882307186f33734b8b89d80a729e9/examples/src/preset-sheets-core/custom-plugin/import-csv-button.ts)
...
For more information about the lifecycle of the plugin, you can check [Plugin Lifecycle](/blog/univer#plugin-lifecycle) for more information.
...
<Callout>
  The mounting order of plugins depends on the internal dependency relationship of the plugin. If plugin A depends on
  plugin B, then plugin B must be mounted to the Univer instance before plugin A.
</Callout>
...
We append the action bar menu button using the [`IMenuManagerService.mergeMenu`](/reference/packages/plugins/univerjs/ui) method.
...
We can register a new command through the [`ICommandService.registerCommand`](/reference/packages/plugins/univerjs/core) method. Similarly, we can obtain the corresponding object instance by injecting the ID of `ICommandService`. We add the following code to the plugin constructor function:
...
[`ICommand.handler`](/reference/packages/plugins/univerjs/core) is the event handler function. When the command is triggered, the function will be called.
...
Next, we need to pop up the file selection box in the click event, read the CSV file selected by the user.Because this code does not involve Univer, so it will not be elaborated in this article. You can go to [Case Introduction](/blog/csv-import-plugin#case-introduction) check the method `waitUserSelectCSVFile` source code.
...
If you want to learn more about commands, you can check [Command System](/blog/univer#command-system) for more information.
  ...
  handler: (accessor: IAccessor) => {
    ...
    return waitUserSelectCSVFile(({ data, rowsCount, colsCount }) => {
      ...
      const setRowCountMutationUndoParams: ISetWorksheetRowCountMutationParams =
        SetWorksheetRowCountUndoMutationFactory(accessor, setRowCountMutationRedoParams)
      ...
      const setColumnCountMutationUndoParams: ISetWorksheetColumnCountMutationParams =
        SetWorksheetColumnCountUndoMutationFactory(accessor, setColumnCountMutationRedoParams)
...
The complete code of the plugin can be found in [Case Introduction](/blog/csv-import-plugin#case-introduction).
...
As the scale of plugins increases, it is recommended to further understand the [Hierarchical Structure](/blog/univer#hierarchical-structure) to better understand the plugin system of Univer.

Univer is still in its infancy, if you have any questions or suggestions, please feel free to submit PR or Issue.

## Reference document

- [Plugin Lifecycle](/blog/univer#plugin-lifecycle)
- [Univer.registerPlugin](/reference/classes/univer)
- [ICellData](/guides/sheets/model/cell-data)
- [ICommandService.registerCommand](/reference/packages/plugins/univerjs/core)
- [ICommand](/reference/packages/plugins/univerjs/core)
