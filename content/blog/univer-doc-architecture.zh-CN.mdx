...
> 如果你对 Univer 架构还比较陌生，建议先阅读《[Univer 架构](/guides/recipes/architecture/univer)》、《[这就是 Univer](/blog/this-is-univer)》，在这两篇文章中有关于 Univer 整体架构的描述，Univer 中的命令系统、底层数据模型、渲染引擎等介绍，同时也分析了从数据层到视图层渲染，从视图层事件响应到数据层数据更新的整个过程
