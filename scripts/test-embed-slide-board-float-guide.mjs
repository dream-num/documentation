...
const origin = process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4280'
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-slide-board-float-next')
...
try {
  for (const [locale, title, headings] of [
    ['en-US', 'Slides in Boards / Pitch Storyboard', ['Variants', 'Actions', 'States']],
    ['zh-CN', 'Slides 嵌入 Boards / 提案故事板', ['变体', '操作', '状态']],
  ]) {
    const response = await page.goto(`${origin}/${locale}/showcase/embed/slides-in-boards-float`, {
    ...
    const root = page.frameLocator('iframe').first().locator('.prism-embed[data-ready=true]')
    ...
    // The native Slides page is dark by design; SDK controls keep their official white surface.
    const palettePixels = await child.evaluate((el) => {
      let count = 0
      for (const canvas of el.querySelectorAll('canvas')) {
        if (canvas.width < 300 || canvas.height < 200) continue
        const pixels = canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height).data
        for (let i = 0; i < pixels.length; i += 64)
          if (pixels[i] === 17 && pixels[i + 1] === 28 && pixels[i + 2] === 48 && pixels[i + 3] === 255) count++
      }
      return count
    })
    assert.ok(palettePixels > 1000, 'Native Slides must paint its authored navy page')
    ...
    await frame.waitForFunction(() =>
      window.univerAPI
        ...
        .getText()
        .getPlainText()
        .includes('Eight local nights.'),
    )
    const read = () =>
      frame.evaluate(() =>
        JSON.parse(
          JSON.stringify({
            host: window.univerAPI.getBoard('prism-pitch-storyboard').save(),
            child: window.univerAPI.getPresentation('prism-night-sky-pitch').save(),
          }),
        ),
      )
    const before = await read()
    assert.equal(before.host.pages.storyboard.elementOrder.length, 11)
...
console.log('PASS Prism EN/ZH native preview and theme preservation')
