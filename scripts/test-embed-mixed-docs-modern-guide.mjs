...
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-mixed-docs-modern-next')
...
try {
  for (const [locale, title, headings] of [
    ['en-US', 'Northstar / Complete Project Brief', ['Variants', 'Actions', 'States']],
    ['zh-CN', 'Northstar / 完整项目说明', ['变体', '操作', '状态']],
  ]) {
    const response = await page.goto(
      (process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4294') +
        '/' +
        locale +
        '/showcase/embed/mixed-in-docs-modern',
    ...
    const root = page.frameLocator('iframe').first().locator('.northstar-project-embed[data-ready=true]')
      ...
      frame.evaluate(() => {
        ...
        return JSON.parse(
          JSON.stringify({
            ...
            slides: a.getPresentation('northstar-seed-strategy').save(),
    ...
    assert.equal(
      await frame.evaluate(() => window.univerAPI.listEmbeds({ hostUnitId: 'northstar-seed-project' }).length),
      4,
    )
...
console.log('PASS Northstar mixed EN/ZH guide and theme checks')
