...
try {
  ...
  async function fresh() {
    ...
    await ready()
    await paint('Univer SDK Pro Slides')
  }
  ...
  for (let i = 0; i < examples.length; i++) {
    await gate('literal-' + (i + 1), async () => {
      ...
      if (i === 1) {
        assert.equal(after.slides.cover.elements.title.shapeData.shapeText.text, 'Univer SDK / Product studio')
        await paint('Univer SDK / Product studio')
      }
      if (i === 2) {
  ...
  await fresh()
  await gate('native-text-input', () => nativeText('nativeText', 'Univer SDK / Native plugin review'))
  ...
  await gate('rebuilt-owner-fresh-native-input', () =>
    nativeText('rebuiltText', 'Univer SDK / Restored product studio'),
  )
  ...
  await gate('same-owner-theme-preserves-edits', async () => {
    await run(
      "window.basicOwner = window.univerAPI; window.basicModel = window.univerAPI.getPresentation('slides-pro-demo'); window.basicModel.getSlideById('cover').getElementById('title').getText().setText('Univer SDK / Theme retained')",
    )
    const before = await snapshot()
