...
const origin = process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4284'
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-sheet-traditional-block-next')
...
try {
  for (const [locale, title, headings] of [
    ['en-US', 'Sheets in Traditional Docs / Grant Cost Schedule', ['Variants', 'Actions', 'States']],
    ['zh-CN', 'Sheets 嵌入传统文档 / 资助成本表', ['变体', '操作', '状态']],
  ]) {
    const response = await page.goto(`${origin}/${locale}/showcase/embed/sheets-in-traditional-docs-block`, {
    ...
    const root = page.frameLocator('iframe').first().locator('.estuary-embed[data-ready=true]')
    ...
    const read = () =>
      frame.evaluate(() =>
        JSON.parse(
          JSON.stringify({
            host: window.univerAPI.getDocument('estuary-grant-memorandum').save(),
            child: window.univerAPI.getWorkbook('estuary-archive-costs').save(),
          }),
        ),
      )
    const before = await read()
    assert.equal(before.host.documentStyle.documentFlavor, 1)
    assert.ok(before.host.body.dataStream.includes('A record worth keeping. Revised.'))
...
console.log('PASS Estuary EN/ZH preview and theme preservation')
