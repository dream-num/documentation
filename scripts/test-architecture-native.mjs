...
const output = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/architecture-native-current')
await fs.mkdir(output, { recursive: true })
const source = (await readShowcaseSources()).find((entry) => entry.slug === 'slides/technical-architecture-overview')
assert.ok(source)
const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'univer-architecture-native-'))
...
const report = {
  scope: 'Selected three-page production export and six literal recipes; not full native acceptance',
  directory,
  pages: [],
  examples: [],
  errors: [],
...
try {
  ...
  await page.locator('.architecture-demo[data-ready="true"]').waitFor()
  const initial = await page.evaluate(() => window.univerAPI.getPresentation('technical-architecture').save())
  ...
  await page.evaluate(() => {
    const deck = window.univerAPI.getPresentation('technical-architecture')
    deck.setActiveSlide(deck.getSlideById('component-map'))
  })
  ...
  const updated = await page.evaluate(() => window.architectureSaved)
  assert.equal(updated.slides['component-map'].elements['data-layer'].transform.left, 735)
  assert.equal(updated.slides['component-map'].elements['data-layer'].transform.top, 405)
  assert.equal(updated.slides['component-map'].elements['data-flow'].transform.left, 735)
  assert.equal(
    updated.slides['component-map'].elements.validation.shapeData.shapeText.text,
    'Layout: Local Data Layer and label moved',
  )
  assert.equal(updated.activeSlideId, 'recovery')
  await page.evaluate(() => window.univerAPI.toggleDarkMode(true))
  assert.deepEqual(
    await page.evaluate(() => window.univerAPI.getPresentation('technical-architecture').save()),
    updated,
  )
