...
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-board-docblock-theme')
await fs.mkdir(directory, { recursive: true })
const sources = (
  await Promise.all(
    ['create-demo.ts', 'data.ts'].map((name) =>
      fs.readFile('showcase/embed/boards-in-docs-block/code/' + name, 'utf8'),
    ),
  )
).join('\n')
const dependencies = [
...
const report = { passed: false, locales: [], errors: [], networkWrites: [], snapshotFailures: [] }
...
const snapshots = () =>
  page.evaluate(() => ({
    host: window.univerAPI.getDocument('pine-architecture-decision').save(),
    child: window.univerAPI.getBoard('pine-service-boundaries').save(),
  }))
try {
  for (const locale of ['en-US', 'zh-CN']) {
    await page.goto('http://127.0.0.1:4428/?locale=' + locale, { timeout: 180000 })
    await page.waitForFunction(
      () => document.querySelector('.pine-embed')?.dataset.ready === 'true',
      {},
      { timeout: 120000 },
    )
    ...
    assert.deepEqual(style, { bg: 'rgb(255, 255, 255)', flex: 'flex' })
    ...
    await page.waitForFunction(() => window.univerAPI.isDarkMode() === false)
    ...
    assert.equal(
      await page.evaluate(() =>
        window.univerAPI
          .getDocument('pine-architecture-decision')
          .getParagraphs()[1]
    ...
    for (const theme of ['dark', 'light']) {
      ...
      try {
        ...
        // Keep this a strict failure, but collect both locales/themes and verify cleanup too.
        report.snapshotFailures.push({ locale, theme, failure: error.message })
      }
    ...
    report.locales.push({
      ...
      sameOwner: true,
      fullSnapshotsPreserved: !report.snapshotFailures.some((failure) => failure.locale === locale),
      unmount: true,
    })
  }
  assert.deepEqual(report.errors, [])
  assert.deepEqual(report.networkWrites, [])
  assert.equal(report.snapshotFailures.length, 0, 'Complete Board saves must survive theme changes unchanged')
  report.passed = true
} catch (error) {
