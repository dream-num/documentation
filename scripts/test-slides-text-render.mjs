...
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/slides-text-render')
const textBox = process.env.SHOWCASE_TEXT_BOX === '1'
...
const source = (await readShowcaseSources()).find((entry) => entry.slug === 'slides/theme-and-background')
const dependencies = [
  ...new Set(
  ...
  plugins: [
    {
      ...
      load(id) {
        ...
        return `
import { createDemo } from '/showcase/slides/theme-and-background/code/create-demo.ts';
import { FUniver } from '@univerjs/core/facade';
const create = FUniver.prototype.createPresentation;
let api, presentation;
FUniver.prototype.createPresentation = function(...args) { api=this; presentation=create.apply(this,args); return presentation; };
const demo=createDemo(document.getElementById('app'));
window.probe={demo, apply(mode) {
  window.textPaint=[];
  const shape=presentation.getSlideByIndex(0).getShape('theme-card');
  if(mode==='noAutoFit') {
    shape.setShapeData({...shape.getShapeData(),isTextBox:${textBox}});
  ...
  window.textPaint=[];
  presentation.getSlideByIndex(0).getShape('theme-card').getText().setFontSize(14);
}, read() {
  const shape=presentation.getSlideByIndex(0).getShape('theme-card');
...
try {
  await page.goto('http://127.0.0.1:4185/', { waitUntil: 'domcontentloaded', timeout: 180000 })
