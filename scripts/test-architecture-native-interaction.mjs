...
const directory = path.resolve(
  process.env.SHOWCASE_RESULTS_DIR || 'test-results/architecture-native-interaction-current',
)
...
try {
  const original = JSON.parse(await fs.readFile('test-results/architecture-native-verified/exports.json', 'utf8'))[0]
  const source = (await readShowcaseSources()).find((s) => s.slug === original.slug)
  const project = await fs.mkdtemp(path.join(os.tmpdir(), 'univer-architecture-interaction-'))
  ...
  await fs.writeFile(
    ...
    source.files['/src/index.ts']
      .replace(
        'import { createArchitectureOverviewDemo }',
        'import { createArchitectureOverviewDemo, validateSnapshot }',
      )
      .replace(
        'const demo = createArchitectureOverviewDemo(container)',
        'window.architectureController = createArchitectureOverviewDemo(container)',
      )
      .replace('() => demo.dispose()', '() => window.architectureController.dispose()') +
      '\nwindow.architectureCreate = createArchitectureOverviewDemo\nwindow.architectureValidate = validateSnapshot\n',
  )
  ...
  const root = page.locator('.architecture-demo'),
    run = (code) => page.evaluate('(async()=>{\n' + code + '\n})()')
  const snapshot = () =>
    run("return JSON.parse(JSON.stringify(window.univerAPI.getPresentation('technical-architecture').save()))")
  const settle = async () => {
  ...
  async function fresh() {
    ...
    await ready()
    await paint('Technical Architecture Overview')
  }
  ...
  async function nativeText(key, text) {
    ...
    const box = await page.evaluate(() => {
      ...
      const canvas = [...document.querySelectorAll('.architecture-demo canvas')].find(
          (c) =>
  ...
  async function rebuild(data) {
    await page.evaluate((saved) => {
      window.architectureOldOwner = window.univerAPI
      window.architectureOldRoot = document.querySelector('.architecture-demo')
      window.architectureController.dispose()
      window.architectureController.dispose()
      window.architectureController = window.architectureCreate(document.getElementById('app'), false, undefined, saved)
    }, data)
    await ready()
    assert.equal(
      await run('return window.architectureOldOwner === window.univerAPI || window.architectureOldRoot.isConnected'),
      false,
    )
  ...
  await gate('normal-entry-layout-parity', async () => {
    const geometry = await page.evaluate(() => ({
      ...
      rootHeight: document.querySelector('.architecture-demo').getBoundingClientRect().height,
  ...
  await gate('literal-current-canvas-and-geometry', async () => {
      ...
      if (index < 2) {
        const position = await page.evaluate(
          (id) => {
            ...
            const render = injector.get(token).getRenderUnitById('technical-architecture')
            const object = render.scene.getObject('slide-drawing-technical-architecture-component-map-' + id)
            const paper = render.scene.getObject('slide-page-rect')
            return { left: object.left - paper.left, top: object.top - paper.top }
          },
          index === 0 ? 'data-layer' : 'data-flow',
        )
        assert.deepEqual(position, { left: 735, top: index === 0 ? 405 : 350 })
      }
      if (index === 2) {
        await paint('Layout: Local Data Layer and label moved')
        await shot('literal-moved-diagram')
      }
      if (index === 4) await paint('Keep ownership explicit')
    }
  })
  await fresh()
  await gate('native-text-input', () => nativeText('nativeText', 'Architecture / Native design review'))
  ...
  await gate('speaker-notes-native-textarea-save', async () => {
    ...
    const text = 'Mira: native architecture decision.\nRecheck local state ownership. 复查'
    ...
    assert.equal(report.evidence.notes.after.slides['component-map'].speakerNotes, text)
  ...
  await gate('speaker-notes-canvas-focus-full-history', async () => {
    ...
    assert.equal(
      ...
      report.evidence.notes.after.slides['component-map'].speakerNotes,
    )
  })
  ...
  await gate('rebuilt-owner-fresh-native-input', () =>
    nativeText('rebuiltText', 'Architecture / Restored native review'),
  )
  ...
  await gate('invalid-snapshot-preserves-active-owner', async () => {
    ...
    await assert.rejects(
      run(
        "window.architectureCreate(document.getElementById('app'), false, undefined, { id:'broken', slides:{}, slideOrder:['missing'], defaultPageSize:{width:0,height:675} })",
      ),
  ...
  for (const [lang, locale, noteLabel, saveLabel] of [
    ...
    await gate('initial-' + lang + '-native-and-all-packs', async () => {
      ...
      await page.unroute('http://127.0.0.1:4418/')
    })
  }
  await gate('active-owner-idempotent-disposal', async () => {
    ...
    await run(
      'window.architectureController.dispose(); window.architectureController.dispose(); await window.architectureController.ready',
    )
