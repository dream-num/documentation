...
const mode = process.env.SHOWCASE_MODE || 'float'
assert.ok(['float', 'tab'].includes(mode))
const slug = 'docs-in-sheets-' + mode
const hostId = mode === 'float' ? 'cedar-supplier-review' : 'juniper-repair-capacity'
const childId = mode === 'float' ? 'cedar-exception-memo' : 'juniper-capacity-assumptions'
const rootClass = mode === 'float' ? '.cedar-embed' : '.juniper-embed'
const { createServer } = await import(pathToFileURL(process.env.SHOWCASE_VITE_MODULE).href)
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-docsheet-theme-' + mode)
await fs.mkdir(directory, { recursive: true })
const sources = (
  await Promise.all(
    ['create-demo.ts', 'data.ts'].map((name) => fs.readFile('showcase/embed/' + slug + '/code/' + name, 'utf8')),
  )
).join('\n')
const dependencies = [
  ...new Set(
...
const snapshots = () =>
  page.evaluate(
    ({ hostId: host, childId: child }) => ({
      host: window.univerAPI.getWorkbook(host).save(),
      child: window.univerAPI.getDocument(child).save(),
    }),
    { hostId, childId },
  )
try {
  for (const locale of ['en-US', 'zh-CN']) {
    ...
    const root = page.locator(rootClass + '[data-ready=true]')
    ...
    if (mode === 'float') {
      ...
      canvas = child.locator('canvas').first()
    } else {
      await page.locator('[data-u-comp="slide-tab-item"]').filter({ hasText: 'Assumptions' }).click()
      await page.locator('[data-embed-sheets-sheet-tab-host]').waitFor()
      canvas = page.locator('[data-embed-sheets-sheet-tab-host] canvas').first()
    }
    await canvas.waitFor()
    await page.screenshot({ path: path.join(directory, locale + '-native.png') })
    const original = await snapshots()
    await canvas.click({ position: { x: mode === 'float' ? 180 : 600, y: 120 } })
    await page.keyboard.type('Native input ')
    await page.waitForFunction(
      (id) => window.univerAPI.getDocument(id).getBody().dataStream.includes('Native input '),
      childId,
    )
    assert.deepEqual((await snapshots()).host, original.host)
    const childEdited = (await snapshots()).child
    await page.evaluate(
      ({ hostId: host, mode: selected }) => {
        const sheet = window.univerAPI
          .getWorkbook(host)
          .getSheetBySheetId(selected === 'float' ? 'supplier-quotes' : 'weekly-capacity')
        sheet.getRange('B6').setValue(selected === 'float' ? 20100 : 5)
      },
      { hostId, mode },
    )
    await page.waitForFunction(
      ({ hostId: host, mode: selected }) => {
        const sheet = window.univerAPI
          .getWorkbook(host)
          .getSheetBySheetId(selected === 'float' ? 'supplier-quotes' : 'weekly-capacity')
        return sheet.getRange(selected === 'float' ? 'B11' : 'E11').getRawValue() === (selected === 'float' ? 1700 : 92)
      },
      { hostId, mode },
    )
        ...
        .evaluate((body) =>
          [
            ...
            ...[...body.querySelectorAll('[aria-label],[title],[placeholder]')]
              ...
              .flatMap((element) =>
                ['aria-label', 'title', 'placeholder'].map((key) => element.getAttribute(key) || ''),
              ),
          ].join('\n'),
        )
      assert.doesNotMatch(visible, /(?:embed-ui|docs-ui|sheets-ui)\.[a-zA-Z][\w.-]+/)
      await page.screenshot({ path: path.join(directory, locale + '-' + theme + '.png') })
    }
    await page.evaluate(() => window.unmountPreview())
    await page.waitForTimeout(200)
    assert.equal(await page.locator(rootClass).count(), 0)
    assert.equal(await page.evaluate(() => window.univerAPI === undefined), true)
    report.locales.push({ locale, nativeInput: true, sameOwner: true, fullEditedSnapshots: true, teardown: true })
  }
