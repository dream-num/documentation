...
const slug = 'big-data'
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/docs-big-native')
...
try {
  for (const locale of ['en-US', 'zh-CN']) {
    ...
    await page.locator('.big-doc-demo[data-ready="true"]').waitFor({ timeout: 120000 })
    await page.waitForFunction(
      () => window.univerAPI.getCurrentLifecycleStage() >= window.univerAPI.Enum.LifecycleStages.Steady,
    )
    await settle()
    const navigationToSteadyMs = Date.now() - started
    const original = await save()
    assert.ok(original.body.dataStream.length > 1000000)
    ...
    // First-page heading position observed in the fixed 1600 × 1100 screenshot.
    await page.mouse.click(550, 222)
    await page.waitForFunction(
      () =>
    ...
    try {
      await Promise.race([
        page.keyboard.insertText('Native large document edit.'),
        new Promise((_, reject) => {
          inputTimer = setTimeout(() => reject(new Error('Native text input exceeded 60 seconds')), 60000)
        }),
      ])
    } finally {
    ...
    await page.waitForFunction(() =>
      window.univerAPI.getActiveDocument().save().body.dataStream.includes('Native large document edit.'),
    )
    await page.keyboard.press('Escape')
    await settle()
    await page.screenshot({ path: path.join(directory, locale + '-edited.png') })
    const snapshot = await save()
    assert.equal(snapshot.body.dataStream.replace('Native large document edit.', ''), original.body.dataStream)
    await page.evaluate(() => {
    ...
    report.locales.push({
      locale,
      nativeEditing: true,
      navigationToSteadyMs,
      originalCharacters: original.body.dataStream.length,
      themeOwnerAndFullSave: true,
    })
    await page.evaluate(() => window.unmountPreview())
    await page.waitForTimeout(200)
    assert.equal(await page.locator('.big-doc-demo').count(), 0)
