...
const origin = process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4290'
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-board-traditional-block-next')
...
try {
  for (const [locale, title, headings] of [
    ['en-US', 'Board in Traditional Docs / Engineering Methods', ['Variants', 'Actions', 'States']],
    ['zh-CN', 'Board 嵌入传统文档 / 工程方法', ['变体', '操作', '状态']],
  ]) {
    const response = await page.goto(`${origin}/${locale}/showcase/embed/boards-in-traditional-docs-block`, {
    ...
    const root = page.frameLocator('iframe').first().locator('.delta-embed[data-ready=true]')
    ...
    const read = () =>
      frame.evaluate(() =>
        JSON.parse(
          JSON.stringify({
            host: window.univerAPI.getDocument('delta-capture-methods').save(),
            child: window.univerAPI.getBoard('delta-review-workflow').save(),
          }),
        ),
      )
    const before = await read()
    assert.equal(before.host.documentStyle.documentFlavor, 1)
    assert.ok(before.host.body.dataStream.includes('Keep the method traceable. Revised.'))
    ...
    for (const colorScheme of ['dark', 'light']) {
      ...
      assert.equal(after.child.theme.id, before.child.theme.id)
      assert.deepEqual({ ...after.child, theme: before.child.theme }, before.child)
    }
...
console.log('PASS Delta EN/ZH preview and theme preservation')
