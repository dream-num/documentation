...
const origin = process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4282'
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-base-board-float-next')
...
try {
  for (const [locale, title, headings] of [
    ['en-US', 'Bases in Boards / Research Backlog', ['Variants', 'Actions', 'States']],
    ['zh-CN', 'Bases 嵌入 Boards / 研究任务', ['变体', '操作', '状态']],
  ]) {
    const response = await page.goto(`${origin}/${locale}/showcase/embed/bases-in-boards-float`, {
    ...
    const root = page.frameLocator('iframe').first().locator('.grove-embed[data-ready=true]')
    ...
    assert.ok(await child.locator('canvas').count(), 'Native Base canvas is mounted')
    const workbench = frame.locator('[data-u-comp="workbench-layout"]').first()
    ...
    await frame.waitForFunction(
      () =>
        window.univerAPI
          ...
          .getValue('title') === 'Move the cue before the turn',
    )
    const read = () =>
      frame.evaluate(() =>
        JSON.parse(
          JSON.stringify({
            host: window.univerAPI.getBoard('grove-route-research').save(),
            child: window.univerAPI.getBase('grove-research-backlog').save(),
          }),
        ),
      )
    const before = await read()
    assert.equal(before.host.pages.research.elementOrder.length, 11)
...
console.log('PASS Grove EN/ZH native preview and theme preservation')
