...
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-mixed-docs-traditional-next')
...
try {
  for (const [locale, title, headings] of [
    ['en-US', 'Estuary / Grant Review Dossier', ['Variants', 'Actions', 'States']],
    ['zh-CN', 'Estuary / 资助评审卷宗', ['变体', '操作', '状态']],
  ]) {
    const response = await page.goto(
      (process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4296') +
        '/' +
        locale +
        '/showcase/embed/mixed-in-docs-traditional',
    ...
    const root = page.frameLocator('iframe').first().locator('.estuary-dossier-embed[data-ready=true]')
      ...
      frame.evaluate(() => {
        ...
        return JSON.parse(
          JSON.stringify({
            ...
            slides: a.getPresentation('estuary-exhibition-strategy').save(),
    ...
    assert.ok(before.host.body.dataStream.includes('Revised.'))
    assert.equal(
      await frame.evaluate(() => window.univerAPI.listEmbeds({ hostUnitId: 'estuary-exhibition-project' }).length),
      4,
    )
    for (const colorScheme of ['dark', 'light']) {
      ...
      for (const key of ['host', 'sheet', 'slides', 'base'])
...
console.log('PASS Estuary mixed EN/ZH guide and theme checks')
