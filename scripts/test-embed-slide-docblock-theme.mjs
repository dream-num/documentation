...
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-slide-docblock-theme')
await fs.mkdir(directory, { recursive: true })
const sources = (
  await Promise.all(
    ['create-demo.ts', 'data.ts'].map((name) =>
      fs.readFile('showcase/embed/slides-in-docs-block/code/' + name, 'utf8'),
    ),
  )
).join('\n')
const dependencies = [
...
const snapshots = () =>
  page.evaluate(() => ({
    host: window.univerAPI.getDocument('lighthouse-strategy-announcement').save(),
    child: window.univerAPI.getPresentation('lighthouse-strategy-deck').save(),
  }))
try {
  for (const locale of ['en-US', 'zh-CN']) {
    await page.goto('http://127.0.0.1:4428/?locale=' + locale, { timeout: 180000 })
    await page.waitForFunction(
      () => document.querySelector('.lighthouse-embed')?.dataset.ready === 'true',
      {},
      { timeout: 120000 },
    )
    ...
    await page.evaluate(() => {
      ...
      rich.getParagraphs()[1].getTextRuns()[0].setText('theme checked.')
    ...
    assert.equal(
      await page.evaluate(() =>
        window.univerAPI
          .getDocument('lighthouse-strategy-announcement')
          .getParagraphs()[1]
