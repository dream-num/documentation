...
const directory = path.resolve(process.env.SHOWCASE_RESULTS_DIR || 'test-results/embed-kestrel-formula-next')
...
try {
  for (const [locale, code, editLabel, confirmLabel] of [
    ...
    const response = await page.goto(
      (process.env.SHOWCASE_GUIDE_ORIGIN || 'http://localhost:4334') +
        '/' +
        locale +
        '/showcase/embed/mixed-to-many-products',
      { timeout: 180000 },
    )
    assert.equal(response.status(), 200)
    const root = page.frameLocator('iframe').locator('.kestrel-embed[data-ready="true"]')
    ...
    const snapshot = () =>
      frame.evaluate(() => {
        ...
        return JSON.parse(
          JSON.stringify({
            base: api.getBase('kestrel-actuals').save(),
            sheet: api.getWorkbook('kestrel-plan').save(),
            doc: api.getDocument('kestrel-planning-note').save(),
    ...
    await frame.waitForFunction(
      () => {
        ...
        const slide = api
          .getPresentation('kestrel-review')
          .getSlideById('overview')
          .getShape('actual')
          .getFormulaResult()
        const board = api.getBoard('kestrel-variance-map').getShape('actual').getFormulaResult()
        return (
          doc.length === 10 &&
          [doc[1], slide, board].every((result) => result.value === 47000 && !result.stale) &&
          api.getWorkbook('kestrel-plan').getSheetBySheetId('plan').getRange('C9').getRawValue() === 47000
        )
      },
      null,
      { timeout: 30000 },
    ...
    await root.screenshot({ path: path.join(directory, locale + '-base.png') })
    report.checks.push({
      locale,
      labels,
