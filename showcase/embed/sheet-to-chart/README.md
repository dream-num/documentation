...
**Known native limitations:** the inactive Sheet source Float is blank, and fullscreen source keyboard editing fails the ownership/recalculation gate. The strict runtime report remains FAIL. Use the explicit workbook-ID snippets to explore the working calculation chain; do not present the native editor as accepted. These defects are not hidden with substitute HTML cells, an iframe, copied chart data or a manual refresh button.
...
Evidence: `scripts/test-embed-tide-formula.mjs`, `test-results/embed-tide-formula-native-final/report.json`, `scripts/test-embed-tide-formula-guide.mjs`, `test-results/embed-tide-formula-next-final/report.json`, and `test-results/tide-formula-export-ui-final/report.json` in the documentation repository.
