...
const metadata: ShowcaseMetadata = {
  ...
  tags: {
    'en-US': ['Embed', 'Bases', 'Sheets', 'Tab', 'Forecast'],
    'zh-CN': ['嵌入', '多维表格', '电子表格', '标签', '预测'],
  },
  packages: [
    '@univerjs-pro/bases',
  ...
  guide: {
    overview: {
      'en-US':
        'Acorn reviews ten fictional installation opportunities across four customer groups. The Base tracks stages, owners and next conversations. A native table-list tab opens a separate two-sheet forecast with editable stage weights and genuine formulas; no fixture panel or duplicate toolbar.',
      'zh-CN':
        'Acorn 评估四个客户分组的十条虚构安装商机。Base 跟踪阶段、负责人和下一次沟通；原生表列表标签打开独立的双工作表预测，提供可编辑阶段权重与真实公式，不添加通用测试面板或重复工具栏。',
    },
    tryIt: {
      'en-US': [
        'Open Weighted forecast from the native Base sidebar, then visit Forecast and Assumptions.',
        'Change Qualified from 40% to 50%, then 0%, or run the first README example. Check three weighted values and native Undo/Redo.',
        'Return to Opportunities and run the Base README example. The workbook must remain unchanged.',
        'Rename a customer group in Accounts, inspect linked labels, then revisit the forecast and switch theme.',
      ],
      'zh-CN': [
        '从 Base 原生侧栏打开 Weighted forecast，切换 Forecast 和 Assumptions。',
        '将 Qualified 从 40% 改为 50%，再改为 0%，或运行 README 第一段示例；检查三项加权值及原生撤销重做。',
        '返回 Opportunities 并运行 Base README 示例；工作簿内容应保持不变。',
        '在 Accounts 重命名客户分组，检查关联标签，再返回预测并切换主题。',
      ],
    },
    expected: {
      'en-US':
        'A native Sheets tab inside Base, with Grid ribbon and official white UI. Qualified weights 40%, 50%, 0% produce 215,125 / 222,725 / 184,725. Base and workbook remain independent, not live CRM synchronization. Native print preview is verified, not physical output. No backend, messages, invoices or approvals. Reload loses edits. Selected checks pass; full acceptance remains partial. See README.',
      'zh-CN':
        'Base 内真正的 Sheets 标签，使用 Grid 菜单和官方白色界面。Qualified 权重 40%、50%、0% 分别得到 215,125 / 222,725 / 184,725。Base 与工作簿保持独立，不是实时 CRM 同步。已验证原生打印预览，不代表实际打印输出。没有后端、消息、发票或审批；刷新丢失修改。选定检查通过，完整验收仍为部分覆盖，详见 README。',
    },
  },
  variants: [
  ...
  actions: [
    ...
    ['return', 'Edit Base follow-ups independently', '独立编辑 Base 跟进记录'],
  ].map(([id, en, zh]) => ({ id, label: { 'en-US': en, 'zh-CN': zh } })),
  states: [
