  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'acorn-local-sheet',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Acorn workbook source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      started = true
      void (async () => {
        // Materialize the local workbook before restoring the native Base table-list anchor.
        const hostContext = { tableIndex: 1, tableName: 'Weighted forecast' }
        const { descriptor } = univer
          .__getInjector()
          .get(EmbedCreationService)
          .prepareCreateEmbed({
            embedId: 'acorn-forecast',
            hostUnitId: HOST_ID,
            hostType: UniverInstanceType.UNIVER_BASE,
            entry: EmbedHostEntryEnum.BasesTableListBlock,
            hostAnchorId: 'acorn-forecast-tab',
            hostContext,
            source: { unitType: UniverInstanceType.UNIVER_SHEET, ref: `#unit=${CHILD_ID}&type=sheet` },
        ...
        // This case owns one workbook: select its type without moving focus away from the Base.
        univer.__getInjector().get(IUniverInstanceService).setCurrentUnitForType(CHILD_ID)
        // The native Base sidebar opens the workbook; its model and history stay independent.
