  ...
  async function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id^="pollen-"] [data-embed-floating-menu-entry]',
      ),
    ]
    ...
    for (const release of [
      ...
      () => unmount(root),
      // Remove formula bindings while their command services and source still live.
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'pollen-local-sources',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || !source) throw new Error('Unknown Pollen source')
    ...
    const printOpened = api.addEvent(api.Event.SheetPrintOpen, ({ workbook }) => {
      if (workbook.getId() === SHEET_UNIT_ID)
        univer.__getInjector().get(EmbedFullscreenService).exit('pollen-sheet-block')
    })
      ...
      void (async () => {
        for (const [index, source] of sources.entries()) {
          ...
          const embed = api.createEmbed({
            embedId: 'pollen-' + source.kind + '-block',
            host: {
              unitId: HOST_ID,
        ...
        message.textContent =
          'The campaign brief could not load its channel Sheet. Reload to retry; details are in the console.'
