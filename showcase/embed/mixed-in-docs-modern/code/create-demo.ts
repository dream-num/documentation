...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'northstar-project-embed'
  ...
  async function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id^="northstar-project-"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'northstar-project-local-sources',
          priority: 200,
          match: { fileKinds: ['self'], unitTypes: ['sheet', 'base', 'slide', 'board'] },
          provider: {
            ensureUnit(input) {
              input.signal?.throwIfAborted()
              const source = sources.find((s) => s.id === input.ref.unit.selector && s.type === input.unitType)
              if (disposed || !source) throw new Error('Unknown Northstar project source')
    ...
    const printOpened = api.addEvent(api.Event.SheetPrintOpen, ({ workbook }) => {
      if (workbook.getId() === SHEET_UNIT_ID)
        univer.__getInjector().get(EmbedFullscreenService).exit('northstar-project-sheet-block')
    })
      ...
      void (async () => {
        for (const [index, source] of sources.entries()) {
          ...
          const embed = api.createEmbed({
            embedId: 'northstar-project-' + source.kind + '-block',
            host: {
              unitId: HOST_ID,
        ...
        message.textContent =
          'The project brief could not load all four resources. Reload to retry; details are in the console.'
