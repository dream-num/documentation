  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="vale-doc-float"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'vale-local-document',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || input.ref.unit.selector !== CHILD_ID || input.unitType !== UniverInstanceType.UNIVER_DOC)
                throw new Error('Unknown Vale document source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          embedId: 'vale-doc-float',
          host: {
            unitId: HOST_ID,
            surface: api.Enum.FEmbedHostSurface.SlideFloating,
            context: { subUnitId: PAGE_ID, left: 342, top: 175, width: 610, height: 415 },
          },
          content: { unitType: UniverInstanceType.UNIVER_DOC, ref: `#unit=${CHILD_ID}&type=doc` },
        })
        await embed.loadAsync({ signal: abort.signal })
        if (disposed) return
        // The native floating document owns the memo; the slides keep the decision context.
        ...
        message.textContent = 'The decision memo could not load. Reload to retry; details are in the console.'
