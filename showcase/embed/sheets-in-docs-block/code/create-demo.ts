...
export function createDemo(container: HTMLElement, darkMode = false, _legacyLocale: LocaleType = LocaleType.EN_US) {
  ...
  root.className = 'northstar-embed'
  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="northstar-sheet-block"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'northstar-local-sheet',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Northstar workbook source')
    ...
    const printOpened = api.addEvent(api.Event.SheetPrintOpen, ({ workbook }) => {
      if (workbook.getId() === CHILD_ID)
        univer.__getInjector().get(EmbedFullscreenService).exit('northstar-sheet-block')
    })
