  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="beacon-board-float"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'beacon-local-board',
          priority: 200,
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          embedId: 'beacon-board-float',
          host: {
            unitId: HOST_ID,
            surface: api.Enum.FEmbedHostSurface.SlideFloating,
            context: { subUnitId: PAGE_ID, left: 310, top: 180, width: 645, height: 410 },
          },
          content: { unitType: UniverInstanceType.UNIVER_BOARD, ref: `#unit=${CHILD_ID}&type=board` },
        })
        await embed.loadAsync({ signal: abort.signal })
        if (disposed) return
        // The native floating Board owns the diagram; slides retain the review narrative.
