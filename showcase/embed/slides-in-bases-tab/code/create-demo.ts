  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'avenue-local-presentation',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Avenue presentation source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        ...
        const hostContext = { tableIndex: 1, tableName: 'Campaign review' }
        const { descriptor } = univer
          .__getInjector()
          .get(EmbedCreationService)
          .prepareCreateEmbed({
            embedId: 'avenue-campaign',
            hostUnitId: HOST_ID,
            hostType: UniverInstanceType.UNIVER_BASE,
            entry: EmbedHostEntryEnum.BasesTableListBlock,
            hostAnchorId: 'avenue-campaign-tab',
            hostContext,
            displayTarget: { pageId: 'brief' },
        ...
        message.textContent = 'The campaign review could not load. Reload to retry; details are in the console.'
