...
native Canvas SheetTab. Four recovery milestones sit above an unconfirmed cause
...
The shared factory uses Grid, a local Canvas provider honoring createOptions,
...
The Canvas uses the official child-scoped EditorUIService via Canvases UI's runtime
...
the Canvas. Active-child teardown emits no browser errors in these selected runs.
...
the real native Canvas tab and official CSS with zero browser errors. The English
...
follower rewrites the saved Canvas color/format schemes. The strict full-snapshot
