  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'aurora-local-sources',
          priority: 200,
          match: { fileKinds: ['self'], unitTypes: ['slide', 'doc', 'board'] },
          provider: {
            ensureUnit(input) {
              input.signal?.throwIfAborted()
              const source = sources.find((s) => s.id === input.ref.unit.selector && s.type === input.unitType)
              if (disposed || !source) throw new Error('Unknown Aurora output source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        ...
        for (const [index, source] of sources.entries()) {
          if (disposed) return
          const names = ['Review deck', 'Brief', 'Allocation map']
          const tab = api.createEmbed({
            embedId: 'aurora-' + source.kind + '-tab',
            host: {
              unitId: HOST_ID,
        ...
        for (const source of sources) {
          if (!api.getFormula().upsertExternalReference({ unitId: source.id, ...externalReferences[0] }))
            throw new Error('Could not persist the Aurora source identity')
        }
        ...
        await sheet.insertChart(
          sheet
            ...
            .setSource({
              sheetName: 'Department plan',
              range: 'E4:F7',
              orientation: api.Enum.ChartSourceOrientation.Columns,
            })
            .setPosition({ row: 17, column: 0 })
            .setSize(850, 370)
            .setCategoryField(0)
            .setValueFields([1])
            .setPalette(['#287F83', '#8567A1', '#C39748'])
            .setTitle('Aurora / Allocation by department')
        ...
        message.textContent =
          'The Aurora workspace could not load completely. Reload to retry; details are in the console.'
