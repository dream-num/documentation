...
export function createDemo(container: HTMLElement, darkMode = false, _locale: LocaleType = LocaleType.EN_US) {
  ...
  root.className = 'harbor-fare-formula'
  ...
  async function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="harbor-fare-source-float"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'harbor-fare-local-source',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Harbor fare source')
    ...
    const formulaMenu = registerEmbedProductMenuContribution(univer.__getInjector(), {
      id: 'harbor-fares.sheet-formulas',
    ...
    const calculated = formula.calculationResultApplied(() => {
      if (sourceLoaded && !disposed) root.dataset.ready = 'true'
    })
    cleanup.push(() => calculated.dispose())
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
          ...
          host: {
            ...
            context: {
              ...
              placement: {
                ...
                bounds: { left: 850, top: 85, width: 680, height: 440 },
              },
            },
          },
          content: { unitType: UniverInstanceType.UNIVER_SHEET, ref: '#unit=' + SOURCE_ID + '&type=sheet' },
          displayTarget: { subUnitId: 'fares' },
        })
        await embed.loadAsync({ signal: abort.signal })
        if (disposed) return
        for (const [qualifier, sourceUnitId] of [
          [SOURCE_NAME, SOURCE_ID],
          ['Missing Fares', 'harbor-missing-fares'],
        ]) {
          if (
            ...
            throw new Error('Could not persist the fare reference binding')
        }
        sourceLoaded = true
        ...
        message.textContent = 'The fare source could not load. Reload to retry; details are in the console.'
