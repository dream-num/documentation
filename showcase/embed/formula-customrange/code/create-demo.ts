  ...
  async function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id^="estuary-"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'estuary-local-sources',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || !source) throw new Error('Unknown Estuary source')
    ...
    const printOpened = api.addEvent(api.Event.SheetPrintOpen, ({ workbook }) => {
      if (workbook.getId() === SHEET_UNIT_ID)
        univer.__getInjector().get(EmbedFullscreenService).exit('estuary-sheet-block')
    })
      ...
      void (async () => {
        for (const [index, source] of sources.entries()) {
          ...
          const embed = api.createEmbed({
            embedId: 'estuary-' + source.kind + '-block',
            host: {
              unitId: HOST_ID,
