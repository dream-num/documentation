...
import { BLOCK_MARKER, CHILD_ID, createChildData, createHostData, HOST_ID } from './data'
...
export function createDemo(container: HTMLElement, darkMode = false, _legacyLocale: LocaleType = LocaleType.EN_US) {
  ...
  function dispose() {
    ...
    // Release this owned child's React roots before beta.2 disposes their locale scope.
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="rowan-base-block"] [data-embed-floating-menu-entry]',
      ),
    ]
    for (const release of [
      ...cleanup.toReversed(),
      ...childRoots.map((child) => () => unmount(child)),
      () => unmount(root),
      () => univer.dispose(),
    ]) {
      try {
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'rowan-local-base',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || input.ref.unit.selector !== CHILD_ID || input.unitType !== UniverInstanceType.UNIVER_BASE)
                throw new Error('Unknown Rowan Relational Table source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          ...
          displayTarget: { tableId: 'evidence', viewId: 'evidence-grid' },
        })
        ...
        message.textContent = 'The evidence register could not load. Reload to retry; details are in the console.'
