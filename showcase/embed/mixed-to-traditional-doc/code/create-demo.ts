...
export function createDemo(
  ...
  saved?: { host: IDocumentData; sheet: IWorkbookData; base: IBaseSnapshot },
) {
  // Restore this example's own three-unit bundle, not arbitrary uploaded files.
  // Reject incomplete bundles before creating a root or replacing the global owner.
  if (saved) {
    if (
      saved.host?.id !== HOST_ID ||
      !saved.host.body ||
      saved.sheet?.id !== SHEET_UNIT_ID ||
      !saved.sheet.sheets?.[SHEET_ID] ||
      saved.base?.id !== BASE_ID ||
      !saved.base.tables?.costs
    )
      throw new Error('Restore all three original Cobalt unit IDs, the revenue sheet and costs table.')
    const resource = saved.host.resources?.find((r) => r.name === 'UNIVER_EMBED_RESOURCE_PLUGIN')
    const embeds = JSON.parse(resource?.data || '{}').embeds
    for (const [kind, sourceId] of [
      ...
      if (
        embeds?.[id]?.hostUnitId !== HOST_ID ||
        embeds[id].source?.ref !== '#unit=' + sourceId + '&type=' + kind ||
        !saved.host.body.customBlocks?.some((block) => block.blockId === embeds[id].hostAnchorId)
      )
        throw new Error('Restore both original Cobalt DocBlock resources and anchors.')
    }
  }
  const hostData = structuredClone(saved?.host ?? createHostData())
  const sheetData = structuredClone(saved?.sheet ?? createSheetData())
  const baseData = structuredClone(saved?.base ?? createBaseData())
  const root = document.createElement('div')
  root.className = 'cobalt-embed'
  ...
  async function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id^="cobalt-"] [data-embed-floating-menu-entry]',
      ),
    ]
    ...
    for (const release of [
      ...
      () => unmount(root),
      // Release inline bindings before source units and shared command services.
  ...
  try {
    ...
    const sources = [
      {
        ...
        kind: 'sheet',
        data: () => structuredClone(sheetData),
      },
      { id: BASE_ID, type: UniverInstanceType.UNIVER_BASE, kind: 'base', data: () => structuredClone(baseData) },
    ] as const
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'cobalt-local-sources',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || !source) throw new Error('Unknown Cobalt source')
    ...
    const printOpened = api.addEvent(api.Event.SheetPrintOpen, ({ workbook }) => {
      if (workbook.getId() === SHEET_UNIT_ID)
        univer.__getInjector().get(EmbedFullscreenService).exit('cobalt-sheet-block')
    })
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        for (const [index, source] of sources.entries()) {
          if (disposed) return
          const startIndex = saved
            ? 0
            : api.getDocument(HOST_ID)!.getBody()!.dataStream.indexOf(BLOCK_MARKERS[index]) - 1
          if (startIndex < 0) throw new Error('Missing project section anchor')
          const embed = saved
            ? api.getEmbed({ hostUnitId: HOST_ID, embedId: 'cobalt-' + source.kind + '-block' })
            : api.createEmbed({
                embedId: 'cobalt-' + source.kind + '-block',
                host: {
                  unitId: HOST_ID,
                ...
                ...(source.kind === 'sheet'
                  ...
                  : source.kind === 'base'
                    ? { displayTarget: { tableId: 'costs', viewId: 'costs-grid' } }
                    : {}),
              })
          if (!embed) throw new Error('Missing saved Cobalt DocBlock: ' + source.kind)
        ...
        // Saved native resources own formulas, formats and bindings, including
        // user edits/removals and unavailable-source mappings. Never reseed them.
        for (const spec of saved ? [] : INLINE_FORMULAS) {
