  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'nova-local-sheet',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Nova operating source')
              const existing = univer.__getInjector().get(IUniverInstanceService).getUnit(SHEET_UNIT_ID, input.unitType)
              if (!existing)
                univer
                  .__getInjector()
                  .get(IUniverInstanceService)
                  .createUnit(input.unitType, createSheetData(), input.createOptions)
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        ...
        const hostContext = { pageIndex: 1, pageName: 'Operating data' }
        const { descriptor } = univer
          .__getInjector()
          .get(EmbedCreationService)
          .prepareCreateEmbed({
            embedId: 'nova-operating-source',
            hostUnitId: HOST_ID,
            hostType: UniverInstanceType.UNIVER_SLIDE,
            entry: EmbedHostEntryEnum.SlidesPageListBlock,
            hostAnchorId: 'nova-operating-source-page',
            hostContext,
            source: { unitType: UniverInstanceType.UNIVER_SHEET, ref: `#unit=${SHEET_UNIT_ID}&type=sheet` },
        ...
        for (const spec of FORMULA_CARDS) {
          ...
          if (!shape) throw new Error('Missing Nova Formula Shape: ' + spec.id)
        ...
        // The native Slides page list opens the workbook; its model and history stay independent.
