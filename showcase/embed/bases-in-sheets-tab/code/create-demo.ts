...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  function dispose() {
    ...
    const childRoots = root.querySelectorAll<HTMLElement>(
      `[data-embed-child-render-mode="base-workbench"][data-embed-child-render-unit-id="${CHILD_ID}"]`,
    )
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'willow-local-base',
          priority: 200,
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          embedId: 'willow-suppliers-tab',
          host: {
            unitId: HOST_ID,
            surface: api.Enum.FEmbedHostSurface.SheetTab,
            context: { sheetIndex: 1, sheetName: 'Supplier operations' },
          },
          content: { unitType: UniverInstanceType.UNIVER_BASE, ref: `#unit=${CHILD_ID}&type=base` },
          displayTarget: { tableId: 'suppliers', viewId: 'suppliers-grid' },
        })
        await embed.loadAsync({ signal: abort.signal })
        if (disposed) return
        // The native SheetBar mounts the child when the reader selects the Supplier operations tab.
