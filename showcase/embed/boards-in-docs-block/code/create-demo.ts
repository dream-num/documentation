...
export function createDemo(container: HTMLElement, darkMode = false, _legacyLocale: LocaleType = LocaleType.EN_US) {
  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="pine-board-block"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    // Boards share the official editor service; the child scope owns its lifetime.
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'pine-local-board',
          priority: 200,
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        ...
        message.textContent =
          'The service boundary diagram could not load. Reload to retry; details are in the console.'
