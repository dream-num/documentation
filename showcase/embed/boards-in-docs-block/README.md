...
and standalone share the same factory and data. Boards use the official child-
...
dragging of the connected retry node and whole-Board restoration through Undo.
Child operations preserve the entire host document. Appending to the host title
moves the native block anchor while preserving the entire edited Board. Selected
...
remains a strict failure in EN/ZH: the same owner and edited document/Board remain,
but switching dark/light changes the Board's saved theme color/format schemes.
...
conversion or persistence is claimed. Reload loses edits. Host and Board content
