  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="maple-doc-float"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'maple-local-document',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || input.ref.unit.selector !== CHILD_ID || input.unitType !== UniverInstanceType.UNIVER_DOC)
                throw new Error('Unknown Maple document source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          embedId: 'maple-doc-float',
          host: {
            unitId: HOST_ID,
            surface: api.Enum.FEmbedHostSurface.BoardFloating,
            context: { subUnitId: PAGE_ID, left: 785, top: 230, width: 680, height: 670 },
          },
          content: { unitType: UniverInstanceType.UNIVER_DOC, ref: `#unit=${CHILD_ID}&type=doc` },
        })
        await embed.loadAsync({ signal: abort.signal })
        if (disposed) return
        // The native modern document and research Board retain distinct models and history.
        ...
        message.textContent = 'The interview brief could not load. Reload to retry; details are in the console.'
