  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="solstice-base-float"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'solstice-local-base',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || input.ref.unit.selector !== CHILD_ID || input.unitType !== UniverInstanceType.UNIVER_BASE)
                throw new Error('Unknown Solstice Relational Table source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          embedId: 'solstice-base-float',
          host: {
            unitId: HOST_ID,
            surface: api.Enum.FEmbedHostSurface.SlideFloating,
            context: { subUnitId: PAGE_ID, left: 288, top: 180, width: 670, height: 390 },
          },
          content: { unitType: UniverInstanceType.UNIVER_BASE, ref: `#unit=${CHILD_ID}&type=base` },
          displayTarget: { tableId: 'checks', viewId: 'checks-grid' },
        })
        await embed.loadAsync({ signal: abort.signal })
        if (disposed) return
        // The native floating Base owns its records; the slide keeps the review context.
        ...
        message.textContent =
          'The supplier readiness register could not load. Reload to retry; details are in the console.'
