...
Posted actuals come directly from Base structured references. Plan totals come
from the Sheet. The ten Doc inline formulas, nine Slide Formula Shapes and seven
Board Formula Shapes read both sources. The native two-series chart reads the
visible Sheet range A4:C7, including the three Base-backed actual formulas.
...
Run in order against window.univerAPI. Use Expenses for Base changes and
...
### 13. Rename the Base display on its own page
...
Base from an active embedded Sheet: that native beta.2 path currently creates
an overlapping Base canvas and blocks the table-list navigation.
...
### 15. Restore the actuals on the Base page
...
Slides and Board editores, visible Sheet totals and the actual heights of both
