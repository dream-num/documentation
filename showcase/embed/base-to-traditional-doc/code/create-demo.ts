  ...
  async function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id^="linen-"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'linen-local-base',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || input.ref.unit.selector !== CHILD_ID || input.unitType !== UniverInstanceType.UNIVER_BASE)
                throw new Error('Unknown Linen Relational Table source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          ...
          displayTarget: { tableId: 'services', viewId: 'services-grid' },
        })
        ...
        if (!api.getFormula().upsertExternalReference({ unitId: HOST_ID, ...externalReferences[0] }))
          throw new Error('Could not bind the service register to the schedule')
        ...
        message.textContent = 'The service register could not load. Reload to retry; details are in the console.'
