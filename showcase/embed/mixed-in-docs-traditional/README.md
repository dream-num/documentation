...
Relational Table, hearing Slides and release Canvas. Eight costs, seven evidence items/four
...
observed backend requests. Selected active-fullscreen Canvas disposal has no
...
Relational Table, Slides and Canvas literal Undo/Redo comparisons pass. After the initial map
...
corrected after screenshot inspection, without changing the Canvas's wider canvas.

EN/ZH guide and theme checks: `test-results/embed-mixed-docs-traditional-next-final/report.json`.
They preserve the API owner and five edited models, apart from the native Canvas's
