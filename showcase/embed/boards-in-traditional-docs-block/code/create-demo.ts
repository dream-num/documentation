  ...
  function dispose() {
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="delta-board-block"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        ...
        message.textContent = 'The capture-method diagram could not load. Reload to retry; details are in the console.'
