...
cards, lavender review, coral exceptions, amber recapture and green records vary
the visual roles. All content is original; no competitor artwork is redistributed.
...
```ts
window.univerAPI
  .getBoard('delta-review-workflow')
  .getShape('review')
  .getText()
  .setText('Exception review\n2 frames / In review')
```

Use native Undo/Redo, drag the exception card and inspect its bound connectors.
In native text editing, use Ctrl+End and Shift+Left to select the final status.
Changing a label does not recalculate counts, change other cards or update the
written interpretation. Those are independent authored data, not Formula Shapes.
Append text above the document body anchor:

```ts
window.univerAPI
  .getDocument('delta-capture-methods')
  .getParagraphs()[1]
  .appendText(' Revised.')
```
...
The selected build has 1848 modules, 18187.80 kB main JavaScript (4519.51 kB gzip)
and 125.97 kB CSS (18.48 kB gzip). Cold Next guide/playground requests took about
58/15 seconds with a Gzip listener warning. These are partial acceptance results,
not full SDK or loading-reliability certification.

All names, dates, counts and control identifiers are fictional. No scanning,
classification, calibration certification, archive publication, notification,
backend, Exchange conversion or Print/PDF output is claimed. License watermarks
remain unchanged. Reload discards local edits.
