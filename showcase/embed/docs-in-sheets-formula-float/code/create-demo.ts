...
export function createDemo(
  ...
  async function dispose() {
    ...
    abort.abort()
    if (api) {
      univer.__getInjector().get(EmbedFullscreenService).exit('saffron-doc-float')
    ...
    const childRoots = [
      ...
      ...root.ownerDocument.querySelectorAll<HTMLElement>(
        '[data-u-comp="embed-float-dom-chrome"][data-embed-id="saffron-doc-float"] [data-embed-floating-menu-entry]',
      ),
    ]
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'saffron-local-document',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (disposed || input.ref.unit.selector !== CHILD_ID || input.unitType !== UniverInstanceType.UNIVER_DOC)
                throw new Error('Unknown Saffron document source')
          ...
          host: {
            ...
            context: {
              ...
              placement: {
                ...
                bounds: { left: 500, top: 90, width: 780, height: 770 },
              },
              resizeBehavior: 'free',
            },
          },
