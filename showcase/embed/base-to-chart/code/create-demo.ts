...
export function createDemo(
  ...
  if (
    ...
    (saved.host?.id !== HOST_ID ||
      ...
      !saved.source.tables?.demand)
  ) {
    throw new Error('Restore both original Moss unit IDs, the comparison Sheet and the demand table.')
  }
  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'moss-local-base',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Moss Relational Table source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = saved
          ? api.getEmbed({ hostUnitId: HOST_ID, embedId: 'moss-demand-tab' })
          : api.createEmbed({
              embedId: 'moss-demand-tab',
              host: {
                unitId: HOST_ID,
                surface: api.Enum.FEmbedHostSurface.SheetTab,
                context: { sheetIndex: 1, sheetName: 'Demand register' },
              },
              content: { unitType: UniverInstanceType.UNIVER_BASE, ref: `#unit=${SOURCE_ID}&type=base` },
              displayTarget: { tableId: 'demand', viewId: 'demand-grid' },
            })
        if (!embed) throw new Error('The saved Moss workbook has no native Relational Table embed resource.')
        ...
        if (
          ...
          throw new Error('Could not bind the demand register')
        ...
        if (!saved)
          await sheet.insertChart(
            sheet
              ...
              .setCategoryField(0)
              .setValueFields([1, 2])
              .setPalette(['#397D69', '#C29B53'])
              .setTitle('Moss / Selected week and comparison week')
