  ...
  try {
    ...
    univer.registerPlugin(UniverEmbedPlugin, {
      resourceRefUnitProviderRegistrations: [
        {
          registrationId: 'marigold-local-presentation',
          priority: 200,
          ...
          provider: {
            ensureUnit(input) {
              ...
              if (
                ...
                throw new Error('Unknown Marigold presentation source')
    ...
    const lifecycle = api.addEvent(api.Event.LifeCycleChanged, ({ stage }) => {
      ...
      void (async () => {
        const embed = api.createEmbed({
          embedId: 'marigold-board-review',
          host: {
            unitId: HOST_ID,
            surface: api.Enum.FEmbedHostSurface.SheetTab,
            context: { sheetIndex: 1, sheetName: 'Board review' },
          },
        ...
        if (disposed) return
        // The native SheetBar mounts the child when the reader selects the Board review tab.
