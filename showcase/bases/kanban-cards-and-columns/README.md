...
Selected native verification in `test-results/base-kanban-cards/report.json` passes all four views, both original image covers, the empty column, field labels/order, six literal recipes and real card dragging that changes only the dragged repair's Status. Both host languages retain English UI, the complete edited snapshot and the same Base owner through themes. The strict overall result remains failing for column collapse: storing the configuration is not a visible collapse. Record-detail editing, arbitrary grouping fields, accessibility, narrow-screen interactions and save/reload remain unverified. These scoped checks are not full Kanban acceptance.
