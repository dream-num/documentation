...
import { IBaseUIStateService, UniverBasesUIPlugin } from '@univerjs-pro/bases-ui'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-calendar-modes'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('repair-studio-calendar'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Calendar demo cleanup failed')
  }
  try {
    ...
    const calendarState = univer.__getInjector().get(IBaseUIStateService)
    for (const viewId of ['month', 'week', 'day']) {
      calendarState.setCalendarAnchorDate(viewId, new Date(2026, 8, 8, 12, 0).getTime())
    }
    if (univerAPI.getCurrentLifecycleStage() >= univerAPI.Enum.LifecycleStages.Rendered) finish?.()
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('appointments')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('month')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The calendar demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Calendar initialization and cleanup failed', {
        cause: cleanupError,
      })
    }
    throw cause
