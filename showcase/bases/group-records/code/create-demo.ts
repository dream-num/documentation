...
export function createDemo(container: HTMLElement, darkMode = false, _locale: LocaleType = LocaleType.EN_US) {
  ...
  root.className = 'base-groups-demo'
  const editor = document.createElement('div')
  editor.className = 'base-groups-editor'
  ...
  try {
    ...
    for (const variant of VARIANTS) {
      if (!table.getViewById(variant.id)!.setGroup(variant.group))
        throw new Error('The SDK rejected the grouping rules.')
    }
    ...
    const activate = async () => {
      ...
      try {
        ...
        await ui.activateTable('returns')
        if (disposed) return
        await ui.activateView('status')
        if (disposed) return
        ui.openLeftSidebar()
