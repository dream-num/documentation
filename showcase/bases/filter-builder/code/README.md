...
On SDK `1.0.0-rc.0`, the selected native check remains **strict FAIL** for preservation of a Facade-authored OR/greater-than-or-equal filter. Merely opening the panel leaves the complete model unchanged. Editing its numeric operand from 200,000 to 210,000 changes the actual saved conjunction from `or` to `and` and the budget operator from `greaterThanOrEqual` to `is`. Thus APAC OR a large budget becomes APAC AND exactly 210,000. This is recorded with before/after complete snapshots and native panel screenshots, not corrected in the demo.
