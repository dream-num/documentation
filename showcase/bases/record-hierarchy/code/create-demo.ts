...
import { createData, PARENTS } from './data'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-record-hierarchy'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('exhibition-hierarchy'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Hierarchy demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        ...
        for (const [child, parent] of PARENTS) table.getRecordById(child)!.setParent(parentField, parent)
        await univerAPI.getBaseUI().activateTable('tasks')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('tree')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The hierarchy demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Hierarchy initialization and cleanup failed', {
        cause: cleanupError,
      })
    }
    throw cause
