...
Six original ceramic batches share one Base. Three native Grid views expose the same calculated records: Numeric cost, Text and dates, and Empty and error. Edit Units, Rate, Hours or Due using the native cell editor; the installed formula engine owns calculated fields. No output values are prefilled or computed in host JavaScript.
...
A 10% allowance applies to every batch. Formula writes explicitly declare an empty external-reference list because all references stay inside this Base. This replaces a field formula, not cached output values.

## Scope and acceptance

The factory explicitly registers the installed Pro formula engine for in-browser calculation, with six complete English locale packs and four official CSS files. Base owns its native toolbar and Grid view tabs. Preview and export share the independent factory. No workers, server, cross-file bindings, custom functions or host calculation panel are added.
