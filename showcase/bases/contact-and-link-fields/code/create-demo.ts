...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-contact-links'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('contact-link-fields'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Contact and link demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('contacts')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('directory')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The contact and link demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Contact and link initialization and cleanup failed', {
        cause,
      })
    }
    throw cause
