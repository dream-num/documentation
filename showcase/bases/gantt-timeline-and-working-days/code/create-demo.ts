...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-gantt-timeline'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('exhibition-gantt'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Gantt demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('installation')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('overview')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The Gantt demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Gantt initialization and cleanup failed')
    }
    throw cause
  }
}
