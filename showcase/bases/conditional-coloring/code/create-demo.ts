...
import { createData, createRules } from './data'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-conditional-coloring'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('cold-chain-colors'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Conditional coloring demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        const table = univerAPI.getBase('cold-chain-colors')!.getTableById('shipments')!
        for (const [viewId, rules] of Object.entries(createRules())) {
          if (!table.getViewById(viewId)!.setConditionalColorRules(rules)) {
            throw new Error(`Could not configure ${viewId} coloring`)
          }
        }
        await univerAPI.getBaseUI().activateTable('shipments')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('rows')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The conditional coloring demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Conditional coloring initialization and cleanup failed', {
        cause: cleanupError,
      })
    }
    throw cause
