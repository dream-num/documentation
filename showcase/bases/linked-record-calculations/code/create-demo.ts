...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-linked-record-calculations'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('equipment-linked-calculations'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Linked record demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('requests')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('requests-grid')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The linked record demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Linked record initialization and cleanup failed', {
        cause: cleanupError,
      })
    }
    throw cause
