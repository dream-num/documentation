...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-checks-ratings'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('print-review-fields'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Checkbox and rating demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('samples')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('review')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The checkbox and rating demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Checkbox and rating initialization and cleanup failed', {
        cause,
      })
    }
    throw cause
