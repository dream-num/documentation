...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-date-formats'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('studio-date-formats'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Date format demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        ...
        if (disposed) return
        await univerAPI.getBaseUI().activateView('comparison')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The date format demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Date format initialization and cleanup failed')
    }
    throw cause
  }
}
