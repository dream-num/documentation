...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-view-lifecycle'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('repair-view-lifecycle'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'View lifecycle demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('items')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        alert.textContent = 'The view lifecycle demo could not load. Reload to retry.'
        root.prepend(alert)
        console.error(cause)
      })
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'View lifecycle initialization and cleanup failed', {
        cause: cleanupError,
      })
    }
    throw cause
