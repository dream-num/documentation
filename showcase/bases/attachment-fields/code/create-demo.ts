...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'base-attachment-fields'
  ...
  function dispose() {
    ...
    for (const release of [
      () => lifecycle?.dispose(),
      () => unmount(root),
      () => api?.disposeUnit('repair-attachments'),
      () => univer.dispose(),
    ]) {
      try {
        release()
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Attachment fields demo cleanup failed')
  }
  try {
    ...
    const ready = rendered
      .then(async () => {
        if (disposed) return
        await univerAPI.getBaseUI().activateTable('packets')
        if (disposed) return
        await univerAPI.getBaseUI().activateView('files')
        if (disposed) return
      ...
      .catch((cause) => {
        ...
        try {
          ...
          failure = new AggregateError([cause, cleanup], 'Attachment fields initialization and cleanup failed', {
            cause,
          })
        }
        failureAlert = document.createElement('p')
        failureAlert.setAttribute('role', 'alert')
        failureAlert.textContent = 'The attachment fields demo could not load. Reload to retry.'
    ...
    try {
      ...
      throw new AggregateError([cause, cleanupError], 'Attachment fields initialization and cleanup failed', {
        cause,
      })
    }
    throw cause
