...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  function dispose() {
    ...
    resize?.disconnect()
    unmount(editor)
    api?.disposeUnit('asteria-financial-review')
    univer.dispose()
  ...
  try {
    ...
    async function fit() {
      ...
      runtime.updateViewport(pdf.getId(), {
        zoom: Math.max(
          0.1,
          Math.min(1, (area.clientWidth - 48) / (size.width / 9525), (area.clientHeight - 48) / (size.height / 9525)),
        ),
      })
    }
    void (async () => {
      ...
      alert.textContent = 'The native financial report could not initialize. See the console for details.'
      root.prepend(alert)
      console.error(error)
    })
