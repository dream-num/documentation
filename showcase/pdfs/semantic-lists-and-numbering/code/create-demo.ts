...
import { createListGallery } from './data'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'pdf-list-gallery'
  ...
  try {
    ...
    owner.univerAPI = api
    const pdf = createListGallery(api)
    function initialize() {
      if (disposed) return
      try {
        if (
        ...
        startupAlert.textContent = 'The native PDF list gallery could not initialize.'
