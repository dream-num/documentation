...
import { createParagraphGallery } from './data'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'pdf-paragraph-gallery'
  ...
  try {
    ...
    owner.univerAPI = api
    const pdf = createParagraphGallery(api)
    function initialize() {
      if (disposed) return
      try {
        if (
        ...
        startupAlert.textContent = 'The native PDF paragraph gallery could not initialize.'
