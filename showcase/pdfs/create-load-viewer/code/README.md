...
Revision: native-viewer-v2. Old audit lifecycle controls/tests are historical, not certification for this revision. Run scripts/test-pdf-viewer-native-gallery.mjs with this case selected in the local server. Binary PDF bytes are not generated by save(): that returns an SDK snapshot.
