  ...
  guide: {
    ...
    tryIt: {
      'en-US': [
        ...
        'Jump to the selected window using the native active range and scroll Facades. Navigation does not populate empty rows; no timing benchmark is displayed.',
        'Edit or clear values directly in the native worksheet; use its own Undo/Redo controls.',
        'Loading an overlapping window overwrites its existing values. The exported README includes an optional sparse snapshot download recipe, not an extra host panel.',
      ],
      'zh-CN': [
        ...
        '通过原生活动区域与滚动 Facade 跳转到选定窗口。导航不会填充空行，也不显示计时基准。',
        '直接在原生工作表中编辑或清除值，使用原生撤销与重做。',
        '载入重叠窗口会覆盖已有值。导出的 README 提供可选稀疏快照下载示例，不添加额外宿主面板。',
      ],
    },
    expected: {
      'en-US':
        'Preview and export share the factory, original data, Core CSS and complete English packs. Theme changes retain the edited owner. Capacity cleanup reads actual sparse values, not a load-history cache, and covers overlapping windows and native edits. Native loading and navigation work without a backend; an optional README recipe saves sparse JSON. Strict beta.2 history gaps remain: Undo after native typing, bulk overwrite or clearing the initial data window adds cell type fields; native typing also leaves an additional style entry. All three Redo snapshots match exactly. No model fields are normalized to claim a pass. Capacity cleanup is a compound action, not a single atomic Undo transaction. Data generation is host integration; this does not claim automatic loading, ten million resident cells, FPS, memory or universal performance.',
      'zh-CN':
        '预览与导出共享工厂、原创数据、Core CSS 和完整英文依赖包，主题切换保留已编辑实例。容量清理依据实际稀疏数值而非加载历史缓存，覆盖重叠窗口和原生编辑。原生载入和导航无需后端；README 提供可选稀疏 JSON 保存示例。beta.2 仍有严格历史缺口：原生输入、批量覆写、清空初始数据区域后的撤销会增加单元格类型字段；原生输入还留下额外样式条目。三项重做快照均精确通过，未通过归一化模型字段伪造通过。容量清理是复合操作，不是单步原子撤销事务。数据生成属于宿主集成，不声称自动载入、一千万常驻单元格、FPS、内存或通用性能结论。',
    },
  },
  variants: [
