...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'viewport-and-zoom'
  ...
  try {
    ...
    workbook.getSheetBySheetId('frozen')!.setFreeze({ startRow: 4, startColumn: 2, xSplit: 2, ySplit: 4 })
    workbook.setActiveSheet('density').getRange('D5').activate()
    ...
    try {
      ...
      throw new AggregateError([error, cleanupError], 'Viewport demo initialization and cleanup failed', {
        cause: error,
      })
    }
    throw error
