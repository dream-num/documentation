...
Selected Chinese runtime evidence now verifies the five available chart PNGs,
native B4 editing and linked PNG change, independent worksheet data, and the
three-step full-update history boundary with no page errors or network writes.
The strict report still fails on unavailable Area:
`test-results/sheet-charts-native-zh-available/report.json`. English and complete
lifecycle/export-project acceptance remain open for this revision.

Revision: native-gallery-v2. Earlier control-panel screenshots/reports do not certify this revision. Run `node scripts/test-sheet-charts.mjs` with SHOWCASE_DEMO_URL pointing to this selected playground case. The script checks native tabs, actual cell editing, PNG output and linked redraw; it also records the SDK's full-update undo sequence.
