...
const metadata = {
  ...
  description: {
    ...
    'zh-TW': '此範例演示了如何在 Univer Docs 中添加水印。水印可以使用不同的文本和樣式進行自定義。',
    'ja-JP':
      'この例では、Univer Docs にウォーターマークを追加する方法を示します。ウォーターマークは、異なるテキストやスタイルでカスタマイズできます。',
  },
  tags: {
...
export const files = {
  '/README.md': fs.readFileSync(path.resolve(__dirname, './README.md'), 'utf-8'),
