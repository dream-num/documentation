...
Install the selected export with `pnpm install`, then `pnpm dev` or `pnpm build`. Build only this selected case. Preview and standalone use the same factory and seven official stylesheets, including Boards' Shape Editor and Ink UI dependencies. Seven complete EN/ZH dependency locale packs are registered; initial language follows the document's `lang`. Changing theme updates the current owner rather than recreating it. Boards keeps its native floating toolbar; the host UI is configured with Grid preference rather than a redundant Sheets-style ribbon. Native trial notices remain intact.
