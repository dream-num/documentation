import type { FConnectorShape } from '@univerjs-pro/engine-shape/facade'
import EmbedUnitUIEnUS from '@univerjs-pro/embed-unit-ui/locale/en-US'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'slide-connectors'
  ...
  try {
    ...
    for (const id of ['straight', 'elbow', 'free']) {
      const slide = presentation.getSlideById(id)!
      const start = slide.getShape(id + '-a')!
      const end = slide.getShape(id + '-b')!
      const connector = slide.getShape(id + '-connector') as FConnectorShape
      const right = start.getConnectionSites().toSorted((a, b) => b.x - a.x)[0]
      const left = end.getConnectionSites().toSorted((a, b) => a.x - b.x)[0]
      if (!right || !left) throw new Error('Missing native shape connection sites')
      connector.bindStart(start.getId(), right.index).setEndArrow(api.Enum.ShapeArrowTypeEnum.Arrow)
      if (id === 'free') connector.setEndPoint({ x: 610, y: 410 })
      else connector.bindEnd(end.getId(), left.index)
    }
