...
export function createDemo(
  ...
  function dispose() {
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Cobalt cleanup failed')
  }
  try {
    ...
    readyTimeout = setTimeout(() => {
      if (disposed) return
      cancelAnimationFrame(readyFrame)
      const error = new Error('Cobalt native canvas did not become ready within 20 seconds.')
