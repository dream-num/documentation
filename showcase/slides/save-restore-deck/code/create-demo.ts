...
export function createDemo(
  ...
  function showStartupError(error: unknown) {
    ...
    message.textContent =
      locale === LocaleType.ZH_CN
        ? 'Tern 幻灯片未能启动。请重新加载；详细错误请查看控制台。'
        : 'The Tern slides could not start. Reload to retry; details are in the console.'
  ...
  function dispose() {
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Tern cleanup failed')
  }
  try {
    ...
    readyTimeout = setTimeout(() => {
      if (disposed) return
      cancelAnimationFrame(readyFrame)
      const error = new Error('Tern native canvas did not become ready within 20 seconds.')
