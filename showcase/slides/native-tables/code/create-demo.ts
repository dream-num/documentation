...
import { SlideTableVerticalAlignEnum, UniverSlidesTablePlugin } from '@univerjs-pro/slides-table'
...
export function createDemo(container: HTMLElement, darkMode = false) {
  ...
  root.className = 'native-slide-tables'
  ...
  try {
    ...
    for (const sample of TABLES) {
      ...
      const values = sample.values.map((row, r) =>
        row.map((value) =>
          api!.newRichText().span(value, {
            fontFamily: 'Arial',
            fontSize: 16,
            bold: r === 0,
            color: r === 0 ? sample.headerText : '#233D48',
          }),
        ),
      )
      const table = slide.insertTable(
        slide
          ...
          .setAbsolutePosition(sample.left, 175)
          .setColumnWidth(sample.width / 3)
          .setRowHeight(sample.height / 4)
          .setSize(sample.width, sample.height)
          .setOptions({ firstRow: true, bandRow: false })
          .setCellStyles(
      ...
      if (!table) throw new Error(`Could not create native table: ${sample.name}`)
    }
    owner.univerAPI = api
