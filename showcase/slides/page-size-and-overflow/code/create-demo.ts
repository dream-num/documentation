...
import { createGalleryData } from './data'
...
export function createDemo(
  ...
  const data = structuredClone(saved ?? createGalleryData())
  // Restore SDK snapshots, not arbitrary file formats. Reject broken page references before mounting.
  if (
    !data.id ||
    !Array.isArray(data.slideOrder) ||
    new Set(data.slideOrder).size !== data.slideOrder.length ||
    !data.slides ||
    !data.slideOrder.every((id) => data.slides[id]?.id === id) ||
  ...
  root.className = 'slide-size-demo'
  ...
  function showStartupError(error: unknown) {
    ...
    message.textContent =
      locale === LocaleType.ZH_CN
        ? 'Rivet 幻灯片未能启动。请重新加载；详细错误请查看控制台。'
        : 'The Rivet slides could not start. Reload to retry; details are in the console.'
  ...
  function dispose() {
    ...
    root.remove()
    if (errors.length) throw new AggregateError(errors, 'Rivet cleanup failed')
  }
  try {
    ...
    readyTimeout = setTimeout(() => {
      if (disposed) return
      cancelAnimationFrame(readyFrame)
      const error = new Error('Rivet native canvas did not become ready within 20 seconds.')
