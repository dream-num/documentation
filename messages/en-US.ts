const enUSMessages = {
  common: {
    'zoom-in': 'Zoom in',
    'zoom-out': 'Zoom out',
    'reset-zoom': 'Reset zoom',
    diagram: 'Diagram',
    'expand-code': 'Expand code',
    'collapse-code': 'Collapse code',
    'display-name': 'English',
    search: 'Search',
    'search-no-result': 'No results found',
    toc: 'Table of Contents',
    'toc-no-headings': 'No headings available',
    'last-update': 'Last updated',
    'choose-language': 'Choose language',
    'next-page': 'Next page',
    'previous-page': 'Previous page',
    'choose-theme': 'Choose theme',
    'copy-code': 'Copy code',
    'edit-on-github': 'Edit on GitHub',
  },
  tools: {
    'expand-navigation': 'Expand tools navigation',
    'collapse-navigation': 'Collapse tools navigation',
    'license-required': 'License required',
    'preview-note': 'Sheets / Docs · up to 20,000 cells · core plugins',
    'unavailable-mobile': 'Desktop only',
    'unavailable-version': 'Not released',
    'resource-count': 'Resources',
    diagnostics: {
      object: 'Expected a JSON object.',
      unknown: 'Unrecognized snapshot structure.',
      id: 'Missing or inconsistent ID.',
      collection: 'Expected an object keyed by IDs or indices.',
      order: 'Expected an ordered array of IDs.',
      missing: 'Ordered ID has no matching object.',
      duplicate: 'ID occurs more than once in the order.',
      unlisted: 'Object is absent from the display order.',
      coordinate: 'Row/column index must be a non-negative integer.',
      bounds: 'Cell lies outside rowCount/columnCount.',
      style: 'Cell references an undefined style ID.',
      body: 'Document dataStream must end with \\r\\n.',
      resource: 'Expected resources with string name and data fields.',
    },
    'feature-labels': {
      'conditional-formatting': 'Conditional formatting',
      'crosshair-highlight': 'Crosshair highlight',
      'data-validation': 'Data validation',
      drawing: 'Images',
      filter: 'Filter',
      'find-replace': 'Find and replace',
      'hyper-link': 'Hyperlinks',
      note: 'Notes',
      sort: 'Sort',
      table: 'Tables',
      'thread-comment': 'Comments',
      chart: 'Charts',
      'exchange-client': 'Import / Export',
      history: 'Edit history',
      outline: 'Outline',
      pivot: 'Pivot tables',
      print: 'Print',
      shape: 'Shapes',
      sparkline: 'Sparklines',
      collaboration: 'Collaboration',
      network: 'Network API',
      'action-recorder': 'Action recorder',
      'web-component': 'Web Components',
      'vue-adapter': 'Vue adapter',
      watermark: 'Watermark',
      embed: 'Embeds',
      resources: 'Resources',
      'advanced-formula': 'Advanced formulas',
      'range-preprocess': 'Range preprocessing',
      'comment-storage': 'Comment snapshots',
      'comment-server': 'Server comments',
      toc: 'Table of contents',
      callout: 'Callouts',
      code: 'Code blocks',
      column: 'Columns',
      formula: 'Formulas',
      latex: 'LaTeX',
      list: 'Lists',
      quote: 'Quotes',
      reference: 'References',
      sdt: 'Content controls',
      dashboard: 'Dashboard',
      mind: 'Mind maps',
      editor: 'PDF editing',
    },
    'snapshot-inspector': 'Snapshot Inspector',
    'snapshot-inspector-description': 'Inspect JSON structure, references and resource sizes locally.',
    'initialization-generator': 'Initialization Generator',
    'initialization-generator-description': 'Configure Univer and export a starter project or a UMD build project.',
    'local-only': 'Local · 8 MB',
    input: 'Snapshot JSON',
    upload: 'Open JSON file',
    sample: 'Load example',
    inspect: 'Inspect',
    format: 'Format JSON',
    clear: 'Clear',
    download: 'Download JSON',
    copy: 'Copy',
    copied: 'Copied',
    'copy-error': 'Could not copy. Select and copy the text manually.',
    'invalid-json': 'Enter a valid JSON object.',
    'too-large': 'Use a JSON file smaller than 8 MB.',
    'checks-note': 'Structure checks · up to 200 issues',
    issues: 'Issues',
    'no-issues': 'No issues found by these checks.',
    statistics: 'Statistics',
    resources: 'Top-level size',
    preview: 'Editor preview',
    'preview-error': 'Unable to open this snapshot in the editor.',
    product: 'Product',
    mode: 'Integration mode',
    plugin: 'Plugins',
    preset: 'Presets',
    language: 'SDK language',
    mobile: 'Mobile UI',
    features: 'Optional features',
    umd: 'Build a UMD bundle',
    'download-project': 'Download project script',
    files: 'Generated files',
    version: 'SDK version',
    'license-note': 'Selected features require your own client license. Replace the placeholder in index.html.',
    size: 'Bytes',
    units: 'Units',
    cells: 'Cells',
    formulas: 'Formulas',
    styles: 'Styles',
    code: 'Check',
    path: 'JSON path',
    'theme-customizer-link': 'Theme Customizer',
    section: 'Tools',
    'theme-customizer': 'Theme Customizer',
    'theme-customizer-description': 'Tune Web SDK themes, design tokens, and dark-mode previews.',
  },
  navigation: {
    overview: 'Overview',
    blog: 'Blog',
    'current-locale': 'Current',
    products: 'Products',
    reference: 'API Reference',
    showcase: 'Showcase',
    tools: 'Tools',
    documentation: 'Documentation',
    'open-site-navigation': 'Open site navigation',
    'primary-navigation': 'Primary navigation',
    'site-navigation': 'Site navigation',
    'univer-home': 'Univer Office SDK home',
  },
  'theme-customizer': {
    apply: 'Apply',
    'apply-green': 'Apply Green',
    appearance: 'Appearance',
    'back-to-editor': 'Back to Editor',
    'base-tokens': 'Base Tokens',
    'choose-color': 'Choose {label} color',
    collapse: 'Collapse',
    'core-palette': 'Core Palette',
    copied: 'Copied',
    'copy-json': 'Copy JSON',
    'copy-patch': 'Copy patch',
    dark: 'Dark',
    'design-tokens': 'Design Tokens',
    docs: 'Docs',
    expand: 'Expand',
    export: 'Export',
    'format-current-result': 'Format current result',
    'full-schema': 'Full Schema',
    highlights: 'Highlights',
    'integration-example': 'Integration example',
    'invalid-hex': 'Enter a valid HEX value, for example #466AF7.',
    json: 'JSON',
    'json-object-required': 'JSON must be an object.',
    'json-parse-failed': 'Failed to parse JSON.',
    light: 'Light',
    'live-preview': 'Live Preview',
    'loop-colors': 'Loop colors',
    mode: 'Mode',
    palette: 'Palette',
    patch: 'Patch',
    presets: 'Presets',
    'preview-action': 'Preview ->',
    'reset-default': 'Reset default',
    root: 'Root',
    scope: 'Scope',
    sheets: 'Sheets',
    'sync-current-theme': 'Sync current theme',
    'theme-json': 'Theme JSON',
    'theme-json-description': 'A valid JSON patch is merged and applied to the running ThemeService immediately.',
    title: 'Theme Customizer',
    token: 'Token',
    'token-intro': 'Generated CSS variables for design-token handoff.',
    'view-default': 'View default',
    preset: {
      default: 'Default',
      'dark-blue': 'Dark Blue',
      green: 'Green',
      orange: 'Orange',
      purple: 'Purple',
      red: 'Red',
      yellow: 'Yellow',
    },
  },
  home: {
    slogan: 'High-performance, fully customizable Office SDK',
    description:
      'Univer Office SDK brings a unified Office runtime into your product, enabling humans and AI agents to work together with freely composable and embeddable ',
    'description-split': ', ',
    'description-period': '.',
    'description-and': ' and ',
    'description-sheets': 'spreadsheets',
    'description-docs': 'documents',
    'description-slides': 'presentations',
    'banner-release': 'has been released',
    examples: {
      eyebrow: 'Interactive examples',
      title: 'One launch, every workspace',
      description:
        'Explore a connected product launch across six fully editable Univer Office SDK experiences — from planning and storytelling to visual collaboration and PDFs.',
    },
    customer: {
      title: 'Trusted by teams and companies across industries',
    },
    logocloud: {
      title: 'Trusted by teams and companies across industries',
    },
    scenes: {
      title: 'Power every workflow',
      subtitle: 'Spreadsheets, documents, presentations, and more',
      sheets: {
        title: 'Spreadsheets',
        desc: 'Familiar Excel-like interface. Supports 500+ functions, cross-sheet references, sorting & filtering, freeze panes, and rich text cell editing.',
      },
      docs: {
        'reference-language-notice':
          'This API page is currently available in English. Code signatures and identifiers are the same across languages.',
        title: 'Documents',
        desc: 'Word-like rich text editor with document statistics, rich-text links, paragraph styles, tables, lists, headers, footers, and comments.',
      },
      slides: {
        title: 'Presentations',
        desc: 'PowerPoint-like slide editor with editable SmartArt, charts, table design, and consistent Canvas2D rendering across platforms.',
      },
      bases: {
        title: 'Bases',
        desc: 'Table-driven business apps with field modeling, multiple views, record management, and low-code data workflows.',
      },
      boards: {
        title: 'Boards',
        desc: 'Infinite canvas for visual collaboration with image backgrounds, containers, shapes, printing, and PNG/JPEG export.',
      },
      pdf: {
        title: 'PDFs',
        desc: 'Create and edit PDFs with continuous page scrolling, printing, text, images, tables, lists, and annotations.',
      },
    },
    devexp: {
      title: 'Built for developers',
      subtitle: 'A full-stack framework with microkernel + plugin architecture. Get started in minutes.',
      step1: {
        title: 'Install',
        desc: 'Install the core Web SDK packages via npm, yarn, or pnpm.',
      },
      step2: {
        title: 'Choose Mode',
        desc: 'Preset mode for quick integration via preset packages; Plugin mode for fine-grained control by manually registering plugins.',
      },
      step3: {
        title: 'Configure',
        desc: 'Initialize the Web SDK instance and load the required plugins and locale resources.',
      },
      step4: {
        title: 'Render',
        desc: 'Mount to a DOM element, load data, and render the editing interface.',
      },
      preset: {
        label: 'Preset',
      },
      plugin: {
        label: 'Plugin',
      },
      copy: 'Copy',
      copied: 'Copied',
    },
    enterprise: {
      title: 'Enterprise-ready performance',
      subtitle: 'Verified benchmarks for production workloads',
      metric1: {
        label: 'Load 1M cells',
        suffix: 's',
      },
      metric2: {
        label: 'Scroll 6M cells',
        suffix: ' FPS',
      },
      metric3: {
        label: '20k formulas',
        suffix: 's',
      },
      metric4: {
        label: 'Concurrent users',
        suffix: '+',
      },
      feature1: {
        title: 'High Performance',
        desc: 'Canvas2D rendering engine with 50–60 FPS scrolling up to 6M cells.',
      },
      feature2: {
        title: 'Formula Engine',
        desc: '500+ built-in functions, custom functions, array formulas, and named ranges.',
      },
      feature3: {
        title: 'Collaboration',
        desc: 'Real-time collaboration with OT algorithm and conflict resolution.',
      },
      feature4: {
        title: 'Import & Export',
        desc: 'Import and export common formats including xlsx, csv, tsv, docx, pptx, and pdf.',
      },
      feature5: {
        title: 'Print & PDF',
        desc: 'Print content, generate PDFs, and export charts and other visual content as images.',
      },
      feature6: {
        title: 'Deployment',
        desc: 'Compose backend capabilities with Server SDK and deploy them in your own application.',
      },
      feature7: {
        title: 'Dark Mode',
        desc: 'Multiple built-in themes with dark mode support.',
      },
      feature8: {
        title: 'Plugin Extensibility',
        desc: '100+ official plugins to easily extend sheets, docs, and slides.',
      },
      feature9: {
        title: 'Internationalization',
        desc: 'Multi-language and locale support for global users.',
      },
      feature10: {
        title: 'Cross-Platform',
        desc: 'Web, Node.js, and mobile support with one codebase.',
      },
      feature11: {
        title: 'Edit History',
        desc: 'Track every change with undo, redo, and version restore.',
      },
      feature12: {
        title: 'Data Visualization',
        desc: 'Pivot tables, charts, and conditional formatting for clear insights.',
      },
    },
    headless: {
      title: 'Headless Web SDK',
      subtitle: 'Process documents on the server with native support for AI and automation',
      feature1: {
        title: 'Node.js Support',
        desc: 'Run Web SDK directly in Node.js without a browser.',
      },
      feature2: {
        title: 'No UI Rendering',
        desc: 'Exclude UI plugins and focus on data and computation.',
      },
      feature3: {
        title: 'Isomorphic Architecture',
        desc: 'Reuse the same codebase in browser and server.',
      },
      feature4: {
        title: 'AI & Automation',
        desc: 'LLMs and agents can directly call the Facade API for automated document read/write and analysis.',
      },
    },
    server: {
      title: 'Compose your server, your way',
      description:
        'Combine real-time collaboration, file conversion, and extensions from Server SDK to add the capabilities your application needs.',
      collaboration: {
        title: 'Real-time collaboration',
        description:
          'Keep teams on the same page with document synchronization and conflict resolution for multiplayer editing.',
      },
      conversion: {
        title: 'File conversion',
        description: 'Convert between Office files and document data, on its own or alongside collaboration services.',
      },
      extensions: {
        title: 'Collaboration extensions',
        description: 'Add history, comments, and Worktree as your collaboration workflow needs them.',
      },
      application: {
        title: 'Your application services',
        description:
          'Connect your existing authentication, permissions, and data storage. Deploy and manage on your own terms.',
      },
      cta: 'Read the Server SDK docs',
    },
    ecosystem: {
      title: 'Extend with plugins & integrations',
      subtitle: 'Build on a rich ecosystem',
      plugins: {
        title: 'Plugins',
        desc: '100+ official plugins covering sheets, docs, slides, collaboration, and more.',
        cta: 'View plugins',
      },
      integrations: {
        title: 'Integrations',
        desc: 'React, Vue, Angular, Next.js, Astro, Web Components, and plain JS.',
        cta: 'View integrations',
      },
      community: {
        title: 'Community',
        desc: 'Share integration experience, report issues, and join community discussions on GitHub.',
        cta: 'Join on GitHub',
      },
    },
    cta: {
      title: 'The next Office experience.\nYours to create.',
      description:
        'Compose spreadsheets, documents, and more with Univer Office SDK.\nBuild a product where people and AI agents create together.',
      start: 'Start building',
    },
    features: {
      conbination: {
        title: 'Rich plugins for diverse products',
      },
      integration: {
        title: 'Integration Anywhere',
      },
      performance: {
        title: 'Unrivaled Performance',
      },
      customizability: {
        title: 'Customize Everything',
      },
    },
  },
  blog: {
    title: 'Blog',
    slogan: 'Explore the latest news, architecture, and best practices of Univer Office SDK',
    author: 'Author',
    date: 'Published Date',
    back: 'Back',
    featured: 'Featured',
    recent: 'Recent posts',
    rss: 'RSS Feed',
  },
  docs: {
    reference: {
      label: 'On this API page',
      filter: 'Filter APIs on this page…',
      browse: 'Browse all {count} APIs',
      matches: '{count} matching APIs',
      empty: 'No matching API. Try a different name.',
    },
    'reference-language-notice':
      'This API page is currently available in English. Code signatures and identifiers are the same across languages.',
    breadcrumb: 'Breadcrumb',
    copied: 'Copied',
    'copy-page': 'Copy Page',
    'copy-page-failed': 'Could not copy page',
    'guides-navigation': 'Guides navigation',
    'guides-sidebar': 'Guides sidebar',
    'open-in-github': 'Open in GitHub',
    'open-guides-navigation': 'Open guides navigation',
    'open-navigation': 'Open {title} navigation',
    'page-actions': 'Page actions',
    pagination: 'Pagination',
    sidebar: '{title} sidebar',
    'sidebar-navigation': '{title} navigation',
    'toc-panel': 'Table of contents panel',
    'view-as-markdown': 'View as Markdown',
    version: 'Documentation version',
    toc: {
      title: 'Table of Contents',
      'no-headings': 'No headings available',
    },
    feedback: {
      question: 'How is this guide?',
      button: 'Leave a suggestion',
      message: 'Please leave your feedback or suggestions',
      submit: 'Submit',
      thanks: 'Thanks for your feedback!',
      retry: 'Submit Again',
    },
  },
  'api-table': {
    'body-parameters': 'Body Parameters',
    description: 'Description',
    example: 'Example',
    headers: 'Headers',
    'no-parameters': 'No Parameters',
    parameter: 'Parameter',
    'path-parameters': 'Path Parameters',
    'query-parameters': 'Query Parameters',
    response: 'Response',
    type: 'Type',
  },
  search: {
    label: 'Search',
    'no-result': 'No results found',
    description: 'Search documentation',
    loading: 'Searching...',
    'scope-label': 'Search scope',
    'type-to-search': 'Type to search documentation',
    clear: 'Clear search',
    close: 'Close search',
    error: 'Search is unavailable. Please try again.',
    retry: 'Retry',
    'result-count': '{count} results',
    'no-result-hint': 'Try another keyword or search all sections.',
    navigate: 'Navigate',
    open: 'Open',
    scope: {
      blog: 'Developer tutorials',
      server: 'Server SDK',
      ai: 'AI SDK',
      guides: 'Web SDK',
      reference: 'API Reference',
      all: 'All',
    },
  },
  playground: {
    demo: 'Demo',
    'click-to-show': 'Show demo',
    'click-to-hide': 'Hide demo',
    preview: 'Preview',
    'fullscreen-preview': 'Fullscreen preview',
  },
  'mdx-meta-data': {
    'server-label': 'Server dependency',
    'server-none': 'No server required for this feature',
    'license-label': 'Client license',
    'license-required': 'Configure a client license',
    'license-none': 'No client license required',
    'not-specified': 'Not specified in this guide',
    'headless-supported': 'Core APIs only; omit UI plugins',
    'browser-only': 'Browser integration only',
    limitations: 'Limitations',
    'preset-mode': 'Preset Mode',
    'plugin-mode': 'Plugin Mode',
    'preset-info': 'Preset Info',
    'plugins-info': 'Plugins Info',
    'mobile-support': 'Mobile Support',
    'mobile-support-desc': 'Optimized UI for touch devices',
    'server-required': 'Requires Server SDK integration',
    'server-required-desc': 'This feature requires server-side integration with Server SDK',
    'server-optional': 'Optional Server SDK integration',
    'server-optional-desc': 'Extend features with Server SDK',
    copied: 'Copied',
    locale: 'Locale',
    style: 'CSS',
    facade: 'Facade',
    'no-items': 'No items available',
  },
} as const

export default enUSMessages

type WidenMessageValues<T> = {
  readonly [Key in keyof T]: T[Key] extends string ? string : WidenMessageValues<T[Key]>
}

export type Messages = WidenMessageValues<typeof enUSMessages>
