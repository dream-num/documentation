import type { Messages } from './en-US'

const esESMessages = {
  common: {
    'zoom-in': 'Ampliar',
    'zoom-out': 'Reducir',
    'reset-zoom': 'Restablecer zoom',
    diagram: 'Diagrama',
    'expand-code': 'Expandir código',
    'collapse-code': 'Contraer código',
    'display-name': 'Español',
    search: 'Buscar',
    'search-no-result': 'No se encontraron resultados',
    toc: 'Tabla de contenido',
    'toc-no-headings': 'No hay títulos disponibles',
    'last-update': 'Última actualización',
    'choose-language': 'Elegir idioma',
    'next-page': 'Página siguiente',
    'previous-page': 'Página anterior',
    'choose-theme': 'Elegir tema',
    'copy-code': 'Copiar código',
    'edit-on-github': 'Editar en GitHub',
  },
  tools: {
    'expand-navigation': 'Expandir la navegación de herramientas',
    'collapse-navigation': 'Contraer la navegación de herramientas',
    'license-required': 'Requiere licencia',
    'preview-note': 'Sheets / Docs · hasta 20 000 celdas · plugins básicos',
    'checks-note': 'Estructura · hasta 200 problemas',
    'unavailable-mobile': 'Solo escritorio',
    'unavailable-version': 'Sin publicar',
    'resource-count': 'Recursos',
    diagnostics: {
      object: 'Se espera un objeto JSON.',
      unknown: 'No se reconoce la estructura del snapshot.',
      id: 'Falta el ID o no coincide con su clave.',
      collection: 'Se espera un objeto con claves de ID o índices.',
      order: 'Se espera un arreglo ordenado de ID.',
      missing: 'Un ID del orden no tiene un objeto correspondiente.',
      duplicate: 'Un ID aparece varias veces en el orden.',
      unlisted: 'El objeto no está en el orden de visualización.',
      coordinate: 'Los índices de fila/columna deben ser enteros no negativos.',
      bounds: 'La celda está fuera de rowCount/columnCount.',
      style: 'La celda hace referencia a un ID de estilo no definido.',
      body: 'El dataStream del documento debe terminar con \\r\\n.',
      resource: 'Los campos name y data de los recursos deben ser cadenas.',
    },
    'feature-labels': {
      'conditional-formatting': 'Formato condicional',
      'crosshair-highlight': 'Resaltado cruzado',
      'data-validation': 'Validación de datos',
      drawing: 'Imágenes',
      filter: 'Filtro',
      'find-replace': 'Buscar y reemplazar',
      'hyper-link': 'Hipervínculos',
      note: 'Notas',
      sort: 'Ordenación',
      table: 'Tablas',
      'thread-comment': 'Comentarios',
      chart: 'Gráficos',
      'exchange-client': 'Importar / Exportar',
      history: 'Historial',
      outline: 'Esquema',
      pivot: 'Tablas dinámicas',
      print: 'Impresión',
      shape: 'Formas',
      sparkline: 'Minigráficos',
      collaboration: 'Colaboración',
      network: 'API de red',
      'action-recorder': 'Grabación de acciones',
      'web-component': 'Web Components',
      'vue-adapter': 'Adaptador Vue',
      watermark: 'Marca de agua',
      embed: 'Contenido incrustado',
      resources: 'Recursos',
      'advanced-formula': 'Fórmulas avanzadas',
      'range-preprocess': 'Preprocesamiento de rangos',
      'comment-storage': 'Snapshots de comentarios',
      'comment-server': 'Comentarios del servidor',
      toc: 'Tabla de contenido',
      callout: 'Avisos',
      code: 'Bloques de código',
      column: 'Columnas',
      formula: 'Fórmulas',
      latex: 'LaTeX',
      list: 'Listas',
      quote: 'Citas',
      reference: 'Referencias',
      sdt: 'Controles de contenido',
      dashboard: 'Panel de control',
      mind: 'Mapas mentales',
      editor: 'Edición PDF',
    },
    'snapshot-inspector': 'Inspector de Snapshot',
    'snapshot-inspector-description':
      'Revisa localmente la estructura JSON, las referencias y el tamaño de los recursos.',
    'initialization-generator': 'Generador de configuración',
    'initialization-generator-description': 'Configura Univer y genera un proyecto inicial o de compilación UMD.',
    'local-only': 'Local · 8 MB',
    input: 'Snapshot JSON',
    upload: 'Abrir archivo JSON',
    sample: 'Cargar ejemplo',
    inspect: 'Revisar',
    format: 'Formatear JSON',
    clear: 'Borrar',
    download: 'Descargar JSON',
    copy: 'Copiar',
    copied: 'Copiado',
    'copy-error': 'No se pudo copiar. Selecciona y copia el texto.',
    'invalid-json': 'Introduce un objeto JSON válido.',
    'too-large': 'Selecciona un archivo JSON de menos de 8 MB.',
    issues: 'Resultados',
    'no-issues': 'Estas comprobaciones no detectaron problemas.',
    statistics: 'Estadísticas',
    resources: 'Tamaño de los campos principales',
    preview: 'Vista previa del editor',
    'preview-error': 'No se pudo abrir este Snapshot en el editor.',
    product: 'Producto',
    mode: 'Modo de integración',
    plugin: 'Plugins',
    preset: 'Presets',
    language: 'Idioma del SDK',
    mobile: 'Interfaz móvil',
    features: 'Funciones opcionales',
    umd: 'Compilar paquete UMD',
    'download-project': 'Descargar script del proyecto',
    files: 'Archivos generados',
    version: 'Versión del SDK',
    'license-note':
      'Las funciones seleccionadas requieren tu licencia de cliente. Sustituye el marcador en index.html.',
    size: 'Bytes',
    units: 'Unidades',
    cells: 'Celdas',
    formulas: 'Fórmulas',
    styles: 'Estilos',
    code: 'Comprobación',
    path: 'Ruta JSON',
    'theme-customizer-link': 'Personalizador de temas',
    section: 'Herramientas',
    'theme-customizer': 'Tema Personalizador',
    'theme-customizer-description': 'Temas de Tune Web SDK, tokens de diseño y vistas previas en modo oscuro.',
  },
  navigation: {
    overview: 'Descripción general',
    blog: 'Blog',
    'current-locale': 'Actual',
    products: 'Productos',
    reference: 'Referencia de API',
    showcase: 'Escaparate',
    tools: 'Herramientas',
    documentation: 'Documentación',
    'open-site-navigation': 'Navegación abierta del sitio',
    'primary-navigation': 'Navegación principal',
    'site-navigation': 'Navegación del sitio',
    'univer-home': 'Univer Office SDK inicio',
  },
  'theme-customizer': {
    apply: 'Aplicar',
    'apply-green': 'Aplicar verde',
    appearance: 'Apariencia',
    'back-to-editor': 'Volver al editor',
    'base-tokens': 'Tokens base',
    'choose-color': 'Elegir color de {label}',
    collapse: 'Contraer',
    'core-palette': 'Paleta principal',
    copied: 'Copiar',
    'copy-json': 'Copiar JSON',
    'copy-patch': 'Copiar parche',
    dark: 'Oscuro',
    'design-tokens': 'Diseño Tokens',
    docs: 'Documentos',
    expand: 'Expandir',
    export: 'Exportar',
    'format-current-result': 'Formatear resultado actual',
    'full-schema': 'Esquema completo',
    highlights: 'Aspectos destacados',
    'integration-example': 'Ejemplo de integración',
    'invalid-hex': 'Ingrese un valor HEX válido, por ejemplo #466AF7.',
    json: 'JSON',
    'json-object-required': 'JSON debe ser un objeto.',
    'json-parse-failed': 'Error al analizar JSON.',
    light: 'Luz',
    'live-preview': 'Vista previa en vivo',
    'loop-colors': 'Colores de bucle',
    mode: 'Modo',
    palette: 'Paleta',
    patch: 'Parche',
    presets: 'Preajustes',
    'preview-action': 'Vista previa ->',
    'reset-default': 'Restablecer valor predeterminado',
    root: 'Raíz',
    scope: 'Ámbito',
    sheets: 'Hojas',
    'sync-current-theme': 'Sincronizar tema actual',
    'theme-json': 'Tema JSON',
    'theme-json-description':
      'Un válido El parche JSON se fusiona y se aplica inmediatamente al ThemeService en ejecución.',
    title: 'Personalizador de temas',
    token: 'Token',
    'token-intro': 'Variables CSS generadas para la transferencia de tokens de diseño.',
    'view-default': 'Ver valor predeterminado',
    preset: {
      default: 'Predeterminado',
      'dark-blue': 'Azul oscuro',
      green: 'Verde',
      orange: 'Naranja',
      purple: 'Púrpura',
      red: 'Rojo',
      yellow: 'Amarillo',
    },
  },
  home: {
    slogan: 'Un SDK de Office de alto rendimiento y totalmente personalizable',
    description:
      'Univer Office SDK integra un entorno de ejecución de Office unificado en su producto para que personas y agentes de IA colaboren con herramientas que se pueden combinar e integrar libremente: ',
    'description-split': ',',
    'description-period': 'libremente.',
    'description-and': 'y',
    'description-sheets': 'hojas de cálculo',
    'description-docs': 'documentos',
    'description-slides': 'presentaciones',
    'banner-release': 'ha sido lanzado',
    examples: {
      eyebrow: 'Ejemplos interactivos',
      title: 'Un lanzamiento, cada espacio de trabajo',
      description:
        'Explore un lanzamiento de producto conectado en seis experiencias de Univer Office SDK totalmente editables, desde la planificación y la narrativa hasta la colaboración visual y los PDF.',
    },
    customer: {
      title: 'Con la confianza de equipos y empresas de todos los sectores',
    },
    logocloud: {
      title: 'Con la confianza de equipos y empresas de todos los sectores',
    },
    scenes: {
      title: 'Impulsa cada flujo de trabajo',
      subtitle: 'Hojas de cálculo, documentos, presentaciones y más',
      sheets: {
        title: 'Hojas de cálculo',
        desc: 'Interfaz familiar similar a Excel. Admite más de 500 funciones, referencias entre hojas, clasificación y filtrado, paneles congelados y edición de celdas de texto enriquecido.',
      },
      docs: {
        'reference-language-notice':
          'Esta página de API está disponible actualmente en inglés. Las firmas y los identificadores no cambian con el idioma.',
        title: 'Documentos',
        desc: 'Editor de texto enriquecido tipo Word con estadísticas de documentos, enlaces en texto enriquecido, estilos de párrafo, tablas, listas, encabezados, pies y comentarios.',
      },
      slides: {
        title: 'Presentaciones',
        desc: 'Editor de diapositivas tipo PowerPoint con SmartArt editable, gráficos, diseño de tablas y renderizado Canvas2D uniforme entre plataformas.',
      },
      bases: {
        title: 'Bases',
        desc: 'Aplicaciones empresariales basadas en tablas con modelado de campo, múltiples vistas, administración de registros y flujos de trabajo de datos con poco código.',
      },
      boards: {
        title: 'Tableros',
        desc: 'Lienzo infinito para colaboración visual con fondos de imagen, contenedores, formas, impresión y exportación PNG/JPEG.',
      },
      pdf: {
        title: 'PDF',
        desc: 'Cree, edite, recorra de forma continua e imprima páginas PDF con texto, imágenes, tablas, listas y anotaciones.',
      },
    },
    devexp: {
      title: 'Creado para desarrolladores',
      subtitle: 'Un marco de trabajo completo con arquitectura de microkernel + complemento. Comience en minutos.',
      step1: {
        title: 'Instalar',
        desc: 'Instale los paquetes principales de Web SDK mediante npm, hilo o pnpm.',
      },
      step2: {
        title: 'Elija el modo',
        desc: 'Modo preestablecido para una integración rápida a través de paquetes preestablecidos; Modo de complemento para un control detallado mediante el registro manual de complementos.',
      },
      step3: {
        title: 'Configurar',
        desc: 'Inicializar la instancia de Web SDK y cargar los complementos y recursos locales necesarios.',
      },
      step4: {
        title: 'Renderizar',
        desc: 'Montar en un elemento DOM, cargar datos y renderizar la edición interfaz.',
      },
      preset: {
        label: 'Preset',
      },
      plugin: {
        label: 'Plugin',
      },
      copy: 'Copiar',
      copied: 'Copiado',
    },
    enterprise: {
      title: 'Rendimiento listo para empresas',
      subtitle: 'Puntos de referencia verificados para cargas de trabajo de producción',
      metric1: {
        label: 'Cargar 1 millón de celdas',
        suffix: 's',
      },
      metric2: {
        label: 'Desplazar 6 millones de celdas',
        suffix: 'FPS',
      },
      metric3: {
        label: '20 000 fórmulas',
        suffix: 's',
      },
      metric4: {
        label: 'Concurrentes usuarios',
        suffix: '+',
      },
      feature1: {
        title: 'Alto rendimiento',
        desc: 'Motor de renderizado Canvas2D con 50–60 FPS con desplazamiento de hasta 6 millones de celdas.',
      },
      feature2: {
        title: 'Motor de fórmulas',
        desc: 'Más de 500 funciones integradas, funciones personalizadas, fórmulas de matriz y rangos con nombre.',
      },
      feature3: {
        title: 'Colaboración',
        desc: 'Colaboración en tiempo real con algoritmo OT y resolución de conflictos.',
      },
      feature4: {
        title: 'Importación y exportación',
        desc: 'Importe y exporte formatos habituales como xlsx, csv, tsv, docx, pptx y pdf.',
      },
      feature5: {
        title: 'Imprimir y PDF',
        desc: 'Imprima contenido, genere archivos PDF y exporte gráficos y otros contenidos visuales como imágenes.',
      },
      feature6: {
        title: 'Implementación',
        desc: 'Combina capacidades de backend con Server SDK y despliega tu propia aplicación.',
      },
      feature7: {
        title: 'Modo oscuro',
        desc: 'Varios temas integrados con soporte para el modo oscuro.',
      },
      feature8: {
        title: 'Extensibilidad de complementos',
        desc: 'Más de 100 complementos oficiales para ampliar fácilmente hojas, documentos y diapositivas.',
      },
      feature9: {
        title: 'Internacionalización',
        desc: 'Soporte multilingüe y local para usuarios globales.',
      },
      feature10: {
        title: 'Multiplataforma',
        desc: 'Soporte web, Node.js y dispositivos móviles con una base de código.',
      },
      feature11: {
        title: 'Historial de edición',
        desc: 'Realice un seguimiento de cada cambio con deshacer, rehacer y restaurar la versión.',
      },
      feature12: {
        title: 'Visualización de datos',
        desc: 'Tablas dinámicas, gráficos y formato condicional para obtener información clara.',
      },
    },
    headless: {
      title: 'Headless Web SDK',
      subtitle: 'Procese documentos en el servidor con soporte nativo para IA y automatización',
      feature1: {
        title: 'Node.js Soporte',
        desc: 'Ejecute Web SDK directamente en Node.js sin un navegador.',
      },
      feature2: {
        title: 'Sin representación de interfaz de usuario',
        desc: 'Excluya complementos de interfaz de usuario y céntrese en los datos y la computación.',
      },
      feature3: {
        title: 'Arquitectura isomorfa',
        desc: 'Reutilice la misma base de código en el navegador y el servidor.',
      },
      feature4: {
        title: 'IA y automatización',
        desc: 'Los LLM y los agentes pueden llamar directamente a la API de Facade para lectura/escritura y análisis automatizados de documentos.',
      },
    },
    server: {
      title: 'Combine las capacidades de su servidor',
      description:
        'Combine la colaboración en tiempo real, la conversión de archivos y las extensiones de Server SDK según las necesidades de su aplicación.',
      collaboration: {
        title: 'Colaboración en tiempo real',
        description:
          'Permita que los equipos editen el mismo contenido con sincronización de documentos y resolución de conflictos.',
      },
      conversion: {
        title: 'Conversión de archivos',
        description:
          'Convierta entre archivos de Office y datos de documentos, de forma independiente o junto con los servicios de colaboración.',
      },
      extensions: {
        title: 'Extensiones de colaboración',
        description: 'Añada historial, comentarios y Worktree según lo necesite su flujo de colaboración.',
      },
      application: {
        title: 'Los servicios de su aplicación',
        description:
          'Conecte la autenticación, los permisos y el almacenamiento existentes. Despliegue y gestione en su propio entorno.',
      },
      cta: 'Leer la documentación de Server SDK',
    },
    ecosystem: {
      title: 'Extienda con complementos e integraciones',
      subtitle: 'Construya un ecosistema rico',
      plugins: {
        title: 'Complementos',
        desc: 'Más de 100 complementos oficiales que cubren hojas, documentos, diapositivas, colaboración y más.',
        cta: 'Ver complementos',
      },
      integrations: {
        title: 'Integraciones',
        desc: 'React, Vue, Angular, Next.js, Astro, componentes web y JS simple.',
        cta: 'Ver integraciones',
      },
      community: {
        title: 'Comunidad',
        desc: 'Comparta experiencias de integración, comunique problemas y participe en la comunidad en GitHub.',
        cta: 'Únase en GitHub',
      },
    },
    cta: {
      title: 'La próxima experiencia Office.\nCréela usted.',
      description:
        'Combine hojas de cálculo, documentos y más con Univer Office SDK.\nCree un producto donde las personas y los agentes de IA creen juntos.',
      start: 'Empezar a crear',
    },
    features: {
      conbination: {
        title: 'Complementos completos para diversos productos',
      },
      integration: {
        title: 'Integración en cualquier lugar',
      },
      performance: {
        title: 'Rendimiento incomparable',
      },
      customizability: {
        title: 'Personalice todo',
      },
    },
  },
  blog: {
    title: 'Blog',
    slogan: 'Explore las últimas noticias, arquitectura y mejores prácticas de Univer Office SDK',
    author: 'Autor',
    date: 'Publicado Fecha',
    back: 'Volver',
    featured: 'Destacados',
    recent: 'Publicaciones recientes',
    rss: 'Feed RSS',
  },
  docs: {
    reference: {
      label: 'API de esta página',
      filter: 'Filtrar las API de esta página…',
      browse: 'Ver las {count} API',
      matches: '{count} API encontradas',
      empty: 'No se encontraron API. Prueba con otro nombre.',
    },
    'reference-language-notice':
      'Esta página de API está disponible actualmente en inglés. Las firmas y los identificadores no cambian con el idioma.',
    breadcrumb: 'Breadcrumb',
    copied: 'Copiado',
    'copy-page': 'Copiar página',
    'copy-page-failed': 'No se pudo copiar la página',
    'guides-navigation': 'Navegación de guías',
    'guides-sidebar': 'Barra lateral de guías',
    'open-in-github': 'Abrir en GitHub',
    'open-guides-navigation': 'Abrir navegación de guías',
    'open-navigation': 'Abrir navegación de {title}',
    'page-actions': 'Acciones de página',
    pagination: 'Paginación',
    sidebar: 'Barra lateral de {title}',
    'sidebar-navigation': 'Navegación de {title}',
    'toc-panel': 'Panel de tabla de contenido',
    'view-as-markdown': 'Ver como Markdown',
    version: 'Versión de la documentación',
    toc: {
      title: 'Tabla de contenido',
      'no-headings': 'Sin encabezados disponible',
    },
    feedback: {
      question: '¿Cómo es esta guía?',
      button: 'Deje una sugerencia',
      message: 'Por favor, deje sus comentarios o sugerencias',
      submit: 'Enviar',
      thanks: '¡Gracias por sus comentarios!',
      retry: 'Enviar de nuevo',
    },
  },
  'api-table': {
    'body-parameters': 'Parámetros del cuerpo',
    description: 'Descripción',
    example: 'Ejemplo',
    headers: 'Encabezados',
    'no-parameters': 'Sin parámetros',
    parameter: 'Parámetro',
    'path-parameters': 'Ruta Parámetros',
    'query-parameters': 'Parámetros de consulta',
    response: 'Respuesta',
    type: 'Tipo',
  },
  search: {
    label: 'Búsqueda',
    'no-result': 'No se encontraron resultados',
    description: 'Buscar documentación',
    loading: 'Buscando...',
    'scope-label': 'Alcance de la búsqueda',
    'type-to-search': 'Tipo para buscar documentación',
    clear: 'Borrar búsqueda',
    close: 'Cerrar búsqueda',
    error: 'La búsqueda no está disponible. Inténtalo de nuevo.',
    retry: 'Reintentar',
    'result-count': '{count} resultados',
    'no-result-hint': 'Prueba otra palabra clave o busca en todas las secciones.',
    navigate: 'Navegar',
    open: 'Abrir',
    scope: {
      blog: 'Tutoriales de desarrollo',
      server: 'Server SDK',
      ai: 'AI SDK',
      guides: 'Web SDK',
      reference: 'Referencia de API',
      all: 'Todos',
    },
  },
  playground: {
    demo: 'Demo',
    'click-to-show': 'Mostrar demo',
    'click-to-hide': 'Ocultar demo',
    preview: 'Vista previa',
    'fullscreen-preview': 'Vista previa en pantalla completa',
  },
  'mdx-meta-data': {
    'server-label': 'Dependencia del servidor',
    'server-none': 'Esta función no requiere servidor',
    'license-label': 'Licencia de cliente',
    'license-required': 'Configura una licencia de cliente',
    'license-none': 'No requiere licencia de cliente',
    'not-specified': 'No se especifica en esta guía',
    'headless-supported': 'Solo API principales, sin plugins UI',
    'browser-only': 'Solo integración en navegador',
    limitations: 'Limitaciones',
    'preset-mode': 'Modo preestablecido',
    'plugin-mode': 'Modo complemento',
    'preset-info': 'Información preestablecida',
    'plugins-info': 'Información de complementos',
    'mobile-support': 'Soporte móvil',
    'mobile-support-desc': 'IU optimizada para dispositivos táctiles',
    'server-required': 'Requiere integración con Server SDK',
    'server-required-desc': 'Esta función requiere integración del lado del servidor con Server SDK',
    'server-optional': 'Integración opcional de Server SDK',
    'server-optional-desc': 'Amplía las funciones con Server SDK',
    copied: 'Copiado',
    locale: 'Configuración regional',
    style: 'CSS',
    facade: 'Fachada',
    'no-items': 'No hay elementos disponibles',
  },
} satisfies Messages

export default esESMessages
