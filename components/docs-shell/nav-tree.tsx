...
function isActive(item: NavTreeItem, pathname: string): boolean {
  if (item.url === pathname) return true
  return item.children.some(child => isActive(child, pathname))
}

function getControlsId(item: NavTreeItem) {
  return `nav-tree-${item.id.replace(/[^\w-]/g, '-')}`
}

function NavContent({ item }: { item: NavTreeItem }) {
  return (
    <>
...
function NavTreeNode({
  ...
  if (item.type === 'separator') {
    return (
      <li
        className="
          mt-5 px-2 text-xs font-medium tracking-normal text-muted-foreground
          first:mt-0
        "
      >
        {item.name}
    ...
    return (
      <li>
        <details className="group" open={level === 0 || active}>
          ...
          <ul
            className={clsx(
              'mt-1 ml-5.5 space-y-1 border-l border-border/70 pl-1.5',
              level > 0 && 'ml-4',
            )}
            id={controlsId}
          >
