...
export function ShowcaseContent({ items, lang, counts }: ShowcaseContentProps) {
  ...
  const activeFilter: ProductId | 'all' = PRODUCT_IDS.includes(requestedFilter as ProductId)
    ? (requestedFilter as ProductId)
    : 'all'
  ...
  const filteredByType = useMemo(() => {
    return activeFilter === 'all' ? items : items.filter((item) => item.product === activeFilter)
  }, [items, activeFilter])
  ...
  return (
    <>
      ...
      <div className="mt-8 flex justify-center">
        <div className="flex flex-wrap items-center justify-center gap-1 rounded-lg bg-muted p-[3px] text-muted-foreground">
          <button
            onClick={() => handleFilterClick('all')}
            className={clsx(
              `inline-flex h-[calc(100%-1px)] items-center justify-center gap-1.5 rounded-md border border-transparent px-3 py-1 text-sm font-medium whitespace-nowrap transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none sm:px-4`,
              activeFilter === 'all'
          ...
          {PRODUCT_IDS.map((product) => (
            <button
              ...
              className={clsx(
                `inline-flex h-9 items-center justify-center gap-1.5 rounded-md border border-transparent px-3 py-1 text-sm font-medium whitespace-nowrap transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none`,
            ...
            >
              ...
              <span className="rounded-full bg-background/70 px-1.5 text-[10px]">{counts[product]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            className="h-10 w-full rounded-lg border bg-background pr-8 pl-9 text-sm transition-colors outline-none focus:border-ring focus:ring-2 focus:ring-ring/20 dark:bg-neutral-900/50"
          />
          {searchQuery && (
            <button
        ...
        <div className="inline-flex h-10 items-center rounded-lg bg-muted p-[3px]">
          <button
            ...
            className={clsx(
              `inline-flex h-[calc(100%-1px)] items-center justify-center rounded-md border border-transparent px-3 py-1 text-sm font-medium whitespace-nowrap transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none`,
              currentView === 'grid'
                ? `bg-background text-foreground shadow-sm dark:border-input dark:bg-input/30`
                : 'hover:text-foreground',
            )}
